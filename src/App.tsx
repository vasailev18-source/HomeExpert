...
import FoundationBlueprint, {
  drawToCanvasBase64,
} from "./components/FoundationBlueprint";
import { exportToExcel, exportAllToExcel } from "./utils/excelExport";
import { exportProExpertSystemToExcel } from "./utils/expertExcelExport";
import {
  ResponsiveContainer,
  PieChart,
  Pie,
...
const PRESETS: ProjectPreset[] = [
  {
    ...
    glazingType: GlazingType.STANDARD,
    facadeTech: FacadeType.WET,
    materialLabel: "Газобетон • Плиты ПБК • Мокрый Фасад",
  },
  {
    ...
    glazingType: GlazingType.STANDARD,
    facadeTech: FacadeType.WET,
    materialLabel: "Котелец • Ж/б Монолит • Штукатурка",
  },
  {
    ...
    roofType: RoofType.GABLE_METAL,
    glazingType: GlazingType.PANORAMIC,
    facadeTech: FacadeType.VENTILATED,
    materialLabel: "Каркас • Дерево • Вентфасад",
  },
  {
    ...
    glazingType: GlazingType.PANORAMIC,
    facadeTech: FacadeType.VENTILATED,
    materialLabel: "Кирпич • Монолит • Вентфасад",
  },
  {
    ...
    glazingType: GlazingType.STANDARD,
    facadeTech: FacadeType.WET,
    materialLabel: "Керамзитоб. • Плиты ПБК • Штукатурка",
  },
  {
    ...
    roofType: RoofType.HIP_CERAMIC,
    glazingType: GlazingType.PANORAMIC,
    facadeTech: FacadeType.FACE_BRICK,
    materialLabel: "Котелец • Монолит • Облиц. Кирпич",
    hasBasement: true,
  },
  {
    ...
    glazingType: GlazingType.PANORAMIC,
    facadeTech: FacadeType.FACE_BRICK,
    materialLabel: "Кирпич • Монолит • Облиц. Кирпич",
  },
];
...
export function getCategoryDetailedItems(
  ...
  switch (catId) {
    ...
    case "03": {
      ...
      if (w > 0) {
        ...
        const consumedConsumables =
          totalCost - manualLaborLong - manualLaborTrans;
    ...
    case "04": {
      // Разработка грунта (JCB)
      const v = m.excavationVolumeM3 || 0;
      if (v > 0) {
        const isPile = opt.id === "piles" || opt.id === "pile";
        const machineryCost = isPile
          ? Math.round(v * 105) + 4500
          : Math.round(v * 105);
        const manualCost = Math.round(v * 50);

        items.push({
          name: isPile
            ? "Выемка почвы экскаватором JCB + глубинное бурение скважин ямобуром"
            : "Механизированная разработка котлована/траншей экскаватором-погрузчиком JCB 3CX",
          qty: v,
          unit: "м³",
    ...
    case "10": {
      ...
      if (totalCost > 0) {
        ...
        const diff =
          totalCost - (sandCost + wpCost + rebarCost + conCost + laborCost);
...
export default function App() {
  ...
  const [wallMaterial, setWallMaterial] = useState<BuildingWallMaterial>(
    BuildingWallMaterial.GASOBETON,
  );
  const [slabMaterial, setSlabMaterial] = useState<SlabMaterial>(
    SlabMaterial.MONOLITH,
  );
  const [roofType, setRoofType] = useState<RoofType>(RoofType.GABLE_METAL);
  const [glazingType, setGlazingType] = useState<GlazingType>(
    GlazingType.STANDARD,
  );
  const [facadeTech, setFacadeTech] = useState<FacadeType>(FacadeType.WET);
  const [hvacSystem, setHvacSystem] = useState<string>("");
  const [ventSystem, setVentSystem] = useState<string>("");
  const [waterSystem, setWaterSystem] = useState<string>("");
  const [sewageSystem, setSewageSystem] = useState<string>("");
  const [elecSystem, setElecSystem] = useState<string>("");
  const [lowVoltSystem, setLowVoltSystem] = useState<string>("");
  const [finishSystem, setFinishSystem] = useState<string>("");
  const [windowSystem, setWindowSystem] = useState<string>("");
  
  const [buildingType, setBuildingType] = useState<"HOUSE" | "GARAGE">("HOUSE");
  const [hasPit, setHasPit] = useState<boolean>(false);
  const [soilType, setSoilType] = useState<SoilType>(SoilType.LOAM);
  const [hasGeologyData, setHasGeologyData] = useState<boolean>(false);
  const [soilResistance, setSoilResistance] = useState<number>(150);
  const [groundwaterDepth, setGroundwaterDepth] = useState<number>(2.5);
  const [safetyFactor, setSafetyFactor] = useState<number>(1.8);
  const [futureFlooringExtension, setFutureFlooringExtension] =
    useState<boolean>(false);
  const [hasBasement, setHasBasement] = useState<boolean>(false);
  const [landSlope, setLandSlope] = useState<number>(0);
  const [buildQuality, setBuildQuality] = useState<"ECONOMY" | "STANDARD" | "PREMIUM">("STANDARD");
  ...
  const [projectReservePercent, setProjectReservePercent] =
    useState<number>(12);

  useEffect(() => {
    if (buildQuality === "ECONOMY") {
      setIncludeVAT(false); // Can be skipped if building individually or "brigada" 
      setProjectReservePercent(0); // Zero reserve
      // Note: we KEEP includeSubDesign and includeSupervision because they are strictly required for Cadastre commissioning
    } else if (buildQuality === "PREMIUM") {
      setIncludeVAT(true);
      setProjectReservePercent(15);
    } else {
      setIncludeVAT(true);
      setProjectReservePercent(10);
    }
  }, [buildQuality]);
  ...
  const [budgetViewMode, setBudgetViewMode] = useState<"single" | "compare">(
    "single",
  );

  // Sub-tab state for advanced decision support engine
  const [activeEngTab, setActiveEngTab] = useState<
    | "dashboard"
    | "risks"
    | "geology"
    | "utilities"
  ...
  const [reinforcementChoice, setReinforcementChoice] = useState<
    "AUTO" | "RECOMMENDED" | "USER"
  >("AUTO");
  const [rebarClassMainOverride, setRebarClassMainOverride] =
    useState<string>("");
  const [rebarClassSecondaryOverride, setRebarClassSecondaryOverride] =
    useState<string>("");
  const [mainBarDiameterOverride, setMainBarDiameterOverride] =
    useState<number>(0);
  const [secondaryBarDiameterOverride, setSecondaryBarDiameterOverride] =
    useState<number>(0);
  const [longitudinalBarsCountOverride, setLongitudinalBarsCountOverride] =
    useState<number>(0);
  const [stirrupSpacingOverride, setStirrupSpacingOverride] =
    useState<number>(0);
  const [protectiveLayerBottomOverride, setProtectiveLayerBottomOverride] =
    useState<number>(0);
  const [protectiveLayerSideOverride, setProtectiveLayerSideOverride] =
    useState<number>(0);
  const [cornerReinforcementOverride, setCornerReinforcementOverride] =
...
Нажмите одну из кнопок **"🚀 Запустить Быстрый расчет"** или **"🔍 Полный аудит"** ниже, чтобы я изучил параметры вашей мансарды или дома и подготовил технический отчет. Или просто напишите свой вопрос!`,
  ...
  const toggleScopeProp = (
    id: string,
    prop: "included" | "purchased" | "completed" | "ownerSupplied" | "optional",
  ) => {
  ...
  if (width <= 0) {
    validationErrors.push(
      "Ширина здания не может быть отрицательной или равной нулю.",
    );
  }
  if (length <= 0) {
    validationErrors.push(
      "Длина здания не может быть отрицательной или равной нулю.",
    );
  }
  if (floors < 1) {
    validationErrors.push(
      "Недопустимая этажность строения: по нормам проектирования этажность не может быть менее 1.",
    );
  }
  if (floors > 4) {
    validationErrors.push(
      "Предельное число надземных этажей: этажность по СНиП для ИЖС не должна превышать 4 этажей.",
    );
  }
  if (floorHeight <= 0) {
    validationErrors.push(
      "Высота этажа не может быть отрицательной или равной нулю.",
    );
  }
  if (safetyFactor < 1) {
    validationErrors.push(
      "Коэффициент запаса по несущей способности (γ_n) должен быть не менее 1.0.",
    );
  }
  if (landSlope < 0) {
    validationErrors.push(
      "Уклон застраиваемой территории не может быть отрицательным.",
    );
  }
  if (groundwaterDepth < 0) {
    validationErrors.push(
      "Глубина залегания грунтовых вод (УГВ) не может быть отрицательной.",
    );
  }
  ...
  const currentInput: CalculatorInput = {
    ...
    windowSystem,
    soilType,
    customSoilType: hasGeologyData ? soilType : undefined,
    customSoilResistance: hasGeologyData ? soilResistance : undefined,
    ...
    mainBarDiameter:
      mainBarDiameterOverride > 0 ? mainBarDiameterOverride : undefined,
    secondaryBarDiameter:
      secondaryBarDiameterOverride > 0
        ? secondaryBarDiameterOverride
        : undefined,
    longitudinalBarsCount:
      longitudinalBarsCountOverride > 0
        ? longitudinalBarsCountOverride
        : undefined,
    stirrupSpacing:
      stirrupSpacingOverride > 0 ? stirrupSpacingOverride : undefined,
    protectiveLayerBottom:
      protectiveLayerBottomOverride > 0
        ? protectiveLayerBottomOverride
        : undefined,
    protectiveLayerSide:
      protectiveLayerSideOverride > 0 ? protectiveLayerSideOverride : undefined,
    cornerReinforcement: cornerReinforcementOverride,
  };

  const results: CalculationResults = calculateFoundation(currentInput);
  if (results.walls) {
    results.whiteBox = calculateWhiteBox(currentInput, results.walls);
  }

  // Active validation safeguards & structural engineering blockers
  if (results.settlementTotalMM > results.settlementLimitMM) {
    validationErrors.push(
      `🚨 ПРЕВЫШЕН ЛИМИТ ОСАДКИ: Расчётная осадка S (${results.settlementTotalMM} мм) превосходит предельно допустимую S_limit (${results.settlementLimitMM} мм) по нормам NCM F.02.02-2008. СТРОИТЕЛЬСТВО НЕ ПРИЕМЛЕМО БЕЗ УКРЕПЛЕНИЯ.`,
    );
  }

  if (
    region === MoldovaRegion.SOUTH &&
    wallMaterial === BuildingWallMaterial.KOTELET
  ) {
    validationErrors.push(
      "🚨 СЕЙСМИЧЕСКАЯ БЛОКИРОВКА (НЕ ПРИЕМЛЕМО ДЛЯ СТРОИТЕЛЬСТВА): В сейсмозоне у очага Вранчи силой 8 баллов (Юг) категорически запрещено возведение стен из хрупкой каменной кладки котельца без жесткого монолитного каркаса и на свайно-ростверковом фундаменте!",
    );
  }

  // Cross-validation (Consistency Checks)
  if (wallMaterial === BuildingWallMaterial.AERATED_CONCRETE && ventSystem === "Естественная вентиляция (Вентканалы)") {
    validationErrors.push("⚠️ ПРОТИВОРЕЧИЕ: Выбран энергоэффективный газобетон, но установлена естественная вентиляция. Большая часть тепла будет уходить через вентканалы. Рекомендуется рекуперация.");
  }
  if (buildQuality === "PREMIUM" && elecSystem === "Эконом (Минимум розеток, ВВГнг)") {
    validationErrors.push("⚠️ ПРОТИВОРЕЧИЕ: Выбран премиум уровень дома, но заложена бюджетная электрика. Это сильно ограничит комфорт эксплуатации.");
  }
  if (facadeTech === FacadeType.VENTILATED && windowSystem === "Эконом (3/4 камеры, 2 стекла)") {
    validationErrors.push("⚠️ ПРОТИВОРЕЧИЕ: Выбран дорогой и теплый вентилируемый фасад в сочетании с дешевыми холодными окнами. Тепло будет уходить через окна.");
  }
  ...
  useEffect(() => {
    ...
    includeSupervision,
    projectReservePercent,
    rebarClassMainOverride,
    rebarClassSecondaryOverride,
    mainBarDiameterOverride,
    secondaryBarDiameterOverride,
    longitudinalBarsCountOverride,
    stirrupSpacingOverride,
    protectiveLayerBottomOverride,
    protectiveLayerSideOverride,
    cornerReinforcementOverride,
  ]);

  const selectedOption =
    results.options.find((o) => o.id === activeFndId) || results.options[0];
  ...
  const scopeMetrics = React.useMemo(() => {
    ...
    const computeItemCosts = (id: string, opt: any) => {
      ...
      if (id === "curing") {
        ...
        if (
          rawW === 0 &&
          (results.utilities?.lowCurrent?.costMDL || 0) > rawM
        ) {
        ...
        if (
          rawW === 0 &&
          (results.utilities?.spareSleeves?.costMDL || 0) > rawM
        ) {
        ...
        const detailedItems = getCategoryDetailedItems(
          id,
          opt,
          landSlope,
          groundwaterDepth,
        ...
        detailedItems.forEach((item) => {
          const isGeotextileMatch =
            !sectionsActive.geotextile &&
            (item.name.toLowerCase().includes("геотекстиль") ||
              item.name.toLowerCase().includes("typar") ||
              item.name.toLowerCase().includes("фильтрующего"));
          const isPgsMatch =
            !sectionsActive.pgs &&
            (item.name.toLowerCase().includes("пгс") ||
              item.name.toLowerCase().includes("песчано-гравийной") ||
              item.name.toLowerCase().includes("гравийно-песчаный"));
          const isInsulationMatch =
            !sectionsActive.insulation &&
            (item.name.toLowerCase().includes("xps") ||
              item.name.toLowerCase().includes("утепления") ||
              item.name.toLowerCase().includes("плит") ||
              item.name.toLowerCase().includes("пенополистирола") ||
              item.name.toLowerCase().includes("penoplex"));
          const isWaterproofingMatch =
            !sectionsActive.waterproofing &&
            (item.name.toLowerCase().includes("битумного") ||
              item.name.toLowerCase().includes("гидроизоляцион") ||
              item.name.toLowerCase().includes("мастики") ||
              item.name.toLowerCase().includes("рулонн") ||
              item.name.toLowerCase().includes("наплавляем") ||
              item.name.toLowerCase().includes("герметизации"));
          const isProtectionMatch =
            !sectionsActive.waterproofing_protection &&
            (item.name.toLowerCase().includes("клей-пена") ||
              item.name.toLowerCase().includes("дюбел") ||
              item.name.toLowerCase().includes("мембрана") ||
              item.name.toLowerCase().includes("профиль") ||
              item.name.toLowerCase().includes("полиуретановая"));

          if (
            isGeotextileMatch ||
            isPgsMatch ||
            isInsulationMatch ||
            isWaterproofingMatch ||
            isProtectionMatch
          ) {
            return;
          }
    ...
    allPosIds.forEach((id) => {
      ...
      if (
        id === "10" &&
        selectedOption.costEstimate.roughFloorCostMDL === undefined
      )
        return;
    ...
    const scopeVAT = scopeSettings["12"] || {
      included: true,
      purchased: false,
      completed: false,
      ownerSupplied: false,
    ...
    if (includeVAT && scopeVAT.included) {
      const vatProject = Math.round(totalProjectCost * 0.2);
  ...
  const getWindowStyle = (roof: RoofType) => {
    switch (roof) {
      ...
      case RoofType.HIP_CERAMIC:
        return {
          frameClass:
            "border-amber-800 bg-[#e0f2fe]/30 shadow-md rounded-t-full",
  ...
  const renderFrenchDormer = (id: string) => {
    return (
      <div
        key={id}
        className="w-6 h-[20px] relative flex flex-col justify-end items-center mb-[1px] pointer-events-none"
      >
  ...
  const renderFrenchBalcony = (id: string) => {
    return (
      <div
        key={id}
        className="h-full flex flex-col justify-end items-center relative w-12 pointer-events-none"
      >
  ...
  const renderWindow = (id: string, className = "w-7 h-4") => {
    ...
    if (w.panes === 1) {
      return (
        <div
          key={id}
          className={`${className} border ${w.frameClass} p-[1px] relative overflow-hidden flex`}
        >
      ...
      return (
        <div
          key={id}
          className={`${className} border ${w.frameClass} p-[1px] relative flex gap-[1px]`}
        >
      ...
      return (
        <div
          key={id}
          className={`${className} border ${w.frameClass} p-[1px] relative overflow-hidden flex gap-[1px]`}
        >
      ...
      return (
        <div
          key={id}
          className={`${className} border ${w.frameClass} flex flex-col justify-between p-[1px] relative`}
        >
          <div className="h-full w-full relative flex flex-col justify-between">
            ...
            <div
              className={`absolute left-1/2 top-0 bottom-0 w-[1px] ${w.innerClass} -translate-x-1/2`}
            />
          </div>
        </div>
      );
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          role: "assistant",
          content:
            data.content || "Извините, не удалось получить осмысленный ответ.",
          timestamp: new Date().toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          }),
      ...
      setMessages((prev) => [
        ...prev,
        {
          ...
          role: "assistant",
          content:
            "⚠️ Ошибка связи с сервером ИИ. Вы можете продолжать полноценно пользоваться интерактивным калькулятором на панели слева.",
          timestamp: new Date().toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          }),
    ...
    <div className="min-h-screen bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-sky-50 via-slate-50 to-neutral-100 text-slate-700 flex flex-col font-sans selection:bg-blue-500 selection:text-white">
      ...
      <div className="bg-amber-50/90 backdrop-blur-md border-b border-amber-200/50 px-4 py-3 text-xs text-amber-800 text-center flex items-center justify-center gap-2.5 shadow-[0_1px_4px_rgba(0,0,0,0.01)] shrink-0">
        ...
        <span className="font-medium">
          <strong>Поверочный калькулятор:</strong> Расчёты основаны на СНиП
          2.02.01-83* и NCM РМ. Строительно-конструкторский проект должен
          выполняться лицензированным инженером.
        </span>
      </div>
        ...
        >
          ...
          <div className="bg-white/95 backdrop-blur-md border border-slate-200/60 rounded-[24px] p-5 shadow-sm shadow-slate-200/40 relative overflow-hidden">
            ...
            <h2 className="text-sm font-display font-bold text-slate-800 mb-2.5 flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-blue-500" /> Готовые
              конфигурации с мансардой
            </h2>
            <p className="text-[11px] text-slate-500 mb-3.5 leading-normal">
              Выберите шаблоны для мгновенной загрузки параметров дома в
              интерактивный CAD-подборщик:
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 text-xs">
              {PRESETS.map((preset, index) => {
                const isSelected =
                  ...
                  (preset.hasBasement === undefined ||
                    hasBasement === preset.hasBasement);
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (preset.glazingType)
                        setGlazingType(preset.glazingType);
                      if (preset.facadeTech) setFacadeTech(preset.facadeTech);
                  ...
                  >
                    <div>
                      <span
                        className={`font-bold block truncate leading-tight ${isSelected ? "text-blue-700" : "text-slate-700"}`}
                      >
                        {preset.name}
                      </span>
          ...
          <div className="bg-white/95 backdrop-blur-md border border-slate-200/60 rounded-[24px] p-6 shadow-sm shadow-slate-200/40">
            <h2 className="text-sm font-display font-bold text-slate-800 mb-4 flex items-center gap-2">
              <MapPin className="w-4 h-4 text-blue-500" /> География и грунт
              основания
            </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-[11px] font-semibold text-slate-500 mb-1.5 flex items-center justify-between">
                  ...
                  <HelpCircle
                    ...
                    title="Задает нормативную глубину заложения и сейсмичность Вранча"
                  />
                </label>
                <div className="relative">
                  ...
                  >
                    <option value={MoldovaRegion.CENTER}>
                      Центр (Кишинёв, Орхей)
                    </option>
                    <option value={MoldovaRegion.NORTH}>
                      Север (Бэлць, Сорока)
                    </option>
                    <option value={MoldovaRegion.SOUTH}>
                      Юг (Кагул, Комрат)
                    </option>
                  </select>
                ...
                <div className="mt-2 flex flex-wrap gap-x-2.5 text-[10px] text-slate-500 font-medium">
                  <span className="flex items-center gap-1">
                    ...
                    <strong className="text-slate-700">
                      {REGION_DATA[region].frostDepth}м
                    </strong>
                  </span>
                  ...
                  <span className="flex items-center gap-1">
                    ⚡ Сейсмика:{" "}
                    <strong className="text-slate-700">
                      {REGION_DATA[region].seismicPoints}б
                    </strong>
                  </span>
                </div>
              </div>

              {/* Geology Analysis Block */}
              <div className="bg-slate-50 border border-slate-200 p-3 rounded-2xl mb-4">
                <label className="flex items-start gap-2 cursor-pointer mb-3">
                  <div className="relative flex items-center justify-center mt-0.5">
                    <input 
                      type="checkbox" 
                      className="peer appearance-none w-4 h-4 border-2 border-slate-300 rounded focus:ring-blue-500 checked:bg-blue-600 checked:border-blue-600 outline-none transition-all" 
                      checked={hasGeologyData} 
                      onChange={(e) => setHasGeologyData(e.target.checked)}
                    />
                    <CheckCircle2 className="w-3 h-3 text-white absolute opacity-0 peer-checked:opacity-100 pointer-events-none" />
                  </div>
                  <div className="text-xs text-slate-700 font-semibold flex-1">
                    Есть данные геологических изысканий
                    <p className="text-[10px] text-slate-500 font-medium">Без этих данных окончательный выбор фундамента будет заблокирован (только оценка)</p>
                  </div>
                </label>
                  ...
                  <div className="space-y-3 pt-3 border-t border-slate-200">
                    <div>
                      <label className="block text-[10px] font-semibold text-slate-500 mb-1">
                        Выявленный преобладающий грунт
                      </label>
                      <div className="relative">
                        <select
                          className="w-full bg-white border border-slate-200 focus:border-blue-500 rounded-lg py-1.5 px-2 text-xs text-slate-800 outline-none"
                          value={soilType}
                          onChange={(e) => setSoilType(e.target.value as SoilType)}
                        >
                          <option value={SoilType.LOAM}>Суглинок</option>
                          <option value={SoilType.CLAY}>Глина пластичная</option>
                          <option value={SoilType.SAND}>Песок средний</option>
                          <option value={SoilType.LOESS}>Лёсс просадочный ⚠️</option>
                        </select>
                      </div>
                    </div>
                    <div>
              ...
              {/* Soil Type Selector Fallback (when no geology data) */}
              {!hasGeologyData && (
                <div>
                  <label className="block text-[11px] font-semibold text-slate-500 mb-1.5">
                    Предполагаемый грунт (оценка)
                  </label>
                  <div className="relative">
                    <select
                      className="w-full bg-slate-55/80 border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-100 rounded-xl py-2 px-3 text-xs text-slate-800 outline-none transition-all cursor-pointer font-medium appearance-none"
                      ...
                      onChange={(e) => setSoilType(e.target.value as SoilType)}
                    >
                      <option value={SoilType.LOAM}>Суглинок (~200 кПа)</option>
                      <option value={SoilType.CLAY}>Глина пластичная (~150 кПа)</option>
                      <option value={SoilType.SAND}>Песок средний (~280 кПа)</option>
                      <option value={SoilType.LOESS}>Лёсс просадочный (~110 кПа) ⚠️</option>
                    </select>
                    <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2.5 text-slate-500">
                      <ChevronRight className="w-3.5 h-3.5 rotate-90" />
                    </div>
                  </div>
                  <div className="mt-2 text-[10px] text-amber-600 bg-amber-50 p-2 rounded-lg flex items-start gap-1.5 border border-amber-200/50">
                    <AlertOctagon className="w-3 h-3 shrink-0 mt-0.5" />
                    <p>Проектирование без геологии увеличивает коэффициент запаса.</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="mt-5 pt-4 border-t border-slate-100">
              <div className="flex justify-between items-center mb-1.5">
                <span className="text-[11px] font-semibold text-slate-500 flex items-center gap-1.5">
                  <Droplet className="w-3.5 h-3.5 text-blue-500" /> Уровень
                  грунтовых вод (УГВ)
                </span>
                <span
                  className={`text-xs font-bold px-2 py-0.5 rounded-full ${groundwaterDepth < 1.2 ? "bg-red-50 text-red-600 border border-red-100" : "bg-emerald-50 text-emerald-700 border border-emerald-100"}`}
                >
                  {groundwaterDepth.toFixed(1)} м —{" "}
                  {groundwaterDepth < 1.2 ? "Высокий" : "Безопасный"}
                </span>
              </div>
              <input
                type="range"
                ...
                onChange={(e) =>
                  setGroundwaterDepth(parseFloat(e.target.value))
                }
              />
              <p className="text-[10px] text-slate-450 mt-1.5 leading-normal">
                При УГВ выше подошвы (меньше 1.2м) рекомендуется монолитная
                плита под весь дом с гидробетоном W6-W8.
              </p>
            </div>
            ...
            <div className="mt-5 pt-4 border-t border-slate-100">
              <div className="flex justify-between items-center mb-1.5">
                <span className="text-[11px] font-semibold text-slate-500 flex items-center gap-1.5">
                  <TrendingUp className="w-3.5 h-3.5 text-amber-500" /> Уклон
                  земельного участка (сложность рельефа)
                </span>
                <span
                  className={`text-xs font-bold px-2 py-0.5 rounded-full ${landSlope > 8 ? "bg-red-50 text-red-600 border border-red-100" : landSlope > 3 ? "bg-amber-50 text-amber-700 border border-amber-100" : "bg-slate-50 text-slate-700 border border-slate-150"}`}
                >
                  {landSlope}% ({Math.round(landSlope * 0.57)}° —{" "}
                  {landSlope > 8
                    ? "Крутой уклон"
                    : landSlope > 3
                      ? "Умеренный"
              ...
              />
              <p className="text-[10px] text-slate-450 mt-1.5 leading-normal">
                Уклон увеличивает объемы земляных работ, требует ступенчатой
                спец-опалубки для ленты или толстой выравнивающей гравийной
                подушки под плиту.
              </p>
            </div>
          </div>
          ...
          {buildingType === "GARAGE" && (
            <div className="mb-4 bg-amber-50 rounded-[20px] p-5 border border-amber-200/60 shadow-sm">
              <h3 className="text-sm font-bold text-amber-800 mb-2 flex items-center gap-2">
                <ShieldCheck className="w-4 h-4 text-amber-600" /> Юридическая
                справка (Законодательство РМ)
              </h3>
              <p className="text-xs text-amber-700/80 leading-relaxed mb-2">
                Согласно <b>Закону №163/2010</b> об авторизации строительных
                работ в Молдове, капитальный гараж (с монолитным фундаментом или
                смотровой ямой) признается <b>капитальным строением</b>. Для его
                возведения требуется:
              </p>
              <ul className="list-disc pl-5 text-xs text-amber-700/80 mb-2 space-y-1">
                <li>
                  Получить Градостроительный сертификат (Certificat de
                  urbanism).
                </li>
                <li>
                  Разработать проект архитектуры (АР) и конструктива (КЖ).
                </li>
                <li>
                  Получить Разрешение на строительство (Autorizație de
                  construire).
                </li>
              </ul>
              <p className="text-[11px] text-amber-600/70 mt-3 border-t border-amber-200/50 pt-2 font-medium">
                Если гараж строится как временное легковозводимое сооружение
                (металлический/панельный без жесткой связи с землей), процедуру
                можно упростить, однако вариант монолитного фундамента переводит
                его в разряд капитальных зданий.
              </p>
            </div>
          )}
          ...
          <div className="bg-white/95 backdrop-blur-md border border-slate-200/60 rounded-[24px] p-6 shadow-sm shadow-slate-200/40">
            <h2 className="text-sm font-display font-bold text-slate-800 mb-4 flex items-center gap-2">
              <Building2 className="w-4 h-4 text-blue-500" /> Архитектура и
              габариты строения
            </h2>
            ...
            <div className="mb-5 pb-5 border-b border-slate-100/80">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  onClick={() => setBuildingType("GARAGE")}
                >
                  🚗 Гараж / Хозблок
                </button>
              </div>
              ...
              {buildingType === "GARAGE" && (
                <div className="mt-3">
                  <label className="flex items-center gap-2 p-2 bg-amber-50/50 rounded-lg cursor-pointer hover:bg-amber-50 transition-colors border border-amber-100/50">
                    <input
                      type="checkbox"
                      checked={hasPit}
                      onChange={(e) => setHasPit(e.target.checked)}
                      className="w-3.5 h-3.5 rounded border-amber-300 text-amber-600 focus:ring-amber-500"
                    />
                    <span className="text-xs font-medium text-slate-700">
                      Наличие смотровой ямы / погреба
                    </span>
                  </label>
                </div>
              )}
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <option value={1.5}>
                      1.5 этажа (Классическая мансарда)
                    </option>
                    <option value={1.6}>
                      1.5 этажа (Полумансарда, аттик 1.2м)
                    </option>
                    <option value={1.7}>
                      1.5 этажа (Тяжелая ж/б мансарда)
                    </option>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-4 pt-4 border-t border-slate-100">
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    value={wallMaterial}
                    onChange={(e) =>
                      setWallMaterial(e.target.value as BuildingWallMaterial)
                    }
                  >
                    <option value={BuildingWallMaterial.GASOBETON}>
                      Газобетон
                    </option>
                    <option value={BuildingWallMaterial.KOTELET}>
                      Котелец 🇲🇩
                    </option>
                    <option value={BuildingWallMaterial.BRICK}>Кирпич</option>
                    <option value={BuildingWallMaterial.KERAMZIT}>
                      Керамзитоб.
                    </option>
                    <option value={BuildingWallMaterial.FRAME}>
                      Каркас SIP
                    </option>
                  </select>
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    value={slabMaterial}
                    onChange={(e) =>
                      setSlabMaterial(e.target.value as SlabMaterial)
                    }
                  >
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  >
                    <option value={RoofType.GABLE_METAL}>
                      Двускатн. металл
                    </option>
                    <option value={RoofType.GABLE_SHINGLE}>
                      Двускатн. битумн. (мягкая)
                    </option>
                    <option value={RoofType.HIP_METAL}>Вальмовая металл</option>
                    <option value={RoofType.HIP_CERAMIC}>
                      Вальмовая керам.
                    </option>
                    <option value={RoofType.FLAT_PVC}>Плоская мембрана</option>
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    value={glazingType}
                    onChange={(e) =>
                      setGlazingType(e.target.value as GlazingType)
                    }
                  >
                    <option value={GlazingType.STANDARD}>Стандартное</option>
                    <option value={GlazingType.PANORAMIC}>
                      Панорамное (Тяжелое)
                    </option>
                  </select>
                  <span className="absolute right-2 top-2 pointer-events-none text-slate-400">
              ...
              <div>
                ...
                <div className="relative">
                  <select
                    ...
                    value={facadeTech}
                    onChange={(e) =>
                      setFacadeTech(e.target.value as FacadeType)
                    }
                  >
                    <option value={FacadeType.WET}>
                      Мокрый фасад (Штукатурка)
                    </option>
                    <option value={FacadeType.VENTILATED}>
                      Вентилируемый фасад
                    </option>
                    <option value={FacadeType.FACE_BRICK}>
                      Облицовочный кирпич
                    </option>
                  </select>
                  <span className="absolute right-2 top-2 pointer-events-none text-slate-400">
            ...
            <div className="mt-5 pt-4 border-t border-slate-100 space-y-3.5">
              <label className="flex items-start gap-3 cursor-pointer group select-none">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[10px] text-slate-500 leading-normal">
                    Добавляет +1 этаж к полезной вертикальной нагрузке для
                    резервирования несущей способности.
                  </span>
                </div>
              </label>
              ...
              <label className="flex items-start gap-3 cursor-pointer group select-none">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[10px] text-slate-500 leading-normal">
                    Удваивает гидроизоляционный барьер, закладывает усиленный
                    дренаж и земляной котлован.
                  </span>
                </div>
              </label>
            </div>
            ...
            <div className="mt-5 pt-4 border-t border-slate-100 space-y-3.5">
              <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                <Sliders className="w-3.5 h-3.5 text-slate-400" /> Сметный
                расчёт и налоги
              </h4>

              <div className="bg-slate-50 rounded-xl p-3 border border-slate-200">
                <span className="text-[11px] font-semibold text-slate-700 mb-2 block">Класс комплектации и отделки (Смета)</span>
                <div className="grid grid-cols-3 gap-1 bg-slate-200/50 p-1 rounded-lg">
                  <button
                    type="button"
                    onClick={() => setBuildQuality("ECONOMY")}
                    className={`py-1.5 px-1 rounded-md text-[10px] font-medium transition-all ${buildQuality === "ECONOMY" ? "bg-white text-emerald-800 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
                  >
                    Эконом (Минимум)
                  </button>
                  <button
                    type="button"
                    onClick={() => setBuildQuality("STANDARD")}
                    className={`py-1.5 px-1 rounded-md text-[10px] font-medium transition-all ${buildQuality === "STANDARD" ? "bg-white text-blue-800 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
                  >
                    Стандарт (Комфорт)
                  </button>
                  <button
                    type="button"
                    onClick={() => setBuildQuality("PREMIUM")}
                    className={`py-1.5 px-1 rounded-md text-[10px] font-medium transition-all ${buildQuality === "PREMIUM" ? "bg-white text-indigo-800 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
                  >
                    Премиум (Люкс)
                  </button>
                </div>
                <div className="mt-2 text-[9px] text-slate-500 leading-tight min-h-[28px]">
                  {buildQuality === "ECONOMY" && <span className="text-emerald-700">Без капли лишнего. Прохождение СНиП (убираем резервы, НДС, декор).</span>}
                  {buildQuality === "STANDARD" && <span className="text-blue-700">Сбалансированно. Стандартные материалы, хороший фасад, утепленная отмостка.</span>}
                  {buildQuality === "PREMIUM" && <span className="text-indigo-700">Люкс комплектация. Клинкерный фасад, 7-камерные окна, высокий резерв.</span>}
                </div>
              </div>

              <label className={`flex items-center justify-between cursor-pointer group select-none ${buildQuality === "ECONOMY" ? 'opacity-50 pointer-events-none' : ''}`}>
                <div className="flex flex-col pr-2">
                  ...
                  <span className="text-[9px] text-slate-400">
                    Включить НДС на готовый бетон и работу
                  </span>
                </div>
                <input
                  type="checkbox"
                  disabled={buildQuality === "ECONOMY"}
              ...
              <label className="flex items-center justify-between cursor-pointer group select-none">
                <div className="flex flex-col pr-2">
                  ...
                  <span className="text-[9px] text-slate-400">
                    АР/КЖ проект + 2 гео-скважины по 6м
                  </span>
                </div>
                <input
                  type="checkbox"
              ...
              <label className="flex items-center justify-between cursor-pointer group select-none">
                <div className="flex flex-col pr-2">
                  ...
                  <span className="text-[9px] text-slate-400">
                    Авторский + Технический надзор РМ
                  </span>
                </div>
                <input
                  type="checkbox"
              ...
              <div className="pt-1.5">
                <div className="flex justify-between items-center mb-1">
                  <span className="text-[11px] font-semibold text-slate-500">
                    Коэффициент проектного резерва
                  </span>
                ...
                <input
                  ...
                  step="1"
                  disabled={buildQuality === "ECONOMY"}
                  className={`w-full cursor-pointer h-1 rounded-full appearance-none ${buildQuality === "ECONOMY" ? 'bg-slate-100 accent-slate-300' : 'bg-slate-200 accent-indigo-600'}`}
                  value={projectReservePercent}
                  onChange={(e) =>
                    setProjectReservePercent(parseInt(e.target.value))
                  }
                />
          ...
          <div className="bg-white/95 backdrop-blur-md border border-slate-200/60 rounded-[24px] p-5 shadow-sm shadow-slate-200/40 relative flex flex-col justify-between">
            <h2 className="text-sm font-display font-bold text-slate-800 mb-3.5 flex items-center gap-2">
              <Compass className="w-4 h-4 text-blue-500" /> Инженерный чертёж
              САПР (Разрез)
            </h2>
            ...
            <div className="h-[400px] rounded-2xl bg-slate-50/50 border border-slate-200 relative overflow-hidden shadow-inner z-0">
              ...
              <div className="absolute inset-x-0 top-0 h-[215px] flex items-end justify-center pointer-events-none">
                ...
                <div
                  ...
                  style={{
                    height:
                      floors === 1
                        ? "36px"
                        : floors === 1.5 || floors === 1.6 || floors === 1.7
                          ? "60px"
                          : floors === 2
                            ? "66px"
                            : floors === 2.5
                ...
                >
                  ...
                  {(roofType === RoofType.GABLE_METAL ||
                    roofType === RoofType.GABLE_SHINGLE) && (
                    <div
                      className="absolute left-1/2 -translate-x-1/2 w-[114%] h-[36px] transition-all duration-300 pointer-events-none z-30"
                      style={{ bottom: "100%" }}
                    >
                      {hasMansard ? (
                        <svg
                        ...
                        >
                          ...
                          <polygon
                            points="0,36 32,10 100,2 168,10 200,36"
                            fill={
                              roofType === RoofType.GABLE_SHINGLE
                                ? "#4b5563"
                                : "#1e293b"
                  ...
                  {roofType === RoofType.FLAT_PVC && (
                    <div
                      className="absolute left-1/2 -translate-x-1/2 w-[104%] h-[10px] transition-all duration-300 pointer-events-none z-30"
                      style={{ bottom: "100%" }}
                    >
                      <svg
                        className="w-full h-full overflow-visible"
                  ...
                  {roofType === RoofType.SHED_BOARD && (
                    <div
                      className="absolute left-1/2 -translate-x-1/2 w-[114%] h-[24px] transition-all duration-300 pointer-events-none z-30"
                      style={{ bottom: "100%" }}
                    >
                      <svg
                        className="w-full h-full overflow-visible"
                      ...
                      >
                        <polygon
                          points="0,4 200,22 200,24 0,6"
                          fill="#475569"
                          stroke="#1e293b"
                          strokeWidth="2"
                        />
                        <polygon
                          points="0,6 200,24 200,24 0,24"
                          fill="#cbd5e1"
                          opacity="0.3"
                        />
                        <line
                  ...
                  >
                    ...
                    {(floors === 1.5 ||
                      ...
                      >
                        ...
                        <span className="absolute bottom-0 left-1/2 -translate-x-1/2 text-[4.5px] font-bold text-slate-800/60 select-none tracking-wider bg-white/35 px-1 rounded-t-xs">
                          МАНСАРДА
                        </span>
                      </div>
                    )}
                    ...
                    {floors >= 3 && (
                      <div className="w-full h-[30px] border-b border-slate-300/40 relative flex items-center justify-between px-3.5 transition-all duration-300 backdrop-blur-[0.3px]">
                        ...
                        <span className="absolute bottom-0 right-1 text-[5px] font-mono font-black text-slate-800/40 select-none bg-white/20 px-0.5 rounded-sm">
                          3 ЭТ
                        </span>
                      </div>
                    )}
              ...
              <div className="w-full relative space-y-[1.5px] text-[10px] text-slate-550 z-10 mt-[215px]">
                ...
                {["slab", "classic_slab", "ribbed_slab"].includes(
                  activeFndId,
                ) && (
                  <div
                ...
                {["strip", "mzlf", "strip_with_slab"].includes(activeFndId) && (
                  <>
                    ...
                    <div
                      ...
                      style={{
                        top: `${getSoilDepthPx(selectedOption.depthM) / 2 - 6}px`,
                      }}
                    >
                ...
                {["piles", "drilled_piles", "tise", "column_footing"].includes(
                  activeFndId,
                ) && (
                  <>
                ...
                <div
                  ...
                  style={{
                    top: `${getSoilDepthPx(REGION_DATA[region].frostDepth)}px`,
                  }}
                >
                  ...
                  style={{
                    backgroundColor:
                      soilType === SoilType.LOESS
                        ? "rgba(224, 130, 60, 0.05)"
                        : "rgba(59, 130, 246, 0.02)",
                  }}
                >
                  <span className="font-bold text-slate-700">
                    Несущий слой: {SOIL_DATA[soilType].name}
                  </span>
                  <span className="text-[9px] font-bold text-neutral-500 bg-neutral-200/50 px-1.5 py-0.5 rounded font-mono">
                    R₀: {SOIL_DATA[soilType].resistanceKPa} кПа
                  </span>
                </div>
                ...
                >
                  <div className="flex items-center gap-1.5 text-blue-700 bg-white border border-blue-200 rounded-full px-2.5 py-0.5 font-bold shadow-sm text-[9px]">
                    ...
                    УГВ: {groundwaterDepth.toFixed(1)} м{" "}
                    {groundwaterDepth < 1.3 ? "⚠️ Высоко" : ""}
                  </div>
                </div>
                ...
                <div className="h-14 bg-slate-100 border-t border-slate-200 flex justify-between items-center px-3 rounded-b-xl">
                  <span className="text-slate-500 text-[10px] font-medium">
                    Коренные плотные грунты республики
                  </span>
                  <span className="text-[9px] font-mono text-slate-450">
                    γ_reliability={safetyFactor}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="mt-3.5 bg-slate-50 border border-slate-200/70 rounded-2xl p-3.5 text-[11px] text-slate-600 leading-relaxed shadow-inner">
              <span className="text-xs font-bold text-slate-800 block mb-1">
                Сводка инженерных сил:
              </span>
              <p>
                Горизонтальные габариты дома {width}х{length} метров составляют
                суммарный вес весовых конструкций с учетом мебели и
                коэффициентов строительного заложения РМ:{" "}
                <strong className="text-blue-600 font-bold">
                  {results.totalFactoredWeightTons} тонн
                </strong>
                . Рекомендованная площадь подошвы фундамента:{" "}
                <strong className="text-emerald-700 font-bold">
                  {results.bearingAreaRequiredM2} м²
                </strong>
                .
              </p>
            </div>
        ...
        <section className="xl:col-span-7 flex flex-col gap-6">
          ...
          {validationErrors.length > 0 && (
            <div className="bg-red-50 border-2 border-red-200/80 rounded-[24px] p-5 shadow-sm relative overflow-hidden animate-fade-in">
              ...
              <div className="flex gap-3">
                ...
                <div>
                  <h3 className="text-xs font-display font-black text-red-800 uppercase tracking-tight">
                    ⛔ ОШИБКА КОНТРОЛЯ ВХОДНЫХ ДАННЫХ
                  </h3>
                  <p className="text-[10px] text-red-600 font-semibold mt-1">
                    Обнаружены критические выходы параметров за конструкционные
                    лимиты СНиП / NCM РМ. Пожалуйста, исправьте следующие
                    значения:
                  </p>
          ...
          <div className="bg-white/95 backdrop-blur-md border border-slate-200/60 rounded-[24px] p-5 shadow-sm shadow-slate-200/40 relative overflow-hidden">
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1.5 text-[11px] bg-slate-50/50 p-2.5 rounded-xl border border-slate-150">
                  ...
                  <div className="flex justify-between items-center py-1 border-b border-dashed border-slate-200/60">
                    ...
                    <span
                      className="font-bold text-slate-800 text-right truncate max-w-[130px]"
                      title={WALL_MATERIAL_DATA[wallMaterial].name}
                    >
                  ...
                  <div className="flex justify-between items-center py-1 border-b border-dashed border-slate-200/60">
                    ...
                    <span
                      className="font-semibold text-slate-700 text-right truncate max-w-[130px]"
                      title={SLAB_DATA[slabMaterial].name}
                    >
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1.5 text-[11px] bg-slate-50/50 p-2.5 rounded-xl border border-slate-150">
                  ...
                  <div className="flex justify-between items-center py-1 border-b border-dashed border-slate-200/60">
                    ...
                    <span className="font-extrabold text-slate-800">
                      {SOIL_DATA[soilType].name.split("(")[0]}
                    </span>
                  </div>
                  <div className="flex justify-between items-center py-1 border-b border-dashed border-slate-200/60">
                    <span className="text-slate-500 flex items-center gap-1">
                  ...
                  <div className="flex justify-between items-center py-1 border-b border-dashed border-slate-200/60">
                    ...
                    <span className="font-bold text-red-600 font-mono">
                      {REGION_DATA[region].seismicClass} баллов MSK
                    </span>
                  </div>
                  <div className="flex justify-between items-center py-1">
                    <span className="text-slate-500 flex items-center gap-1">
            ...
            <div className="mt-3.5 p-3 rounded-2xl bg-amber-50/50 border border-amber-100 leading-normal text-[10.5px] text-amber-900 flex gap-2">
              ...
              <p>
                <strong>Опорная калькуляция:</strong> Расчет автоматически
                учитывает повышенный риск пучения просадочных суглинков РМ и
                предписывает установку сейсмопоясов, дренажного отвода{" "}
                {activeFndId === "slab"
                  ? "и заложение мелкозаглубленной утепленной шведской плиты (УШП) по специальному теплотехническому расчету, предотвращающему промерзание грунта под плитой"
                  : "и заложение ниже СНиП-уровня промерзания для долговечной службы"}
                .
              </p>
            </div>
          </div>
          ...
          <div className="bg-white/95 backdrop-blur-md border border-slate-200/60 rounded-[24px] p-6 shadow-sm shadow-slate-200/40 relative overflow-hidden">
            {validationErrors.length > 0 && (
              <div className="mb-6 p-4.5 bg-rose-50 border border-rose-200 rounded-2xl animate-fade-in shadow-xs">
                <div className="flex items-center gap-2 mb-2 text-rose-800 font-extrabold text-xs uppercase tracking-wider">
                  <span className="text-sm">⚠️</span> Обнаружены нарушения
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 py-4 text-xs font-medium">
              <div className="bg-slate-50/80 border border-slate-200/40 rounded-2xl p-3">
                ...
                <span className="text-sm font-extrabold text-slate-800">
                  {results.wallWeightTons} т
                </span>
                <div className="w-full bg-slate-200/70 h-1 rounded-full mt-2 overflow-hidden">
                  <div
                    className="bg-blue-500 h-full rounded-full transition-all"
                    style={{
                      width: `${Math.min(100, (results.wallWeightTons / results.totalFactoredWeightTons) * 100)}%`,
                    }}
                  ></div>
                </div>
              </div>
              ...
              <div className="bg-slate-50/80 border border-slate-200/40 rounded-2xl p-3">
                ...
                <span className="text-sm font-extrabold text-slate-800">
                  {results.slabWeightTons} т
                </span>
                <div className="w-full bg-slate-200/70 h-1 rounded-full mt-2 overflow-hidden">
                  <div
                    className="bg-indigo-400 h-full rounded-full transition-all"
                    style={{
                      width: `${Math.min(100, (results.slabWeightTons / results.totalFactoredWeightTons) * 100)}%`,
                    }}
                  ></div>
                </div>
              </div>
              ...
              <div className="bg-slate-50/80 border border-slate-200/40 rounded-2xl p-3">
                ...
                <span className="text-sm font-extrabold text-slate-800">
                  {(results.snowLoadTons + results.windLoadTons).toFixed(1)} т
                </span>
                <div className="w-full bg-slate-200/70 h-1 rounded-full mt-2 overflow-hidden">
                  <div
                    className="bg-sky-400 h-full rounded-full transition-all"
                    style={{
                      width: `${Math.min(100, ((results.snowLoadTons + results.windLoadTons) / results.totalFactoredWeightTons) * 100)}%`,
                    }}
                  ></div>
                </div>
              </div>
              ...
              <div className="bg-slate-50/80 border border-slate-200/40 rounded-2xl p-3">
                ...
                <span className="text-sm font-extrabold text-slate-800">
                  {results.seismicForceTons} т
                </span>
                <div className="w-full bg-slate-200/70 h-1 rounded-full mt-2 overflow-hidden">
                  <div
                    className="bg-rose-400 h-full rounded-full transition-all"
                    style={{
                      width: `${Math.min(100, (results.seismicForceTons / results.totalFactoredWeightTons) * 100)}%`,
                    }}
                  ></div>
                </div>
              </div>
              ...
              <table className="w-full text-left text-xs border-collapse min-w-[580px]">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200/60 text-slate-500 font-bold uppercase tracking-wider text-[10px]">
                    ...
                    <th className="py-2.5 px-3 text-right rounded-r-xl text-blue-600 font-extrabold">
                      С полом (MDL)
                    </th>
                  </tr>
                </thead>
                  ...
                  {results.options.map((opt) => (
                    ...
                    >
                      ...
                      <td className="py-3 px-3 text-right font-mono font-bold text-xs">
                        <span
                          className={`px-2 py-0.5 rounded-full text-[11px] ${
                            opt.reliabilityScore >= 90
                              ? "text-emerald-700 bg-emerald-50 border border-emerald-100"
                              : "text-amber-700 bg-amber-50 border border-amber-100"
                          }`}
                        >
                          {opt.reliabilityScore}%
                        </span>
                      </td>
                      ...
                      <td className="py-3 px-3 max-w-[170px]">
                        <p className="truncate text-slate-600 text-[11px] font-sans font-medium">
                          {opt.pros[0]}
                        </p>
            ...
            <div className="mt-5 p-4.5 bg-slate-50 border border-slate-200/80 rounded-2xl">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-3.5 pb-3.5 border-b border-slate-200/60">
                <div>
                  ...
                  <p className="text-[10px] text-slate-400 mt-0.5">
                    Требуемые объемы бетона и проектные нормы Молдовы
                  </p>
                </div>
                <div className="text-right">
                  ...
                  {scopeMetrics &&
                  scopeMetrics.activeBudgetCost !== selectedOption.costMDL ? (
                    <div className="flex flex-col items-end">
                      <span className="text-xs text-slate-400 line-through">
                        {selectedOption.costMDL.toLocaleString()} MDL
                      </span>
                      <span className="text-base font-extrabold text-blue-600 font-mono">
                        {scopeMetrics.activeBudgetCost.toLocaleString()} MDL
                      </span>
                      <span className="text-[8.5px] font-bold text-blue-500 bg-blue-50 px-1 py-0.5 rounded-md mt-0.5">
                        С учётом настроек и статусов
                      </span>
                    </div>
                  ) : (
                    <span className="text-base font-extrabold text-blue-600 font-mono">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div className="bg-white border border-slate-200/85 rounded-xl p-2.5 shadow-[0_1px_2px_rgba(0,0,0,0.03)]">
                      ...
                      <strong className="text-slate-800 text-sm">
                        {selectedOption.materials.concreteVolumeM3} м³
                      </strong>
                      <span className="text-[9px] text-slate-450 block mt-0.5">
                        Доставка миксером (RM 2026)
                      </span>
                    </div>

                    {selectedOption.materials.fbsBlocksCount && (
                      <div className="col-span-2 bg-white border border-slate-200/85 rounded-xl p-2.5 shadow-[0_1px_2px_rgba(0,0,0,0.03)]">
                        <span className="text-slate-400 text-[10px] block font-medium">
                          Фундаментные блоки ФБС 24.x.6:
                        </span>
                        <strong className="text-slate-800 text-sm">
                          {selectedOption.materials.fbsBlocksCount} шт
                        </strong>
                        <span className="text-[9px] text-slate-450 block mt-0.5">
                          Сборка на раствор автокраном
                        </span>
                      </div>
                    )}
                    ...
                    <div className="bg-white border border-slate-200/85 rounded-xl p-2.5 shadow-[0_1px_2px_rgba(0,0,0,0.03)]">
                      ...
                      <strong className="text-slate-800 text-sm">
                        {selectedOption.materials.reinforcementBarKg} кг
                      </strong>
                      <span className="text-[9.5px] text-slate-450 block mt-0.5 font-sans">
                        раб. d
                        {selectedOption.materials.rebarLongitudinalDiameter ||
                          12}{" "}
                        ({selectedOption.materials.rebarLongitudinalKg || 0}кг)
                        + хм. d
                        {selectedOption.materials.rebarTransverseDiameter || 8}{" "}
                        ({selectedOption.materials.rebarTransverseKg || 0}кг)
                      </span>
                    </div>

                    <div className="bg-white border border-slate-200/85 rounded-xl p-2.5 shadow-[0_1px_2px_rgba(0,0,0,0.03)]">
                      <span className="text-slate-400 text-[10px] block font-medium">
                        Песок-гравий подушки:
                      </span>
                      <strong className="text-slate-800 text-sm">
                        {selectedOption.materials.sandGravelM3} м³ (
                        {selectedOption.materials.sandGravelWeightTons || 0} т)
                      </strong>
                      <span className="text-[9px] text-slate-450 block mt-0.5">
                        Perne de nisip уплотнённый
                      </span>
                    </div>

                    <div className="bg-white border border-slate-200/85 rounded-xl p-2.5 shadow-[0_1px_2px_rgba(0,0,0,0.03)]">
                      <span className="text-slate-400 text-[10px] block font-medium">
                        Мастика и XPS цоколя:
                      </span>
                      <strong className="text-slate-800 text-sm">
                        {selectedOption.materials.waterproofingM2} м² /{" "}
                        {selectedOption.materials.insulationM3} м³
                      </strong>
                      <span className="text-[9px] text-slate-450 block mt-0.5">
                        Пеноплэкс + влагозищита
                      </span>
                    </div>

                    <div className="bg-white border border-slate-200/85 rounded-xl p-2.5 shadow-[0_1px_2px_rgba(0,0,0,0.03)]">
                      <span className="text-slate-400 text-[10px] block font-medium">
                        Калиброванная доска опалубки:
                      </span>
                      <strong className="text-slate-800 text-sm">
                        {selectedOption.materials.formworkM2 || 0} м²
                      </strong>
                      <span className="text-[10px] text-slate-450 block mt-0.5 leading-normal">
                        Требуется:{" "}
                        <strong className="text-blue-600">
                          {selectedOption.materials.formworkBoardsCount || 0} шт
                        </strong>{" "}
                        досок (25x150x6000)
                      </span>
                    </div>
                    ...
                    <div className="bg-white border border-slate-200/85 rounded-xl p-2.5 shadow-[0_1px_2px_rgba(0,0,0,0.03)]">
                      ...
                      <strong className="text-slate-800 text-sm">
                        {selectedOption.materials.excavationVolumeM3 || 0} м³
                      </strong>
                      <span className="text-[9px] text-slate-450 block mt-0.5">
                        Выемка спецтехникой с подчисткой
                      </span>
                    </div>

                    <div
                      className={`col-span-2 bg-gradient-to-br border rounded-xl p-2.5 shadow-[0_1px_2px_rgba(0,0,0,0.03)] ${selectedOption.id === "slab" ? "from-emerald-50/40 to-teal-50/20 border-emerald-200/60" : "from-slate-50/40 to-slate-100/10 border-slate-200/80"}`}
                    >
                      <span className="text-slate-500 font-bold text-[10px] uppercase block mb-1">
                        🏗️ Черновой пол по грунту (систематический учёт)
                      </span>
                      {selectedOption.id === "slab" ? (
                        <div className="text-[11px] text-slate-600 leading-normal">
                          <strong className="text-emerald-700 font-bold block mb-0.5">
                            Встроен в плиту (0 MDL экстра)
                          </strong>
                          Монолитная плита сама выступает жестким основанием
                          пола 1-го этажа. Не требует дополнительных трат на
                          стяжки, засыпку и обратную трамбовку подушки пола.
                        </div>
                      ) : (
                        <div className="space-y-1">
                          <div className="flex justify-between items-baseline">
                            <strong className="text-slate-800 text-[11px] font-semibold">
                              Доп. материалы пола по грунту внутри цоколя:
                            </strong>
                            <span className="text-orange-600 font-mono font-bold text-xs">
                              +
                              {Math.round(
                                selectedOption.costEstimate.roughFloorCostMDL ||
                                  0,
                              ).toLocaleString()}{" "}
                              MDL
                            </span>
                          ...
                          <p className="text-[10px] text-slate-500 leading-normal">
                            Подушка:{" "}
                            <strong className="text-slate-700">
                              {selectedOption.materials.roughFloorSandM3} м³
                            </strong>
                            , Бетон стяжки:{" "}
                            <strong className="text-slate-700">
                              {selectedOption.materials.roughFloorConcreteM3} м³
                            </strong>
                            , Сетка d8:{" "}
                            <strong className="text-slate-700">
                              {selectedOption.materials.roughFloorRebarKg} кг
                            </strong>{" "}
                            на площадь{" "}
                            <strong className="text-slate-700">
                              {selectedOption.materials.roughFloorAreaM2} м²
                            </strong>
                            .
                          </p>
                        </div>
                    ...
                    <div className="mt-3 bg-blue-50/70 border border-blue-200/50 p-3 rounded-2xl">
                      <div className="flex items-center gap-1.5 mb-1 text-blue-800">
                        ...
                        <span className="text-[11px] font-bold">
                          Пристенный дренаж комплекса (УГВ {groundwaterDepth}м):
                        </span>
                      </div>
                      <p className="text-[10px] text-blue-700 leading-normal mb-2">
                        Защищает основание из лессового суглинка от размывания и
                        морозного пучения.
                      </p>
                      <div className="grid grid-cols-2 gap-x-2 gap-y-1 text-[10.5px] text-blue-900 leading-tight">
                        <div>
                          • Дренажная труба d110:{" "}
                          <strong>
                            {selectedOption.materials.drainagePipeM} м
                          </strong>
                        </div>
                        <div>
                          • Геотекстиль Typar SF40:{" "}
                          <strong>
                            {selectedOption.materials.drainageGeotextileM2} м²
                          </strong>
                        </div>
                        <div>
                          • Гранитный щебень 20-40:{" "}
                          <strong>
                            {selectedOption.materials.drainageStoneM3} м³
                          </strong>
                        </div>
                        <div>
                          • Ревизионные колодцы d315:{" "}
                          <strong>
                            {selectedOption.materials.drainageWellsCount} шт
                          </strong>
                        </div>
                      </div>
                    </div>
                  ...
                  {!selectedOption.materials.hasDrainage &&
                    currentInput.buildQuality === "ECONOMY" &&
                    groundwaterDepth < 1.5 && (
                      <div className="mt-3 bg-amber-50/70 border border-amber-200/50 p-3 rounded-2xl relative">
                        <div className="absolute top-0 right-0 bg-amber-500 text-white text-[8px] font-bold px-2 py-0.5 rounded-bl select-none">
                          ECONOMY
                        </div>
                        <div className="flex items-center gap-1.5 mb-1 text-amber-800">
                          <Droplet className="w-3.5 h-3.5" />
                          <span className="text-[11px] font-bold">
                            Дренаж комплекса исключен
                          </span>
                        </div>
                        <p className="text-[10px] text-amber-700 leading-normal mb-2">
                          Режим экономии отключил систему кольцевого дренажа
                          (экономия ~{" "}
                          {Math.round(
                            2 * (width + length) * 650,
                          ).toLocaleString()}{" "}
                          MDL). При УГВ {groundwaterDepth}м это допустимо для
                          сдачи в Кадастр, однако несет риски намокания
                          фундамента.
                        </p>
                      </div>
                    )}
                ...
                <div className="flex flex-col justify-between">
                  <div>
                    <span className="text-[10px] font-bold text-slate-550 uppercase tracking-widest block mb-2">
                      🏗️ Геотехнические параметры
                    </span>
                    <ul className="text-[11px] text-slate-600 space-y-2 list-none pl-1">
                      <li className="flex items-center gap-1.5">
                        ...
                        Ширина подошвы лент/опор:{" "}
                        <strong className="text-slate-800 ml-auto">
                          {selectedOption.widthM} м
                        </strong>
                      </li>
                      <li className="flex items-center gap-1.5">
                        <ChevronRight className="w-3.5 h-3.5 text-blue-500 shrink-0" />{" "}
                        Глубина заглубления d_fund:{" "}
                        <strong className="text-slate-800 ml-auto">
                          {selectedOption.depthM} м
                        </strong>
                      </li>
                      {REGION_DATA[region].beltMandatory && (
                        <li className="text-amber-700 bg-amber-50 px-2.5 py-1.5 rounded-xl border border-amber-200/50 mt-1 flex items-start gap-1.5">
                          ...
                          <span>
                            <strong>Обязательно:</strong> Сейсмопояс (Centură
                            antiseismică h=20-30 см)
                          </span>
                        </li>
                      )}
                    </ul>
                    ...
                    {selectedOption.id === "piles" && (
                      <div className="mt-2.5 bg-rose-50 border border-rose-200/80 rounded-xl p-3 text-[10.5px] text-rose-950 leading-relaxed space-y-1.5 shadow-xs">
                        ...
                        <p>
                          Принятые объемы бетона и арматуры резко снижены (
                          <strong>на -21% и -55%</strong>) при неизменности веса
                          здания. Это допустимо лишь при детальном расчете!
                        </p>
                        <div className="pl-3.5 space-y-1 list-disc text-[10px] text-rose-900 font-medium">
                          <div>
                            • <strong>Риск при сейсмике 7 баллов:</strong>{" "}
                            Требуется пересчет несущей способности каждой сваи
                            по боковой поверхности (трение) и по пяте (опорному
                            давлению).
                          </div>
                          <div>
                            • <strong>Рекомендация:</strong> Согласовать паспорт
                            ИГИ (инженерной геологии) и проверить проектную
                            прочность свай до начала забивки/заливки.
                          </div>
                        </div>
                      </div>
                    )}
                  ...
                  <div className="mt-3.5 text-[9.5px] text-slate-450 font-medium leading-normal bg-white/70 px-2.5 py-2 rounded-xl border border-slate-200/50 text-center">
                    💡 Розничный прайс-лист республики на май 2026: Бетон C20/25
                    от 1750 MDL/м³, рифлёная сталь А500С от 21 MDL/кг.
                  </div>
                </div>
              </div>
            </div>
            ...
            {selectedOption.justification && (
              <div className="mt-5 p-5 bg-gradient-to-br from-indigo-50/50 to-blue-50/20 border-2 border-indigo-100 rounded-3xl shadow-sm">
                <div className="flex items-center gap-2 mb-4 pb-3 border-b border-indigo-200/50">
                  ...
                  <div>
                    <h3 className="text-sm font-extrabold text-indigo-900 tracking-tight">
                      Инженерное обоснование выбора
                    </h3>
                    <p className="text-[10px] uppercase tracking-widest text-indigo-500 font-bold mt-0.5">
                      Eurocode & NCM Compliance
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    <div>
                      <h4 className="text-xs font-bold text-slate-700 mb-2 uppercase tracking-wide">Почему рекомендовано:</h4>
                      <ul className="text-xs text-slate-600 space-y-1.5 list-none">
                        {selectedOption.justification.reasons.map((r, i) => (
                          <li key={i} className="flex items-start gap-1.5"><ChevronRight className="w-4 h-4 text-emerald-500 shrink-0" /> {r}</li>
                        ))}
                      </ul>
                    </div>
                    <div>
                      <h4 className="text-xs font-bold text-slate-700 mb-2 uppercase tracking-wide">Инженерные факторы:</h4>
                      <ul className="text-xs text-slate-600 space-y-1.5 list-none">
                        {selectedOption.justification.engineeringComparison.map((r, i) => (
                          <li key={i} className="flex items-start gap-1.5"><Activity className="w-4 h-4 text-blue-500 shrink-0" /> {r}</li>
                        ))}
                      </ul>
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    <div>
                      <h4 className="text-xs font-bold text-slate-700 mb-2 uppercase tracking-wide">Экономика (LCC - Life Cycle Cost):</h4>
                      <div className="bg-white/60 p-3 rounded-xl border border-indigo-100 space-y-2">
                        {selectedOption.financialModel && (
                          <>
                            <div className="flex justify-between items-center text-xs">
                              <span className="text-slate-600 font-medium">CAPEX (Строительство):</span>
                              <strong className="text-slate-800">{selectedOption.capex?.totalMDL.toLocaleString()} MDL</strong>
                            </div>
                            <div className="flex justify-between items-center text-xs">
                              <span className="text-slate-600 font-medium">OPEX (в год):</span>
                              <strong className="text-slate-800">{selectedOption.opex?.totalAnnualMDL.toLocaleString()} MDL</strong>
                            </div>
                            <div className="flex justify-between items-center text-xs">
                              <span className="text-slate-600 font-medium">NPV:</span>
                              <strong className="text-blue-600">{selectedOption.financialModel.NPV_MDL > 0 ? "+" : ""}{selectedOption.financialModel.NPV_MDL.toLocaleString()} MDL</strong>
                            </div>
                            <div className="flex justify-between items-center text-xs">
                              <span className="text-slate-600 font-medium">Окупаемость:</span>
                              <strong className="text-emerald-600">{selectedOption.financialModel.paybackPeriodYears} лет</strong>
                            </div>
                          </>
                        )}
                      </div>
                    ...
                    <div>
                      <h4 className="text-xs font-bold text-slate-700 mb-2 uppercase tracking-wide">Нормативный контроль:</h4>
                      <div className="space-y-2">
                        {selectedOption.justification.normativeChecks.map((check, i) => (
                          <div key={i} className={`flex items-start gap-2 p-2 rounded-lg text-[11px] border ${check.isPass ? "bg-emerald-50/50 border-emerald-100/50 text-emerald-800" : "bg-rose-50/50 border-rose-100/50 text-rose-800"}`}>
                            {check.isPass ? <CheckCircle2 className="w-4 h-4 text-emerald-500 shrink-0" /> : <AlertOctagon className="w-4 h-4 text-rose-500 shrink-0" />}
                            <div>
                              <strong className="block">{check.normativeDocument}</strong>
                              <span className="opacity-80 block">{check.clause}</span>
                              <span className="font-mono mt-0.5 block opacity-90">{check.calculationDetails}</span>
                            </div>
                          </div>
                        ))}
                      </div>
            ...
            <div className="mt-4 pt-4 border-t border-slate-100">
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 mb-4">
                <div>
                  <span className="text-[10px] font-bold text-slate-550 uppercase tracking-widest block">
                    📋 Подробная калькуляция сметных затрат
                  </span>
                ...
                <div className="flex flex-wrap items-center gap-2 shrink-0">
                  <button
                    ...
                    onClick={() => {
                      ...
                      const isAllExpanded = visibleCatIds.every((id) =>
                  ...
                  <button
                    ...
                    disabled={isExporting}
                    className="px-3.5 py-1.5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 disabled:opacity-50 text-white font-bold text-[10px] rounded-xl flex items-center justify-center gap-1.5 shadow-xs transition-colors cursor-pointer select-none disabled:cursor-not-allowed"
                  >
                    {isExporting ? (
                      <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                    ) : (
                    ...
                    {isExporting
                      ? "Формирование..."
                      : "Сравнение (Только Фундаменты)"}
                  </button>
                  <button
                    type="button"
                    id="btn-excel-export-pro"
                    
                    onClick={async () => {
                      
                      try {
                        setIsExporting(true);
                        await exportProExpertSystemToExcel(
                          results.input,
                          results,
                          safetyFactor,
                          selectedOption,
                        ...
                        console.error("Failed to export pro dashboard", err);
                      } finally {
                        setIsExporting(false);
                      }
                    }}
                    disabled={isExporting}
                    className="px-3.5 py-1.5 bg-slate-800 hover:bg-slate-900 disabled:bg-slate-300 disabled:opacity-50 text-white font-bold text-[10px] rounded-xl flex items-center justify-center gap-1.5 shadow-xs transition-colors cursor-pointer select-none disabled:cursor-not-allowed"
                  >
                    {isExporting ? (
                      <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                    ) : (
                      <FileSpreadsheet className="w-3.5 h-3.5 text-slate-100" />
                    )}
                    {isExporting
                      ? "Сборка BIM..."
                      : "Скачать Сводную Смету + Все Варианты (Excel)"}
                  </button>
                </div>
              </div>
                        ...
                        <div className="flex items-center justify-between border-b border-slate-800 pb-3 mb-3">
                          <div>
                            ...
                            <h4 className="text-xs font-bold text-slate-100">
                              PROJECT STATUS (Бюджет & Готовность)
                            </h4>
                          </div>
                        ...
                        <div className="grid grid-cols-2 gap-3 text-xs">
                          <div className="bg-slate-950/75 p-2.5 rounded-2xl border border-slate-800/60">
                            ...
                            <strong className="text-sm text-slate-100 font-mono block mt-0.5">
                              {(
                                scopeMetrics?.totalProjectCost || 0
                              ).toLocaleString()}{" "}
                          ...
                          <div className="bg-blue-950/55 p-2.5 rounded-2xl border border-blue-900/60 ring-2 ring-blue-500/15">
                            <span className="text-[9px] text-blue-300 block font-medium">
                              Осталось потратить (Закупка + СМР):
                            </span>
                            <strong className="text-sm text-blue-400 font-mono block mt-0.5">
                              {(
                                scopeMetrics?.activeBudgetCost || 0
                              ).toLocaleString()}{" "}
                              <span className="text-[10px] font-sans font-normal text-blue-400">
                                MDL
                              </span>
                            </strong>
                          </div>
                      ...
                      <div className="mt-3.5 bg-slate-950/80 p-2.5 rounded-2xl border border-slate-800/50">
                        <div className="flex items-center justify-between mb-1.5 text-xs">
                          ...
                          <strong className="text-xs font-mono text-emerald-400">
                            {scopeMetrics?.progressPercent || 0}%
                          </strong>
                        </div>
                        <div className="w-full bg-slate-850 h-2 rounded-full overflow-hidden border border-slate-800 shadow-inner">
                          <div
                            ...
                            style={{
                              width: `${scopeMetrics?.progressPercent || 0}%`,
                            }}
                          />
                        </div>
                    ...
                    <div className="lg:col-span-6 bg-white border border-slate-200 shadow-sm rounded-3xl p-5 flex flex-col justify-between">
                      <div>
                        <div className="border-b border-slate-100 pb-2 mb-3">
                          <span className="text-[9px] font-extrabold uppercase tracking-widest text-[#5c607a]">
                            Управление разделами проекта
                          </span>
                          <h4 className="text-xs font-extrabold text-slate-800">
                            Быстрый выбор конструктивов строительного блока
                          </h4>
                        </div>
                      ...
                      <div className="mt-3.5 pt-3 border-t border-slate-100">
                        <span className="text-[9px] font-extrabold uppercase tracking-widest text-[#5c607a] block mb-1.5">
                          Фильтрация таблицы (Cost Filters)
                        </span>
                    ...
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-slate-100/90 text-[10px] text-slate-500 font-bold uppercase border-b border-slate-200">
                          ...
                          <th className="py-2.5 px-3 w-[30%]">
                            Описание затрат
                          </th>
                          <th className="py-2.5 px-2 w-[35%] text-center">
                            Настройки состава (Управление опциями)
                          </th>
                      ...
                      <tbody className="divide-y divide-slate-150 text-[11px] text-slate-700">
                        {(() => {
                          const budgetCategories = [
                            {
                              ...
                              cost:
                                selectedOption.costEstimate.concreteCostMDL ||
                                0,
                              showIf: true,
                            },
                            {
                              ...
                              cost:
                                selectedOption.costEstimate.steelCostMDL || 0,
                              showIf: true,
                            },
                            {
                              id: "03",
                              ...
                              cost:
                                selectedOption.costEstimate
                                  .rebarBindingCostMDL || 0,
                              showIf: true,
                            },
                            {
                              id: "04",
                              ...
                              cost:
                                selectedOption.costEstimate.excavationCostMDL ||
                                0,
                              showIf: true,
                            },
                            {
                              ...
                              cost:
                                selectedOption.costEstimate.formworkCostMDL ||
                                0,
                              showIf: true,
                            },
                            {
                            ...
                            },
                            {
                              ...
                              cost:
                                selectedOption.costEstimate.drainageCostMDL ||
                                0,
                            ...
                            },
                            {
                              ...
                              cost:
                                selectedOption.costEstimate
                                  .pileDrillingCostMDL || 0,
                              showIf:
                                selectedOption.id === "piles" &&
                                (selectedOption.costEstimate
                                  .pileDrillingCostMDL || 0) > 0,
                            ...
                            },
                            {
                              ...
                              desc:
                                results.concreteCuring?.methodDescription ||
                                "Влажностный уход за плитой, укрытие полиэтиленом",
                            ...
                            },
                            {
                              ...
                              cost:
                                selectedOption.costEstimate.designCostMDL || 0,
                            ...
                            },
                            {
                              ...
                              cost:
                                selectedOption.costEstimate.geologyCostMDL || 0,
                              showIf: !!includeSubDesign,
                              className: "bg-indigo-50/10",
                            },
                            {
                              ...
                              cost:
                                selectedOption.costEstimate.expertiseCostMDL ||
                                0,
                          ...
                          const matchesFilter = (
                            ...
                            const scope = scopeSettings[catId] || {
                              included: true,
                              purchased: false,
                              completed: false,
                              ownerSupplied: false,
                            ...
                            if (costFilter === "not_purchased")
                              return !scope.purchased;
                            if (costFilter === "not_completed")
                              return !scope.completed;
                            if (costFilter === "mandatory")
                              return !scope.optional;
                            if (costFilter === "optional")
                              return scope.optional;
                            ...
                            if (costFilter === "machinery") {
                              const items = getCategoryDetailedItems(
                                catId,
                                selectedOption,
                                landSlope,
                                groundwaterDepth,
                          ...
                          return budgetCategories
                            .filter(
                              (c) => c.showIf && matchesFilter(c.id, c.cost),
                            )
                            .map((cat) => {
                                ...
                                <React.Fragment key={cat.id}>
                                  ...
                                  >
                                    ...
                                    >
                                      <div className="flex items-center justify-center gap-2 text-[10px]">
                                        ...
                                        <label
                                          ...
                                          title="Включение в смету"
                                        >
                                          <input
                                            type="checkbox"
                                            checked={
                                              scopeSettings[cat.id]?.included ??
                                              true
                                            }
                                            onChange={() =>
                                              toggleScopeProp(
                                        ...
                                        >
                                          ...
                                          <span
                                            className={
                                              scopeSettings[cat.id]?.purchased
                                        ...
                                        >
                                          ...
                                          <span
                                            className={
                                              scopeSettings[cat.id]?.completed
                                        ...
                                        <label
                                          className="flex items-center gap-0.5 cursor-pointer"
                                          title="Опциональная работа"
                                        >
                                          <input
                                            type="checkbox"
                                            checked={
                                              scopeSettings[cat.id]?.optional ??
                                              false
                                            }
                                            onChange={() =>
                                              toggleScopeProp(
                                          ...
                                          <span
                                            className={
                                              scopeSettings[cat.id]?.optional
                                                ? "text-slate-800 font-bold bg-slate-100 px-1 rounded"
                                                : "text-slate-400"
                                            }
                                          >
                                            Опц.
                                    ...
                                    <td className="py-2.5 px-3 text-right font-mono font-bold text-slate-900">
                                      {cat.isSlabBuiltin ? (
                                        <span className="text-emerald-700 font-semibold text-[10.5px] font-sans">
                                          0 (Включено)
                                        </span>
                                      ) : (
                                      ...
                                      <td colSpan={4} className="p-3">
                                        <div className="bg-white border border-slate-200 rounded-xl p-3 shadow-inner space-y-3">
                                          <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                                            <span className="text-[9.5px] font-bold text-slate-550 uppercase tracking-wider block">
                                              Детализация затрат категории:
                                            </span>
                                            <span className="text-[10px] font-mono bg-blue-50 text-blue-700 px-2 py-0.5 border border-blue-100 rounded-full font-bold">
                                              Итого по статье:{" "}
                                              {cat.isSlabBuiltin
                                                ? "0"
                                                : (
                                          ...
                                          {cat.isSlabBuiltin ? (
                                            <div className="text-[11px] p-2.5 bg-emerald-50 text-emerald-800 rounded-xl leading-relaxed border border-emerald-100/60 font-sans">
                                              ✨{" "}
                                              <strong>
                                                Монолитная несущая плита
                                                толщиной{" "}
                                                {selectedOption.depthM * 100} см
                                              </strong>{" "}
                                              заливается сразу по всей площади
                                              карты здания. Она автоматически
                                              служит долговечным чистовым
                                              бетонным полом 1-го этажа с
                                              предельной прочностью свыше 15
                                              тонн на м². Не требует обратной
                                              отсыпки грунта в пазухи, проливки
                                              и послойного трамбования вручную
                                              внутри фундамента, повторной
                                              закупки кладочной сетки и заливки
                                              стяжек на отдельных этапах
                                              строительных работ. Экономическое
                                              преимущество до{" "}
                                              <strong>35-45 MDL / м²</strong> на
                                              этапе финишной отделки!
                                            </div>
                                          ) : (
                                              ...
                                              <table className="w-full text-[10.5px] text-slate-600 border-collapse">
                                                <thead>
                                                  <tr className="border-b border-slate-100 text-[8.5px] text-slate-400 font-bold uppercase text-left">
                                                    <th className="pb-1.5 font-semibold w-[45%]">
                                                      Элемент расходов / вид
                                                      деятельности
                                                    </th>
                                                    <th className="pb-1.5 text-center font-semibold w-[15%]">
                                                      Категория
                                                    </th>
                                                    <th className="pb-1.5 text-right font-semibold w-[10%]">
                                                      Кол-во
                                                    </th>
                                                    <th className="pb-1.5 text-right font-semibold w-[10%]">
                                                      Ед. изм.
                                                    ...
                                                    return (
                                                      ...
                                                      >
                                                        <td className="py-2 pr-2 font-medium text-slate-800">
                                                          <div>{item.name}</div>
                                                          <div className="text-[9.5px] text-slate-400 font-normal leading-normal mt-0.5">
                                                            {item.desc}
                                                          </div>
                                                        </td>
                                                        <td className="py-2 px-1 text-center whitespace-nowrap">
                                                          <span
                                                            className={`inline-block px-1.5 py-0.5 text-[8px] font-bold uppercase border rounded-md ${badgeColors[item.type]}`}
                                                          >
                                                            {
                                                              badgeLabels[
                                                                item.type
                                                        ...
                                                        <td className="py-2 pl-2 text-right font-mono font-bold text-slate-900">
                                                          {item.total === 0 ? (
                                                            <span className="text-[9px] text-slate-450 italic font-medium font-sans">
                                                              Вкл. в смету
                                                            </span>
                                                          ) : (
                        ...
                        <tr className="bg-slate-100 text-slate-800 font-bold border-t border-slate-200 text-xs">
                          ...
                          <td className="py-2.5 px-3 text-[10px] text-slate-500 font-medium">
                            Суммарная стоимость фундамента и чернового пола под
                            ключ
                          </td>
                          <td className="py-2.5 px-3 text-right font-mono text-blue-600 text-[12.5px]">
                            {selectedOption.costMDL.toLocaleString()} MDL
                          </td>
                        </tr>
                      </tbody>
                ...
                <div className="space-y-4">
                  <div className="bg-indigo-50/50 border border-indigo-100 p-3.5 rounded-2xl space-y-1">
                    <div className="flex items-center gap-1.5 text-indigo-900 font-bold text-[11px]">
                      ...
                      <span>
                        РЕЖИМ СРАВНЕНИЯ СМЕТ: ВСЕ ПУНКТЫ С КОММЕНТАРИЯМИ БЕЗ
                        КУПЮР
                      </span>
                    </div>
                    <p className="text-[10px] text-indigo-800/90 leading-tight">
                      Ниже представлена детальная поливариантная смета. Статьи
                      расходов сопоставлены для всех трёх видов фундаментов
                      (Плита УШП, Ленточный, Свайно-ростверковый) одновременно в
                      рамках каждого раздела работ. Кликните по шапке раздела,
                      чтобы развернуть или скрыть этот пункт.
                    </p>
                  </div>
                  ...
                  <div className="space-y-3.5">
                    {(() => {
                      const slabOpt =
                        results.options.find((o) => o.id === "slab") ||
                        results.options[0];
                      const stripOpt =
                        results.options.find((o) => o.id === "strip") ||
                        results.options[1] ||
                        results.options[0];
                      const pilesOpt =
                        results.options.find((o) => o.id === "piles") ||
                        results.options[2] ||
                        results.options[0];
                      ...
                      const compareCategories = [
                        ...
                        },
                        {
                          ...
                          showIf: results.options.some(
                            (o) => o.materials.hasDrainage,
                          ),
                        },
                        {
                          id: "09",
                        ...
                        },
                        {
                          ...
                          showIf: results.options.some(
                            (o) =>
                              o.costEstimate.roughFloorCostMDL !== undefined,
                          ),
                        },
                        {
                          id: "11",
                          ...
                          showIf: results.options.some(
                            (o) =>
                              o.id === "piles" &&
                              (o.costEstimate.pileDrillingCostMDL || 0) > 0,
                          ),
                        },
                        {
                          id: "12",
                      ...
                      return compareCategories
                        ...
                        .map((cat) => {
                          ...
                          return (
                            <div
                              key={cat.id}
                              className="border border-slate-200 rounded-2xl bg-white overflow-hidden shadow-xs animate-fade-in"
                            >
                              ...
                              >
                                <div className="flex items-center gap-2">
                                  <span
                                    className={`text-slate-550 transform transition-transform duration-200 ${isExpanded ? "rotate-180" : ""}`}
                                  >
                                    <ChevronDown className="w-4 h-4 text-slate-500" />
                                  </span>
                                  <span className="font-bold text-slate-800 text-[11.5px] tracking-tight">
                                    {cat.name}
                                  </span>
                                </div>
                                  ...
                                  {[
                                    {
                                      opt: slabOpt,
                                      label: "УШП Плита",
                                      border: "border-t-3 border-amber-400",
                                      titleBg: "bg-amber-500/5 text-amber-800",
                                      badgeName: "⭐ Рекомендуемый",
                                    },
                                    {
                                      opt: stripOpt,
                                      label: "Ленточный",
                                      border: "border-t-3 border-blue-500",
                                      titleBg: "bg-blue-500/5 text-blue-800",
                                      badgeName: "🛡️ Капитальный",
                                    },
                                    {
                                      opt: pilesOpt,
                                      label: "Свайно-ростверковый",
                                      border: "border-t-3 border-emerald-500",
                                      titleBg:
                                        "bg-emerald-500/5 text-emerald-800",
                                      badgeName: "💸 Экономичный",
                                    },
                                  ].map((optionBlock) => {
                                    const {
                                      opt,
                                    ...
                                    if (cat.id === "10" && opt.id === "slab") {
                                      ...
                                      const costVal =
                                        opt.costEstimate[
                                          cat.costKey as keyof CostEstimate
                                        ];
                                      sectionCostStr =
                                        costVal !== undefined
                                          ? `${Math.round(costVal).toLocaleString()} MDL`
                                          : "—";
                                    }
                                    ...
                                    return (
                                      <div
                                        key={opt.id}
                                        className={`bg-white rounded-xl border border-slate-200 overflow-hidden flex flex-col justify-between shadow-2xs ${border}`}
                                      >
                                        <div>
                                          {/* Block Title Segment */}
                                          <div
                                            className={`px-3 py-2.5 ${titleBg} border-b border-slate-100 flex items-center justify-between`}
                                          >
                                            <div className="leading-tight">
                                              <span className="font-extrabold text-[11px] block text-slate-800">
                                                {label}
                                              </span>
                                              <span className="text-[8.5px] font-bold block text-slate-500 mt-0.5">
                                                {badgeName}
                                              </span>
                                            </div>
                                            <div className="text-right">
                                              {isSlabBuiltInVal ? (
                                                <span className="text-[9.5px] font-extrabold text-emerald-700 bg-emerald-50 border border-emerald-200/50 px-1.5 py-0.5 rounded-md">
                                                  Встроено в плиту
                                                </span>
                                              ) : isNotRequiredVal ? (
                                                <span className="text-[9.5px] font-bold text-slate-400 bg-slate-100 border border-slate-200/50 px-1.5 py-0.5 rounded-md">
                                                  Не требуется
                                                </span>
                                              ) : (
                                                <span className="text-[11.5px] font-mono font-extrabold text-slate-900 bg-slate-100 border border-slate-150 px-1.5 py-0.5 rounded-md">
                                                  {sectionCostStr}
                                                </span>
                                              )}
                                            </div>
                                          ...
                                          <div className="p-3 space-y-3">
                                            {isSlabBuiltInVal ? (
                                              <div className="text-[10px] p-2.5 bg-emerald-50/50 text-emerald-800 rounded-lg leading-relaxed border border-emerald-100/30">
                                                ✨ <strong>УШП Плита</strong>{" "}
                                                заливается сразу по всей
                                                площади, заменяя черновой пол.
                                                Исключает затраты на повторные
                                                стяжки, подушку под стяжку и
                                                обратную трамбовку.
                                              </div>
                                            ) : isNotRequiredVal ? (
                                              <div className="text-[10px] p-2.5 bg-slate-100/50 text-slate-450 rounded-lg leading-relaxed border border-slate-200/20 text-center italic">
                                                Не применяется для данного
                                                конструктива. Принято для
                                                расчёта как 0 MDL.
                                              </div>
                                            ) : items.length === 0 ? (
                                              <div className="text-[10px] p-2.5 bg-slate-100/50 text-slate-450 rounded-lg leading-relaxed border border-slate-200/20 text-center italic">
                                                Статьи расходов в данном разделе
                                                пусты.
                                              </div>
                                            ) : (
                                              <div className="space-y-2.5">
                                                {items.map((item, itemIdx) => {
                                                  const badgeColors = {
                                                    material:
                                                      "bg-amber-50 text-amber-700 border-amber-100/65",
                                                    delivery:
                                                      "bg-sky-50 text-sky-700 border-sky-100/65",
                                                    labor:
                                                      "bg-indigo-50 text-indigo-700 border-indigo-100/65",
                                                    machinery:
                                                      "bg-purple-50 text-purple-700 border-purple-100/65",
                                                  };
                                                  const badgeLabels = {
                                                    material: "📦 Мат.",
                                                    delivery: "🚚 Дост.",
                                                  ...
                                                  return (
                                                    <div
                                                      key={itemIdx}
                                                      className="bg-slate-50/60 p-2.5 rounded-xl border border-slate-200/80 hover:bg-slate-50 transition-colors flex flex-col justify-between gap-1.5 animate-fade-in"
                                                    >
                                                      <div className="flex items-start justify-between gap-1">
                                                        <span className="text-[8.5px] font-mono text-slate-450 mt-0.5 font-bold shrink-0">
                                                          {cat.id}.{itemIdx + 1}
                                                        </span>
                                                        ...
                                                        <span
                                                          className={`px-1 py-0.2 shrink-0 border rounded text-[7.5px] font-bold shrink-0 uppercase tracking-wider ${badgeColors[item.type]}`}
                                                        >
                                                          {
                                                            badgeLabels[
                                                              item.type
                  ...
                  <div className="bg-gradient-to-br from-slate-900 via-slate-900 to-indigo-950 text-white rounded-[20px] p-5.5 shadow-md border border-slate-800 space-y-4">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[10px] font-extrabold uppercase tracking-widest text-indigo-300">
                        Суммарная сметная стоимость всех вариантов по проекту
                      </span>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {[
                        {
                          opt: results.options.find((o) => o.id === "slab")!,
                          label: "Монолитная Плита УШП",
                          badgeClass:
                            "bg-amber-400/20 text-amber-300 border-amber-500/20",
                          badgeText: "⭐ Рекомендуемый",
                        },
                        {
                          opt: results.options.find((o) => o.id === "strip")!,
                          label: "Ленточный глубокий",
                          badgeClass:
                            "bg-blue-400/20 text-blue-300 border-blue-500/20",
                          badgeText: "🛡️ Капитальный",
                        },
                        {
                          opt: results.options.find((o) => o.id === "piles")!,
                          label: "Свайно-ростверковый",
                          badgeClass:
                            "bg-emerald-400/20 text-emerald-300 border-emerald-500/20",
                          badgeText: "💸 Экономичный",
                        },
                      ].map((item, idx) => (
                        <div
                          key={idx}
                          className="bg-white/[0.04] border border-white/10 rounded-xl p-3.5 hover:bg-white/[0.06] transition-colors flex flex-col justify-between"
                        >
                          <div className="flex justify-between items-start mb-2.5">
                            ...
                            <span
                              className={`text-[8px] font-extrabold px-1.5 py-0.2 border rounded-md uppercase tracking-wider ${item.badgeClass}`}
                            >
                              {item.badgeText}
                            </span>
                          </div>
                          <div className="flex items-baseline justify-between mt-1 pt-1.5 border-t border-white/[0.05]">
                            <span className="text-sm font-extrabold font-mono text-indigo-200">
                              {item.opt.costMDL.toLocaleString()} MDL
                            </span>
                            <span className="text-[10px] text-slate-400 font-mono italic">
                              ~{" "}
                              {Math.round(
                                item.opt.costMDL / 19.8,
                              ).toLocaleString()}{" "}
                              EUR
                            </span>
                          </div>
              ...
              <p className="text-[10px] text-slate-450 mt-2 italic leading-normal">
                * Цены рассчитаны по среднему прейскуранту Кишинева, Бельц и
                Оргеева на 2026 год. Расчет является предварительным проектным
                планом и корректируется после геодезического бурения
                непосредственно на пятне застройки.
              </p>
            </div>

            {/* WALLS & SEISMIC FRAME CALCULATION */}
            {results.walls && (
              <div className="mt-8 bg-white/95 backdrop-blur-md border border-slate-200/60 rounded-[20px] p-6 shadow-sm shadow-slate-200/30 font-sans">
                <h2 className="text-sm font-display font-bold text-slate-800 mb-4 flex items-center gap-2">
                  <Warehouse className="w-5 h-5 text-indigo-500" />
                  Расчет возведения коробки (Стены и сейсмокаркас)
                </h2>

                {(() => {
                  const walls = results.walls;
                  if (!walls) return null;
                  const isStone = walls.needsBelts;

                  return (
                    <>
                      {!isStone ? (
                        <div className="mb-5 bg-emerald-50/50 rounded-xl p-4 border border-emerald-100">
                          <h3 className="text-xs font-bold text-emerald-900 mb-1 flex items-center gap-2">
                            ✅ Сейсмокаркас не требуется (Легкая конструкция)
                          </h3>
                          <p className="text-xs text-emerald-800/80 leading-relaxed">
                            Деревянные и SIP-панельные дома обладают высокой
                            пространственной жесткостью и легкостью, поэтому не
                            требуют заливки тяжелого железобетонного каркаса.
                            Такие конструкции отлично переносят сейсмику Молдовы
                            без разрушений.
                          </p>
                        </div>
                      ) : walls.needsColumns ? (
                        <div className="mb-5 bg-indigo-50/50 rounded-xl p-4 border border-indigo-100 pb-5">
                          <h3 className="text-xs font-bold text-indigo-900 mb-2 flex items-center gap-2">
                            <ShieldAlert className="w-4 h-4 text-indigo-600" />
                            Требуется Железобетонный Каркас (Зона{" "}
                            {walls.seismicity} баллов)
                          </h3>
                          <p className="text-xs text-indigo-800/80 leading-relaxed mb-3">
                            Согласно <b>NCM F.03.02-2005</b> (Строительство в
                            сейсмических районах РМ), для вашей зоны
                            сейсмичности ({walls.seismicity} баллов) и этажности
                            ({results.input.floors} эт.){" "}
                            <b>строго обязательно</b> возведение комплексной
                            кладки (колонны + армопояса). Штучный камень
                            (котелец, газобетон) при такой нагрузке не может
                            работать как самостоятельная несущая стена — он
                            служит лишь заполнителем монолитного железобетонного
                            каркаса.
                            {walls.seismicity === 9 &&
                              " Южный регион (Кагул, Вулканешты) подвержен влиянию зоны Вранча, поэтому требования наиболее жесткие."}
                            {walls.seismicity === 8 &&
                              " Центральный регион (Кишинев) находится в зоне 8 баллов, где железобетонные сердечники обеспечивают безопасную приемку дома без нарушений."}
                          </p>
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            <div className="bg-white rounded-lg p-3 border border-indigo-50 shadow-sm text-center">
                              <span className="block text-[10px] text-indigo-500 font-semibold uppercase mb-1">
                                Колонны
                              </span>
                              <span className="block font-mono text-indigo-900 font-bold text-sm">
                                {walls.columnsCount} шт
                              </span>
                        ...
                        <div className="mb-5 bg-amber-50/50 rounded-xl p-4 border border-amber-100 pb-5">
                          <h3 className="text-xs font-bold text-amber-900 mb-2 flex items-center gap-2">
                            <ShieldAlert className="w-4 h-4 text-amber-600" />
                            Допускается кладка без колонн (Зона{" "}
                            {walls.seismicity} баллов)
                          </h3>
                          <p className="text-xs text-amber-800/80 leading-relaxed mb-3">
                            Согласно <b>NCM F.03.02-2005</b>, для Северной зоны
                            ({walls.seismicity} баллов) и малой этажности (
                            {results.input.floors} эт.){" "}
                            <b>колонны не являются строго обязательными</b>{" "}
                            (простая кладка допускается). ОБЯЗАТЕЛЬНЫМ
                            требованием остается монолитный <b>сейсмопояс</b> по
                            верху стен (под перекрытием). Камни выкладываются с
                            армированием сеткой каждые 3-4 ряда. Это существенно
                            удешевляет коробку без проблем с приемкой проекта
                            органами архитектуры региона.
                          </p>
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            <div className="bg-white rounded-lg p-3 border border-amber-50 shadow-sm text-center opacity-60">
                              <span className="block text-[10px] text-amber-600 font-semibold uppercase mb-1">
                                Колонны
                              </span>
                              <span className="block font-mono text-amber-900 text-sm">
                                Не требуются
                              </span>
                            ...
                            <div className="bg-white rounded-lg p-3 border border-amber-50 shadow-sm text-center">
                              <span className="block text-[10px] text-amber-600 font-semibold uppercase mb-1">
                                Армопояса
                              </span>
                              <span className="block font-mono text-amber-900 font-bold text-sm">
                                {walls.seismicBeltLengthM} м.п.
                              </span>
                              <span className="block text-[10px] text-amber-600 mt-0.5">
                                Под все перекрытия
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-4">
                  <div>
                    <h4 className="text-xs font-semibold text-slate-700 mb-3 uppercase tracking-wider pl-1 border-l-2 border-slate-300">
                      Стеновой комплект
                    </h4>
                    <div className="flex flex-col gap-2">
                      <div className="flex justify-between items-center text-xs p-2 rounded-lg bg-slate-50 border border-slate-100">
                        <span className="text-slate-600">
                          Площадь стен брутто{" "}
                          {currentInput.roofType === "GABLE_METAL" &&
                            "(с учетом фронтонов)"}
                        </span>
                        <span className="font-mono font-medium text-slate-800">
                          {results.walls.wallAreaGrossM2} м²
                        </span>
                      </div>
                      <div className="flex justify-between items-center text-xs p-2 rounded-lg bg-slate-50 border border-slate-100">
                        <span className="text-slate-600">
                          Чистая площадь (без проемов)
                        </span>
                        <span className="font-mono font-bold text-slate-800">
                          {results.walls.wallAreaNetM2} м²
                        </span>
                      </div>
                      <div className="flex justify-between items-center text-xs p-2 rounded-lg bg-slate-50 border border-slate-100">
                        <span className="text-slate-600">Толщина стен</span>
                        <span className="font-mono font-medium text-slate-800">
                          {results.walls.wallThicknessM * 1000} мм
                        </span>
                      </div>
                      {!results.walls.isFrame && (
                        <div className="flex justify-between items-center text-xs p-2 rounded-lg bg-slate-50 border border-slate-100">
                          <span className="text-slate-600">
                            Сетка рядовая кладочная
                          </span>
                          <span className="font-mono font-medium text-slate-800">
                            {results.walls.masonryRebarKg} кг
                          </span>
                        </div>
                      )}
                      {results.walls.ventChannelsCount > 0 && (
                        <div className="flex justify-between items-center text-xs p-2 rounded-lg bg-slate-50 border border-slate-100">
                          <span className="text-slate-600">
                            Вентканалы / Вентблоки
                          </span>
                          <span className="font-mono font-medium text-slate-800">
                            {results.walls.ventChannelsCount} шт /{" "}
                            {results.walls.ventBlocksCount} блоков
                          </span>
                        </div>
                      )}
                      <div className="flex justify-between items-center text-xs p-2 rounded-lg border border-indigo-100 bg-indigo-50/30">
                        <span className="text-indigo-800 font-semibold">
                          Объем кладки
                        </span>
                        <span className="font-mono font-bold text-indigo-900">
                          {results.walls.blocksVolumeM3} м³
                        </span>
                      </div>
                      <div className="flex justify-between items-center text-xs p-2 rounded-lg border border-indigo-100 bg-indigo-50/80">
                        <span className="text-indigo-800 font-bold">
                          Количество блоков
                        </span>
                        <span className="font-mono font-black text-indigo-900 text-sm">
                          {results.walls.blocksCount} шт
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    <h4 className="text-xs font-semibold text-slate-700 mb-3 uppercase tracking-wider pl-1 border-l-2 border-emerald-300">
                      Бюджет Коробки (Черновой)
                    </h4>
                    <div className="flex flex-col gap-2">
                      <div className="flex justify-between items-center text-xs p-2 rounded-lg bg-slate-50 border border-slate-100">
                        <span className="text-slate-600">Стоимость блоков</span>
                        <span className="font-mono text-slate-700">
                          {results.walls.blocksCostMDL.toLocaleString()} MDL
                        </span>
                      </div>
                      {results.walls.needsColumns && (
                        <>
                          <div className="flex justify-between items-center text-xs p-2 rounded-lg bg-slate-50 border border-slate-100">
                            <span className="text-slate-600">
                              Бетон на каркас (С16/20)
                            </span>
                            <span className="font-mono text-slate-700">
                              {results.walls.concreteCostMDL.toLocaleString()}{" "}
                              MDL
                            </span>
                          </div>
                          <div className="flex justify-between items-center text-xs p-2 rounded-lg bg-slate-50 border border-slate-100">
                            <span className="text-slate-600">
                              Арматура (сердечники, пояса)
                            </span>
                            <span className="font-mono text-slate-700">
                              {results.walls.rebarCostMDL.toLocaleString()} MDL
                            </span>
                          </div>
                        </>
                      )}
                      {!results.walls.needsColumns &&
                        results.walls.needsBelts && (
                          <>
                            <div className="flex justify-between items-center text-xs p-2 rounded-lg bg-slate-50 border border-slate-100">
                              <span className="text-slate-600">
                                Бетон (пояса С16-С20)
                            ...
                            <div className="flex justify-between items-center text-xs p-2 rounded-lg bg-slate-50 border border-slate-100">
                              ...
                              <span className="font-mono text-slate-700">
                                {results.walls.rebarCostMDL.toLocaleString()}{" "}
                                MDL
                              </span>
                            </div>
                          </>
                      ...
                      {!results.walls.isFrame && (
                        <div className="flex justify-between items-center text-xs p-2 rounded-lg bg-slate-50 border border-slate-100">
                          <span className="text-slate-600">
                            Кладочная сетка (Стеклопластик/Сталь)
                          </span>
                          <span className="font-mono text-slate-700">
                            {results.walls.masonryRebarCostMDL.toLocaleString()}{" "}
                            MDL
                          </span>
                        </div>
                      )}
                      {results.walls.ventChannelsCount > 0 && (
                        <div className="flex justify-between items-center text-xs p-2 rounded-lg bg-slate-50 border border-slate-100">
                          <span className="text-slate-600">
                            Керамоблоки для вентканалов
                          </span>
                          <span className="font-mono text-slate-700">
                            {results.walls.ventBlocksCostMDL.toLocaleString()}{" "}
                            MDL
                          </span>
                        </div>
                      )}
                      <div className="flex justify-between items-center text-xs p-2 rounded-lg bg-slate-50 border border-slate-100">
                        ...
                        <span className="font-mono text-slate-700">
                          {results.walls.laborCostMDL.toLocaleString()} MDL
                        </span>
                      </div>
                      <div className="flex justify-between items-center text-[13px] p-2.5 rounded-lg border border-emerald-200 bg-emerald-50/70 mt-1">
                        <span className="text-emerald-900 font-bold">
                          ИТОГО КОРОБКА:
                        </span>
                        <span className="font-mono font-black text-emerald-700">
                          {results.walls.totalCostMDL.toLocaleString()} MDL
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="mt-6 border-t border-slate-100 pt-5">
                    ...
                    <ul className="space-y-3">
                      {!results.walls.isFrame && (
                        <li className="flex items-start gap-2">
                          ...
                          <span className="text-xs text-slate-700">
                            <strong>Армирование рядов кладки:</strong> Согласно
                            NCM, кладочная сетка (стальная оцинкованная или
                            композитная) должна укладываться каждые{" "}
                            <strong>600 мм по высоте</strong> (обычно каждый
                            3-ий ряд для блоков высотой 200 мм). Нахлест сеток
                            по длине не менее <strong>150 мм</strong>.
                          </span>
                        </li>
                      )}
                      {!results.walls.isFrame && (
                        <li className="flex items-start gap-2">
                          ...
                          />
                          <span className="text-xs text-slate-700">
                            <strong>Защитный слой раствора:</strong> Толщина
                            растворного шва при укладке сетки не должна
                            превышать <strong>15 мм</strong>, при этом сетка
                            должна быть полностью утоплена в растворе (защита от
                            коррозии).
                          </span>
                        </li>
                      )}
                      {results.walls.needsColumns && (
                        <li className="flex items-start gap-2">
                          ...
                          <span className="text-xs text-slate-700">
                            <strong>
                              Анкеровка кладки к монолитным сердечникам
                              (колоннам):
                            </strong>{" "}
                            В местах примыкания кладки к колоннам должны быть
                            выпущены арматурные стержни или сетка{" "}
                            <strong>в тело колонны</strong> (до её заливки) для
                            жесткой связи каркаса с камнем.
                          </span>
                        </li>
                      )}
                      {results.walls.needsColumns && (
                        <li className="flex items-start gap-2">
                          ...
                          />
                          <span className="text-xs text-slate-700">
                            <strong>Выпуски из фундамента:</strong> Проверить
                            жесткую связку (нахлест) арматуры фундаментной
                            плиты/ленты с арматурой колонн. Перепуск должен
                            составлять не менее <strong>40-50 диаметров</strong>{" "}
                            арматуры (т.е. ~50-60 см). Обязательна Г-образная
                            отгибка стержней колонны в теле фундамента.
                          </span>
                        </li>
                      )}
                      {results.walls.needsBelts && (
                        <li className="flex items-start gap-2">
                          ...
                          />
                          <span className="text-xs text-slate-700">
                            <strong>Сейсмопояс (Армопояс):</strong> Проверить
                            непрерывность! Пояс должен быть обязательно замкнут
                            по всему периметру ВСЕХ несущих стен. В углах и
                            пересечениях обязательно ставятся дополнительные
                            П-образные или Г-образные хомуты-усилители.
                          </span>
                        </li>
                      )}
                      {!results.walls.isFrame && (
                        <li className="flex items-start gap-2">
                          ...
                          />
                          <span className="text-xs text-slate-700">
                            <strong>Опирание перемычек:</strong> Для
                            сейсмоопасных районов РМ опирание железобетонных
                            перемычек (над окнами и дверями) на кладку должно
                            составлять <strong>строго не менее 250 мм</strong> с
                            каждой стороны проема.
                          </span>
                        </li>
                      )}
                      {results.walls.ventChannelsCount > 0 && (
                        <li className="flex items-start gap-2">
                          ...
                          />
                          <span className="text-xs text-slate-700">
                            <strong>Вентканалы и дымоходы:</strong> Проверить
                            чистоту каналов "на свет" (отсутствие плюх раствора
                            внутри). При возведении в газобетоне, для дымоходов
                            котла и камина обязательно использовать
                            специализированные жаростойкие керамические/стальные
                            гильзы.
                          </span>
                        </li>
                      )}
                      <li className="flex items-start gap-2">
                        ...
                        />
                        <span className="text-xs text-slate-700">
                          <strong>Геометрия и вертикальность:</strong>{" "}
                          Отклонение стены по вертикали не превышает{" "}
                          <strong>10 мм</strong> на высоту одного этажа.
                          Проверка проводится двухметровым правилом-уровнем или
                          лазерным нивелиром по всем углам здания.
                        </span>
                      </li>
                    </ul>
                  </div>
                  ...
                  {!results.walls.isFrame && (
                    <div className="mt-4 bg-slate-800 rounded-xl p-4 text-slate-200">
                      <h5 className="text-[11px] font-bold text-slate-300 uppercase tracking-wider mb-3">
                        Схема: Правильное армирование кладки (NCM F.03.02-2005)
                      </h5>
                      <div className="flex flex-col md:flex-row gap-6 items-center">
                        <div className="w-full md:w-1/2 rounded bg-slate-900 border border-slate-700 p-4 aspect-video flex flex-col justify-center items-center relative overflow-hidden">
                          {/* Abstract Block Wall visual */}
                          <div className="flex flex-col gap-[2px] w-full max-w-[200px]">
                            {[1, 2, 3, 4, 5, 6].map((row) => (
                              <div key={row} className="relative">
                                <div
                                  className={`flex gap-[2px] ${row % 2 === 0 ? "-ml-[20px]" : ""}`}
                                >
                                  {[1, 2, 3, 4].map((block) => (
                                    <div
                                      key={block}
                                      className="h-[20px] min-w-[60px] bg-slate-600 rounded-[1px] border border-slate-500/50"
                                    ></div>
                                  ))}
                                </div>
                                {/* Reinforcement layer every 3rd row */}
                                {row % 3 === 0 && (
                                  <div className="absolute -bottom-[3px] left-[-20px] right-[-20px] h-[3px] z-10 flex my-auto items-center overflow-hidden opacity-90">
                                    <div className="w-full h-[1px] bg-emerald-400 absolute top-1/2"></div>
                                    <div className="w-full h-[1px] h-full bg-transparent flex justify-between absolute top-0">
                                      {[...Array(20)].map((_, i) => (
                                        <div
                                          key={i}
                                          className="w-[1px] h-full bg-emerald-400 rotate-45"
                                        ></div>
                                      ))}
                                    </div>
                                  </div>
                          ...
                          {/* Column intersection */}
                          {results.walls.needsColumns && (
                            <div className="absolute right-[20px] top-0 bottom-0 w-[40px] bg-slate-500/20 border-x border-dashed border-indigo-400/50 flex flex-col items-center justify-between py-2">
                              <div className="w-1 h-full bg-indigo-500/80 rounded-full"></div>
                              <div className="w-1 h-full bg-indigo-500/80 rounded-full absolute left-[10px]"></div>
                              <div className="w-1 h-full bg-indigo-500/80 rounded-full absolute right-[10px]"></div>
                            </div>
                          )}
                        </div>
                        <div className="w-full md:w-1/2">
                          <ol className="text-xs space-y-2 text-slate-400">
                            <li>
                              <span className="inline-block w-3 h-3 bg-slate-600 mr-2 rounded-sm outline outline-1 outline-slate-500"></span>{" "}
                              Блок (Котелец / Газобетон)
                            </li>
                            <li>
                              <span className="inline-block w-4 h-[2px] bg-emerald-400 mr-2 align-middle"></span>{" "}
                              Кладочная сетка (в шве) — каждые 600мм
                            </li>
                            {results.walls.needsColumns && (
                              <li>
                                <span className="inline-block w-3 h-full bg-indigo-500 px-1 border-x border-indigo-400 border-dashed mr-2 align-middle text-[8px] text-white">
                                  Ж/Б
                                </span>{" "}
                                Монолитный сердечник (Ж/Б Колонна) с анкеровкой
                                сетки внутрь.
                              </li>
                            )}
                            <li>
                              <span className="inline-block w-3 h-3 border-t-4 border-slate-300 mr-2 rounded-sm align-middle"></span>{" "}
                              Растворный шов — строго ≤ 15мм. Сетка полностью
                              утоплена.
                            </li>
                          </ol>
                        </div>
            ...
            {results.whiteBox && (
              <div className="mt-8 bg-white/95 backdrop-blur-md border border-slate-200/60 rounded-[20px] p-6 shadow-sm shadow-slate-200/30 font-sans">
                <h2 className="text-sm font-display font-bold text-slate-800 mb-4 flex items-center gap-2">
                  ...
                  Расчет до "Белого Варианта" (Кровля, Окна, Фасад, Перекрытия и
                  Внутренняя отделка)
                </h2>

                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                  {/* КРОВЛЯ */}
                  <div className="bg-sky-50/50 rounded-xl p-4 border border-sky-100 flex flex-col justify-between relative overflow-hidden">
                    {currentInput.buildQuality === "ECONOMY" && (
                      <div className="absolute top-0 right-0 bg-amber-500 text-white text-[8px] font-bold px-2 py-0.5 rounded-bl select-none">
                        ECONOMY
                      </div>
                    ...
                    <div>
                      <h3 className="text-xs font-bold text-sky-900 mb-2">
                        Кровля:{" "}
                        {currentInput.roofType === "GABLE_METAL"
                          ? "Двускатная (М/Ч)"
                          : currentInput.roofType === "GABLE_SHINGLE"
                            ? "Двускатная (Мягкая)"
                            : currentInput.roofType === "HIP_CERAMIC"
                              ? "Вальмовая (Керам.)"
                              : currentInput.roofType === "HIP_METAL"
                                ? "Вальмовая (М/Ч)"
                                : currentInput.roofType === "FLAT_PVC"
                                  ? "Плоская ПВХ"
                                  : "Односкатная"}
                      </h3>
                      <div className="text-[10px] text-sky-800/80 mb-3 space-y-1">
                        <div className="flex justify-between">
                          <span>Площадь (с учетом свесов):</span>{" "}
                          <span className="font-mono">
                            {results.whiteBox.roofAreaM2} м²
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span>Деревянная стропильная система:</span>{" "}
                          <span className="font-mono">Включена</span>
                        </div>
                        <div className="flex justify-between text-slate-500">
                          <span>Утепление (200мм):</span>{" "}
                          <span className="font-mono">
                            {currentInput.buildQuality === "ECONOMY"
                              ? "150мм эконом"
                              : currentInput.buildQuality === "PREMIUM"
                                ? "Минвата 250мм + Мембраны"
                                : "Включено 200мм"}
                          </span>
                        </div>
                      </div>
                    </div>
                    <div className="mt-2 border-t border-sky-100 pt-2 flex justify-between items-center text-xs font-bold text-sky-900">
                      <span>Стоимость кровли:</span>
                      <span className="font-mono">
                        {results.whiteBox.roofCostMDL.toLocaleString()} MDL
                      </span>
                    </div>
                  </div>

                  {/* ОСТЕКЛЕНИЕ */}
                  <div className="bg-blue-50/50 rounded-xl p-4 border border-blue-100 flex flex-col justify-between relative overflow-hidden">
                    {currentInput.buildQuality === "ECONOMY" && (
                      <div className="absolute top-0 right-0 bg-amber-500 text-white text-[8px] font-bold px-2 py-0.5 rounded-bl select-none">
                        ECONOMY
                      </div>
                    ...
                    <div>
                      <h3 className="text-xs font-bold text-blue-900 mb-2">
                        Окна:{" "}
                        {currentInput.glazingType === "PANORAMIC"
                          ? "Панорамное"
                          : "Стандартное"}
                      </h3>
                      <div className="text-[10px] text-blue-800/80 mb-3 space-y-1">
                        <div className="flex justify-between">
                          <span>Площадь окон:</span>{" "}
                          <span className="font-mono">
                            {results.whiteBox.glazingAreaM2} м²
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span>Процент от стен:</span>{" "}
                          <span className="font-mono">
                            {currentInput.glazingType === "PANORAMIC"
                              ? "35%"
                              : "15%"}
                    ...
                    <div className="mt-2 border-t border-blue-100 pt-2 flex justify-between items-center text-xs font-bold text-blue-900">
                      <span>Окна / Витражи:</span>
                      <span className="font-mono">
                        {results.whiteBox.windowsCostMDL.toLocaleString()} MDL
                      </span>
                    </div>
                  </div>

                  {/* ФАСАД */}
                  <div className="bg-slate-50/50 rounded-xl p-4 border border-slate-200 flex flex-col justify-between relative overflow-hidden">
                    {currentInput.buildQuality === "ECONOMY" && (
                      <div className="absolute top-0 right-0 bg-amber-500 text-white text-[8px] font-bold px-2 py-0.5 rounded-bl select-none">
                        ECONOMY
                      </div>
                      ...
                      <div className="text-[10px] text-slate-600 mb-3 space-y-1">
                        <div className="flex justify-between">
                          ...
                          <span className="font-mono truncate ml-2 text-right">
                            {currentInput.buildQuality === "ECONOMY"
                              ? "Базовый слой утепл. (Серый)"
                              : currentInput.buildQuality === "PREMIUM"
                                ? currentInput.facadeTech === "WET" ? "Силиконовая штукатурка premium" : currentInput.facadeTech === "FACE_BRICK" ? "Импортный Клинкер" : "HPL панели (Премиум)"
                                : currentInput.facadeTech === "WET"
                                  ? "Мокрый фасад (Штукат.)"
                                  : currentInput.facadeTech === "FACE_BRICK"
                                    ? "Облицовочный кирпич"
                                    : "Вентилируемый (Плитка)"}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span>Площадь стен:</span>{" "}
                          <span className="font-mono">
                            {results.whiteBox.facadeAreaM2} м²
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span>Утеплитель (Спецификация):</span>{" "}
                          <span className="font-mono truncate max-w-[120px]">
                            {currentInput.buildQuality === "ECONOMY"
                              ? "EPS50 50мм"
                              : currentInput.buildQuality === "PREMIUM"
                                ? "Фасадная Базальтовая Минвата 150мм"
                                : "Минвата / EPS 100мм"}
                          </span>
                        </div>
                      </div>
                    </div>
                    <div className="mt-2 border-t border-slate-200 pt-2 flex justify-between items-center text-xs font-bold text-slate-800">
                      <span>Фасад с работой:</span>
                      <span className="font-mono">
                        {results.whiteBox.facadeCostMDL.toLocaleString()} MDL
                      </span>
                    </div>
                  </div>

                  {/* ПЕРЕКРЫТИЯ */}
                  {(currentInput.floors > 1 || currentInput.hasBasement) && (
                    <div className="bg-emerald-50/50 rounded-xl p-4 border border-emerald-100 flex flex-col justify-between relative overflow-hidden">
                      {currentInput.buildQuality === "ECONOMY" && (
                        <div className="absolute top-0 right-0 bg-amber-500 text-white text-[8px] font-bold px-2 py-0.5 rounded-bl select-none">
                          ECONOMY
                        </div>
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-emerald-800/80 mb-3 space-y-1">
                          <div className="flex justify-between">
                            <span>Тип:</span>{" "}
                            <span className="font-mono text-right">
                              {currentInput.slabMaterial === "MONOLITH"
                                ? "Ж/Б Монолит (Арматура+Бетон)"
                                : currentInput.slabMaterial === "HOLLOW_CORE"
                                  ? "Пустотные ПБК Плиты"
                                  : "Деревянные балки"}
                            </span>
                          </div>
                          <div className="flex justify-between">
                            ...
                            <span className="font-mono">
                              {Math.floor(
                                !results.whiteBox.slabCostMDL
                                  ? 0
                                  : results.whiteBox.slabCostMDL /
                                      (currentInput.slabMaterial === "MONOLITH"
                                        ? 1400
                                        : currentInput.slabMaterial === "TIMBER"
                                          ? 450
                                          : 900),
                              )}{" "}
                              м²
                      ...
                      <div className="mt-2 border-t border-emerald-100 pt-2 flex justify-between items-center text-xs font-bold text-emerald-900">
                        <span>Перекрытия:</span>
                        <span className="font-mono">
                          {results.whiteBox.slabCostMDL.toLocaleString()} MDL
                        </span>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="bg-purple-50/50 rounded-xl p-4 border border-purple-100 flex flex-col justify-between relative overflow-hidden">
                    {currentInput.buildQuality === "ECONOMY" && (
                      <div className="absolute top-0 right-0 bg-amber-500 text-white text-[8px] font-bold px-2 py-0.5 rounded-bl select-none">
                        ECONOMY
                      </div>
                    ...
                    <div>
                      <h3 className="text-xs font-bold text-purple-900 mb-2">
                        Внутренняя отделка (Стяжка и Штукатурка)
                      </h3>
                      <div className="text-[10px] text-purple-800/80 mb-3 space-y-1">
                        <div className="flex justify-between">
                          <span>
                            Штукатурка (
                            {currentInput.buildQuality === "ECONOMY"
                              ? "Базовая песчаная"
                              : currentInput.buildQuality === "PREMIUM"
                                ? "Машинная МР75 + Финиш Q3"
                                : "Машинная МР75"}
                            ):
                          </span>{" "}
                          <span className="font-mono">
                            {results.whiteBox.internalPlasterAreaM2} м²
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span>
                            ...
                            {currentInput.buildQuality === "ECONOMY"
                              ? "Черновая"
                              : currentInput.buildQuality === "PREMIUM"
                                ? "Шумоизоляция + XPS + Финишная"
                                : "с утеплением XPS"}
                            ):
                          </span>{" "}
                          <span className="font-mono">
                            {results.whiteBox.floorsScreedAreaM2} м²
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span>Ж/Б Лестница:</span>{" "}
                          <span className="font-mono">
                            {results.whiteBox.hasStairs ? "Включена" : "Нет"}
                          </span>
                        </div>
                      </div>
                    </div>
                    <div className="mt-2 border-t border-purple-100 pt-2 flex justify-between items-center text-xs font-bold text-purple-900">
                      <span>Стяжка, штукатурка, лестницы:</span>
                      <span className="font-mono">
                        {(
                          results.whiteBox.internalPlasterCostMDL +
                          results.whiteBox.floorsScreedCostMDL +
                          results.whiteBox.stairsCostMDL
                        ).toLocaleString()}{" "}
                        MDL
                      </span>
                    </div>
                ...
                <div className="mt-4 bg-emerald-50 border border-emerald-100 rounded-xl p-4 flex justify-between items-center flex-wrap gap-2 text-sm">
                  <div className="text-emerald-900">
                    <span className="font-bold">
                      Итоговая стоимость коробки (Дом в "Белом Варианте"):
                    </span>
                    <br />
                    <span className="text-[10px] opacity-80">
                      (Фундамент + Стены Каркас + Кровля + Окна + Фасад +
                      Перекрытия/Лестница + Штукатурка/Стяжка + Доп. Работы)
                    </span>
                  </div>
                  <div className="font-mono font-bold text-lg text-emerald-700 bg-emerald-100/50 px-3 py-1 rounded">
                    {(
                      (selectedOption?.costEstimate?.totalCostMDL || 0) +
                      (results.walls?.totalCostMDL || 0) +
                      results.whiteBox.totalWhiteBoxCostMDL +
                      (results.utilities?.totalCostMDL || 0) +
                      (results.backfill?.totalCostMDL || 0) +
                      (results.blindArea?.totalCostMDL || 0) +
                      (results.concreteCuring?.totalCostMDL || 0)
                    ).toLocaleString()}{" "}
                    MDL
                  </div>
                </div>
                ...
                <div className="mt-6 border-t border-slate-100 pt-5">
                  <h4 className="text-[13px] font-bold text-slate-800 mb-3 flex items-center gap-2">
                    <ShieldCheck className="w-4 h-4 text-sky-600" /> Чек-лист
                    приемки: Кровля, Остекление, Фасад (NCM F.03.02-2005)
                  </h4>
                  <div className="bg-slate-50 border border-slate-200 rounded-lg p-4">
                    <ul className="space-y-3">
                      <li className="flex items-start gap-2">
                        ...
                        />
                        <span className="text-xs text-slate-700">
                          <strong>Кровля (Дерево & Металл):</strong> Все
                          элементы стропильной системы обработаны{" "}
                          <strong>огнебиозащитой</strong>. Для металлочерепицы:
                          крепление осуществляется кровельными саморезами с
                          ЭПДМ-резинками в <strong>нижнюю</strong> волну строго
                          под прямым углом, исключая перетяжку (деформацию
                          резинки).
                        </span>
                      </li>
                      <li className="flex items-start gap-2">
                        ...
                        />
                        <span className="text-xs text-slate-700">
                          <strong>Крепление Мауэрлата:</strong> Жесткое
                          крепление мауэрлата (опорного бруса) анкерами или
                          шпильками в <strong>монолитный армопояс</strong>.
                          Забивание шпилек просто в котелец или газоблок СТРОГО
                          запрещено — кровлю снесет ветром. Шаг шпилек не более
                          1000 мм.
                        </span>
                      </li>
                      <li className="flex items-start gap-2">
                        ...
                        />
                        <span className="text-xs text-slate-700">
                          <strong>Ветрозащита и Вентиляция кровли:</strong>{" "}
                          Обязательно наличие <strong>контррейки</strong> (зазор
                          50мм) над гидроизоляционной мембраной для отвода
                          конденсата из-под кровельного покрытия.
                        </span>
                      </li>
                      <li className="flex items-start gap-2">
                        <input
                        ...
                        />
                        <span className="text-xs text-slate-700">
                          <strong>Теплый монтаж окон (Трехслойный шов):</strong>{" "}
                          Обязательное применение паропроницаемой ленты (ПСУЛ)
                          снаружи для вывода влаги и пароизоляционной ленты
                          изнутри помещения, защищающих монтажную пену от
                          увлажнения и разрушения ультрафиолетом.
                        </span>
                      </li>
                      {currentInput.glazingType === "PANORAMIC" && (
                        <li className="flex items-start gap-2">
                          ...
                          <span className="text-xs text-slate-700">
                            <strong>
                              Панорамные фасады (Усиленный крепеж):
                            </strong>{" "}
                            Большой вес витражей обязаны нести{" "}
                            <strong>
                              специальные теплые подставочные профили
                            </strong>{" "}
                            (пеностекло или чистый ПВХ, не пластиковый
                            эконом-базовый). Анкеровка рам в проеме: шаг не
                            более <strong>600 мм</strong> для защиты от ветровой
                            пульсации по нормам высотных нагрузок РМ.
                          </span>
                        </li>
                      )}
                      <li className="flex items-start gap-2">
                        ...
                        <span className="text-xs text-slate-700">
                          <strong>
                            Фасад: Тепловой контур (Цокольное примыкание):
                          </strong>{" "}
                          Обязательное использование перфорированной{" "}
                          <strong>цокольной планки</strong> (капельника) в месте
                          перехода фасадного утеплителя к ЭППС-утеплителю
                          цоколя. Запрещается доводить обычный пенопласт мокрого
                          фасада до уровня грунта — он впитывает капиллярную
                          влагу.
                        </span>
                      </li>
                      {currentInput.facadeTech === "VENTILATED" && (
                        <li className="flex items-start gap-2">
                          <input
                            type="checkbox"
                            className="mt-1 rounded text-sky-500 focus:ring-sky-500"
                          />
                          <span className="text-xs text-slate-700">
                            <strong>Вентилируемый фасад: Зазор:</strong>{" "}
                            Воздушный зазор между гидро-ветрозащитной мембраной
                            утеплителя и облицовочной панелью должен составлять{" "}
                            <strong>строго не менее 40 мм</strong> для
                            обеспечения тяги и просушки ваты.
                          </span>
                        </li>
                      )}
                    </ul>
            ...
            <div className="mt-6 pt-5 border-t border-slate-100">
              <h3 className="text-xs font-display font-bold text-slate-800 mb-3 block">
                ⚠️ Инженерные риски проседания и геотехнического пучения
              </h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-slate-50/80 border border-slate-200/40 rounded-2xl p-3.5 text-xs">
                  <div className="flex justify-between items-center mb-1.5 font-semibold">
                    <span className="text-slate-600">
                      Пучение глин (касательное)
                    </span>
                    <span
                      className={`font-mono font-bold ${results.frostHeavingPercent > 60 ? "text-red-500" : "text-emerald-600"}`}
                    >
                ...
                <div className="bg-slate-50/80 border border-slate-200/40 rounded-2xl p-3.5 text-xs">
                  <div className="flex justify-between items-center mb-1.5 font-semibold">
                    <span className="text-slate-600">
                      Просадочность коробки
                    </span>
                    <span
                      className={`font-mono font-bold ${results.collapsibilityPercent > 60 ? "text-red-500" : "text-emerald-600"}`}
                    >
                ...
                <div className="bg-slate-50/80 border border-slate-200/40 rounded-2xl p-3.5 text-xs">
                  <div className="flex justify-between items-center mb-1.5 font-semibold">
                    ...
                    <span
                      className={`font-mono font-bold ${results.floodingPercent > 60 ? "text-red-500" : "text-emerald-600"}`}
                    >
            ...
            <div className="mt-5 grid grid-cols-1 md:grid-cols-3 gap-3 pt-5 border-t border-slate-100 text-[11px] text-slate-500 font-medium">
              <div className="bg-slate-50 p-3 rounded-2xl border border-slate-200/50">
                ...
                <strong className="text-slate-800 text-sm block mt-0.5">
                  {soilType === SoilType.LOAM && width === 10 ? "75%" : "95%"}
                </strong>
                <p className="text-[9.5px] text-slate-450 mt-1 leading-normal">
                  Зависит от слоёв грунтов по отчету бурения ИГИ под конкретный
                  СНиП.
                </p>
              </div>
              ...
              <div className="bg-slate-50 p-3 rounded-2xl border border-slate-200/50">
                <span className="text-slate-400 block text-[9px] uppercase font-bold tracking-wider">
                  🛡️ Модельная надёжность:
                </span>
                <strong
                  className={`text-sm block mt-0.5 ${selectedOption.reliabilityScore >= 90 ? "text-emerald-600" : "text-indigo-600"}`}
                >
                  {selectedOption.reliabilityScore}%
                </strong>
                <p className="text-[9.5px] text-slate-450 mt-1 leading-normal">
                  Применяется внутренний расчетный коэффициент запаса прочности{" "}
                  {safetyFactor}x.
                </p>
              </div>
            ...
            <div className="mt-5 p-4 bg-slate-50 border border-slate-200 rounded-2xl text-xs">
              <span className="text-[10px] text-slate-500 font-extrabold uppercase tracking-widest block mb-2.5">
                🔮 Сценарии стоимости после ИГИ
              </span>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div className="p-3 bg-white rounded-xl border border-slate-200 shadow-xs">
                  <span className="text-emerald-600 font-bold flex items-center gap-1 text-[11px]">
                    <TrendingDown className="w-3.5 h-3.5" /> Оптимистичный
                  </span>
                  <span className="font-mono text-slate-800 font-bold text-sm mt-1 block">
                    {(selectedOption.costMDL * 0.9).toLocaleString()} MDL
                  </span>
                  <span className="text-[9px] text-slate-450 mt-1 block leading-normal">
                    Геология покажет плотные суглинки, что позволит сузить
                    ширину подошвы на 15%.
                  </span>
                </div>
                ...
                <div className="p-3 bg-white rounded-xl border border-blue-200 shadow-sm">
                  ...
                  <span className="text-[9px] text-slate-450 mt-1 block leading-normal">
                    Характеристики грунта в точности совпадут с табличными
                    данными нормативного СНиП.
                  </span>
                </div>

                <div className="p-3 bg-white rounded-xl border border-slate-200 shadow-xs">
                  <span className="text-rose-500 font-bold flex items-center gap-1 text-[11px]">
                    <TrendingUp className="w-3.5 h-3.5" /> Неблагоприятный
                  </span>
                  <span className="font-mono text-slate-800 font-bold text-sm mt-1 block">
                    {(selectedOption.costMDL * 1.15).toLocaleString()} MDL
                  </span>
                  <span className="text-[9px] text-slate-450 mt-1 block leading-normal">
                    Изыскания обнаружат плывун или пластичный лёсс. Понадобится
                    замена грунтов.
                  </span>
                </div>
              </div>
          ...
          <div
            id="advanced-decision-support"
            className="bg-white/95 backdrop-blur-md border border-slate-200/60 rounded-[24px] p-6 shadow-sm shadow-slate-200/40 relative overflow-hidden animate-fade-in space-y-6"
          >
            ...
            <div className="pb-4 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-3">
              <div>
                <span className="text-[10px] font-bold text-emerald-600 uppercase tracking-widest block mb-0.5">
                  NCM F.02.02 / NCM EN 1997 / ISO 31000 ERP
                </span>
                <h2 className="text-base font-display font-black text-slate-800 uppercase tracking-tight flex items-center gap-2">
                  🧠 Экспертная система обеспечения качества и принятия решений
                  (QA 2.0)
                </h2>
                <p className="text-[10px] text-slate-400 mt-0.5 font-sans">
                  Автоматизированный анализ строительных рисков, технологических
                  регламентов и геотехнического соответствия
                </p>
              </div>
            ...
            {activeEngTab === "dashboard" && results && (
              <DashboardPro 
                input={currentInput}
                results={results}
                selectedOption={selectedOption}
                setWallMaterial={setWallMaterial}
                setSlabMaterial={setSlabMaterial}
                setRoofType={setRoofType}
                setFacadeTech={setFacadeTech}
                setGlazingType={setGlazingType}
                setWindowSystem={setWindowSystem}
                setHvacSystem={setHvacSystem}
                setVentSystem={setVentSystem}
                setWaterSystem={setWaterSystem}
                setSewageSystem={setSewageSystem}
                setElecSystem={setElecSystem}
                setLowVoltSystem={setLowVoltSystem}
                setFinishSystem={setFinishSystem}
              />
            )}
            ...
            {activeEngTab === "reinforcement" &&
              (() => {
                ...
                const reinforcementWork = selectedOption.bimEntities?.find(
                  (e: any) => e.id === "REINFORCEMENT_WORKS",
                ) || { qualityChecks: [] };
                ...
                const getSourceBadge = (source: string) => {
                  if (source === "USER") {
                    return (
                      <span className="bg-amber-100 text-amber-800 text-[8.5px] font-extrabold px-1.5 py-0.5 rounded border border-amber-200">
                        РУЧНОЙ ВВОД (USER)
                      </span>
                    );
                  }
                  if (source === "DEFAULT") {
                    return (
                      <span className="bg-emerald-100 text-emerald-800 text-[8.5px] font-extrabold px-1.5 py-0.5 rounded border border-emerald-200">
                        ПО УМОЛЧАНИЮ (DEFAULT)
                      </span>
                    );
                  }
                  return (
                    <span className="bg-blue-100 text-blue-800 text-[8.5px] font-extrabold px-1.5 py-0.5 rounded border border-blue-200">
                      РАСЧЕТ ВЕСА (AUTO)
                    </span>
                  );
                };
                ...
                return (
                  <div className="space-y-6 animate-fade-in text-slate-800">
                    <div className="p-4 bg-blue-50 border border-blue-100 rounded-xl flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
                      <div>
                        <strong className="text-xs text-blue-900 block font-display tracking-tight uppercase">
                          🏗️ Проектирование и заклиночное армирование
                        </strong>
                        <span className="text-[10px] text-blue-700 leading-normal block mt-0.5">
                          Настройте конфигурации стальных каркасов для
                          фундамента <strong>{selectedOption.type}</strong>. Все
                          изменения автоматически пересчитывают объемы стали,
                          стоимость работ и влияют на надежность конструкции в
                          КЭМ-модели.
                        </span>
                      </div>
                      <button
                      ...
                      <div className="lg:col-span-7 bg-slate-50/50 border border-slate-200 rounded-[20px] p-5 space-y-4">
                        ...
                        <div className="bg-slate-100 p-3 rounded-xl border border-slate-200">
                          <span className="block text-[9.5px] font-black text-slate-500 uppercase tracking-wider mb-2">
                            Режим построения каркаса и армирования / Mod
                            selectare schemă armare
                          </span>
                            ...
                            {[
                              {
                                key: "AUTO",
                                name: "🤖 ИИ Авторасчет",
                                desc: "Строгий расчет по NCM & EC2",
                              },
                              {
                                key: "RECOMMENDED",
                                name: "📋 Наш стандарт",
                                desc: "Типовые надежные схемы РМ",
                              },
                              {
                                key: "USER",
                                name: "🛠️ Свой выбор",
                                desc: "Разблокировать редактирование",
                              },
                            ].map((mode) => (
                              <button
                                key={mode.key}
                                type="button"
                                onClick={() =>
                                  setReinforcementChoice(mode.key as any)
                                }
                              ...
                              >
                                ...
                                <div
                                  className={`text-[8px] leading-tight mt-0.5 ${reinforcementChoice === mode.key ? "text-slate-300" : "text-slate-400"}`}
                                >
                                  {mode.desc}
                                </div>
                              </button>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          ...
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) =>
                                setRebarClassMainOverride(e.target.value)
                              }
                            >
                              <option value="">
                                Автоматически ({rData.rebar_class_main})
                              </option>
                              <option value="A500C">
                                A500C (Горячекатаная периодическая)
                              </option>
                              <option value="A400">
                                A400 (Классическая рифленая)
                              </option>
                              <option value="A300">A300 (Устаревшая)</option>
                              <option value="COMPOSITE">
                                Композитная полимерная (АСП)
                              </option>
                            </select>
                            <div className="mt-1 flex items-center justify-between">
                              <span className="text-[9px] text-slate-450">
                                Используется для продольных поясов
                              </span>
                          ...
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) =>
                                setRebarClassSecondaryOverride(e.target.value)
                              }
                            >
                              <option value="">
                                Автоматически ({rData.rebar_class_secondary})
                              </option>
                              <option value="A240">
                                A240 (Гладкая монтажная)
                              </option>
                              <option value="A500C">
                                A500C (Периодическая жесткая)
                              </option>
                              <option value="COMPOSITE">
                                Композитная гладкая
                              </option>
                            ...
                            <div className="mt-1 flex items-center justify-between">
                              <span className="text-[9px] text-slate-450">
                                Используется для хомутов и сеток
                              </span>
                              {getSourceBadge(
                                rData.sources.rebar_class_secondary,
                              )}
                            </div>
                          </div>
                          ...
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) =>
                                setMainBarDiameterOverride(
                                  parseInt(e.target.value) || 0,
                                )
                              }
                            >
                              <option value="">
                                Автоматически ({rData.main_bar_diameter} мм)
                              </option>
                              <option value="8">d8 мм</option>
                              <option value="10">d10 мм</option>
                              <option value="12">
                                d12 мм (Стандартный минимум)
                              </option>
                              <option value="14">
                                d14 мм (Усиленная под кирпич)
                              </option>
                          ...
                          <div>
                            ...
                            <select
                              ...
                              onChange={(e) =>
                                setSecondaryBarDiameterOverride(
                                  parseInt(e.target.value) || 0,
                                )
                              }
                            >
                              <option value="">
                                Автоматически ({rData.secondary_bar_diameter}{" "}
                                мм)
                              </option>
                              <option value="6">d6 мм (Тонкая)</option>
                              <option value="8">d8 мм (Рекомендуемая)</option>
                              <option value="10">
                                d10 мм (Жесткая под высокие балки)
                              </option>
                              <option value="12">d12 мм</option>
                            </select>
                            <div className="mt-1 flex items-center justify-between">
                              <span className="text-[9px] text-slate-450">
                                Хомуты, связывающие каркас
                              </span>
                              {getSourceBadge(
                                rData.sources.secondary_bar_diameter,
                              )}
                            </div>
                          </div>
                          ...
                          <div>
                            ...
                            <div className="relative">
                              <input
                                ...
                                onChange={(e) =>
                                  setLongitudinalBarsCountOverride(
                                    parseInt(e.target.value) || 0,
                                  )
                                }
                              />
                            ...
                            <div className="mt-1 flex items-center justify-between">
                              <span className="text-[9px] text-slate-450">
                                Для ленты (мин. 4 стержня по СНиП)
                              </span>
                              {getSourceBadge(
                                rData.sources.longitudinal_bars_count,
                              )}
                            </div>
                          </div>
                          ...
                          <div>
                            ...
                            <div className="relative">
                              <input
                                ...
                                onChange={(e) =>
                                  setProtectiveLayerBottomOverride(
                                    parseInt(e.target.value) || 0,
                                  )
                                }
                              />
                            ...
                            <div className="mt-1 flex items-center justify-between">
                              <span className="text-[9px] text-slate-450">
                                Мин. 40мм без тощей подготовки
                              </span>
                              {getSourceBadge(
                                rData.sources.protective_layer_bottom,
                              )}
                            </div>
                          </div>
                          ...
                          <div>
                            ...
                            <div className="relative">
                              <input
                                ...
                                onChange={(e) =>
                                  setProtectiveLayerSideOverride(
                                    parseInt(e.target.value) || 0,
                                  )
                                }
                              />
                            ...
                            <div className="mt-1 flex items-center justify-between">
                              <span className="text-[9px] text-slate-450">
                                Мин. 35-40мм для защиты от влаги
                              </span>
                              {getSourceBadge(
                                rData.sources.protective_layer_side,
                              )}
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="pt-3 border-t border-slate-200/60 flex items-center justify-between">
                          <div>
                            <span className="text-xs font-bold text-slate-700 block">
                              Угловое Г- и П-образное анкерование
                            </span>
                            <span className="text-[9.5px] text-slate-450 block font-medium">
                              Специальные угловые замки вместо вязки нахлестом
                              по СП 50-101
                            </span>
                          </div>
                          <div className="flex bg-slate-205 p-0.5 rounded-lg select-none font-sans font-black text-[9px] gap-1">
                            <button
                              ...
                              disabled={reinforcementChoice !== "USER"}
                              onClick={() =>
                                setCornerReinforcementOverride(true)
                              }
                            ...
                            <button
                              ...
                              disabled={reinforcementChoice !== "USER"}
                              onClick={() =>
                                setCornerReinforcementOverride(false)
                              }
                      ...
                      <div className="lg:col-span-5 flex flex-col justify-between bg-white border border-slate-205 rounded-[20px] p-5 shadow-xs">
                          ...
                          <div className="space-y-3 overflow-y-auto max-h-[360px] pr-1.5 scrollbar-thin">
                            ...
                            rData.audit_checks.length > 0
                              ...
                              return (
                                <div
                                  key={cidx}
                                  className="p-3 bg-slate-50/50 border border-slate-200/60 rounded-xl flex items-start gap-2.5"
                                >
                                  <span
                                    className={`px-1.5 py-0.5 rounded text-[8.5px] font-black tracking-wider border shrink-0 ${badgeStyle}`}
                                  >
                                    {statusText}
                                  </span>
                                  <div className="space-y-1">
                                    <div className="font-bold text-slate-850 text-[10.5px] leading-tight">
                                      {check.criterion}
                                    </div>
                            ...
                            {(!rData.audit_checks ||
                              ...
                              rechecks.length === 0 && (
                                <div className="text-center py-8 text-slate-450 text-[10px]">
                                  Для данного типа фундамента спецификация
                                  проверок будет сформирована после
                                  инициализации.
                                </div>
                              )}
                          </div>
                        ...
                        <div className="mt-5 pt-4 border-t border-slate-100 bg-slate-50 p-3 rounded-xl space-y-1.5 text-[10px] text-slate-500 font-medium">
                          ...
                          <div className="flex justify-between">
                            ...
                            <strong className="text-slate-850 font-mono">
                              {rData.spacers_count} шт
                            </strong>
                          </div>
                          <div className="flex justify-between">
                            <span>Длина нахлестных стыков стержней:</span>
                            <strong className="text-slate-850 font-mono">
                              {rData.lap_length} мм (40-50d)
                            </strong>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="p-5 bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-[20px] space-y-3">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <h4 className="text-xs font-black uppercase text-amber-900 tracking-wider">
                              Сейсмически-конструктивный модуль РМ
                            </h4>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                          <div className="bg-white/80 p-2.5 rounded-xl border border-amber-200 text-center">
                            ...
                            <span className="font-mono text-xs font-black text-amber-955">
                              {rData.seismic_zone || "7 POINTS"}
                            </span>
                          </div>
                          <div className="bg-white/80 p-2.5 rounded-xl border border-amber-200 text-center">
                            <span className="text-[8px] uppercase font-bold text-slate-500 block mb-0.5">
                          ...
                          <div className="bg-white/80 p-2.5 rounded-xl border border-amber-200 text-center">
                            ...
                            <span className="font-mono text-xs font-black text-amber-955">
                              ×{rData.seismic_factor || 1.15}
                            </span>
                          </div>
                        </div>
                        <p className="text-[10px] text-amber-850 leading-snug select-text">
                          Из-за близости сейсмоактивного очага Враца в Молдове,
                          расчетное сопротивление арматурных стыков увязано по
                          нормам сейсмической безопасности. Продольные
                          арматурные пояса снабжены Г- и П-образными
                          дополнительными анкерами на всех стыках и углах
                          застройки. Сейсмический коэффициент увеличения
                          составляет{" "}
                          {Math.round(
                            ((rData.seismic_factor || 1) - 1.0) * 100,
                          )}
                          %.
                        </p>
                      </div>
                        ...
                        <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                          <div className="flex items-center gap-2">
                            ...
                            <h4 className="text-[10px] font-black uppercase tracking-wider text-slate-400">
                              Спецификация укладки / Schemă așezare
                            </h4>
                          </div>
                        ...
                        <div className="text-[10px] space-y-2 select-text">
                          <div>
                            <span className="text-slate-400 block text-[9px] uppercase font-bold">
                              Официальный СНиП Чертеж (Русский):
                            </span>
                            <p className="text-slate-200 italic leading-snug bg-slate-920/60 p-2 border border-slate-800 rounded-lg">
                              {rData.layout_scheme_ru ||
                                "Раскладка стандартная"}
                            </p>
                          </div>
                          <div>
                            <span className="text-slate-400 block text-[9px] uppercase font-bold">
                              Desen tehnic oficial (Română):
                            </span>
                            <p className="text-slate-350 italic leading-snug bg-slate-920/60 p-2 border border-slate-800 rounded-lg">
                              {rData.layout_scheme_ro || "- "}
                            </p>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-5">
                      ...
                      <div className="p-5 bg-white border border-slate-205 rounded-[20px] space-y-3 shadow-xs">
                        ...
                        <div className="text-[10.5px] text-slate-600 space-y-2 overflow-y-auto max-h-[300px] pr-1.5 select-text leading-normal">
                          <p className="font-bold text-slate-850">
                            1. Верификация сечений и количества арматуры:
                          </p>
                          <p className="pl-3 border-l border-slate-200">
                            Проверьте количество продольной арматуры по
                            спецификации (должно быть ровно{" "}
                            <strong>
                              {rData.longitudinal_bars_count || 4} шт.
                            </strong>{" "}
                            класса S500/A500C диаметром{" "}
                            <strong>d{rData.main_bar_diameter} мм</strong>).
                            Убедитесь, что строители не произвели самовольную
                            замену диаметров в целях сомнительной экономии.
                          </p>

                          <p className="font-bold text-slate-850">
                            2. Дистанция хомутов и угловые зоны:
                          </p>
                          <p className="pl-3 border-l border-slate-200">
                            Шаг поперечных хомутов в средних зонах каркаса
                            должен составлять ровно{" "}
                            <strong>{rData.stirrup_spacing} мм</strong>. На
                            приопорных угловых участках (в пределах 1 метра от
                            сопряжения) шаг хомутов обязательно уменьшается в
                            два раза — пересчитайте количество лично рулеткой!
                          </p>

                          <p className="font-bold text-slate-850">
                            3. Проверка защитных слоев бетона:
                          </p>
                          <p className="pl-3 border-l border-slate-200">
                            Стержни ни в коем случае не должны соприкасаться с
                            опалубкой или лежать на грунте. Каркас должен
                            укладываться на специализированные пластиковые
                            фиксаторы («стульчики») высотой не менее{" "}
                            <strong>{rData.protective_layer_bottom} мм</strong>{" "}
                            на дне и не менее{" "}
                            <strong>{rData.protective_layer_side} мм</strong> по
                            бокам. Подручные средства (деревянные бруски, куски
                            кирпича) запрещены!
                          </p>

                          <p className="font-bold text-slate-850">
                            4. Проверка поддерживающих столиков (лягушек):
                          </p>
                          <p className="pl-3 border-l border-slate-200">
                            Нахлест продольных плетей по длине должен составлять
                            минимум <strong>{rData.lap_length} мм</strong>.
                            Нахлесты разных стержней устраивают «вразбежку» (не
                            допускайте стыковки всех стержней в одном месте).
                            Убедитесь в наличии поддерживающих «лягушек» (не
                            менее <strong>{rData.chairs_count || 0} шт.</strong>
                            ) для обеспечения жесткости верхних сеток.
                          </p>

                          <p className="font-bold text-rose-600 bg-rose-50 p-2.5 rounded-lg text-[9.5px] leading-snug">
                            ⚠️ <strong>Опасный дефект застройщика:</strong>{" "}
                            Сварка арматуры класса А500С в обычных построечных
                            условиях ослабляет металл. Соединения каркасов
                            должны производиться исключительно ручной вязкой
                            стальной вязальной проволокой!
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="p-5 bg-white border border-slate-205 rounded-[20px] space-y-3 shadow-xs">
                        <div className="flex items-center gap-2 border-b border-slate-100 pb-2.5">
                          ...
                          <div>
                            <h4 className="text-xs font-black uppercase text-slate-800 tracking-wider">
                              Чек-лист Технадзора перед заливкой
                            </h4>
                            <span className="text-[9px] text-slate-450 block font-medium uppercase tracking-wider">
                        ...
                        <div className="text-[10.5px] text-slate-650 space-y-2.5 overflow-y-auto max-h-[300px] pr-1.5 select-text leading-tight">
                          {[
                            {
                              title:
                                "[ ] Контроль диаметров и сортности сплава",
                              desc: `Заверить класс стержней А500С ф${rData.main_bar_diameter} мм и укладку хомутовой обвязки ф${rData.secondary_bar_diameter} мм. Контролировать штангенциркулем соответствие проекту.`,
                            },
                            {
                              title:
                                "[ ] Стыковая нахлестка и перепуск стержней",
                              desc: `Промерить перекрытия наращиваемых плетей. Перепуск должен составлять не менее ${rData.lap_length} мм со смещением мест стыков вразбежку.`,
                            },
                            {
                              title:
                                "[ ] Жесткие угловые Г- и П-образные сопряжения со стенками",
                              desc: `Углы застройки усиливаются загибаемыми Г-образными анкерами d${rData.main_bar_diameter} и П-образными монтажными элементами d${rData.secondary_bar_diameter}. Обычный вяз нахлестом по прямой линии на углу СТРОГО ЗАПРЕЩЕН.`,
                            },
                            {
                              title:
                                "[ ] Расстановка пластиковых ограничителей зазоров бетона",
                              desc: `Зафиксировать наличие защитных фиксаторов на дне и боковинах каркаса. Использовано не менее ${rData.spacers_count} защитных опор.`,
                            },
                            {
                              title:
                                "[ ] Выдерживание заложенного шага вязки хомутов",
                              desc: `Шаг обвязки хомутами строго выдержан в размере ${rData.stirrup_spacing} мм вдоль всей оси. В угловых зонах шаг хомутов учащен вдвое.`,
                            },
                            {
                              title:
                                "[ ] Санитарный строительный контроль траншеи / опалубки",
                              desc: "Удалить пыль, грязь, лед, ржавчину со стержней. Убедиться в отсутствии следов масел и древесной щепы на дне.",
                            },
                            {
                              title:
                                "[ ] Выпуск стартовых монолитных анкеров-выпусков",
                              desc: `Спроектировать и установить выпуски связи под колонны или несущие надфундаментные стены в соответствии с Eurocode 2.`,
                            },
                            {
                              title:
                                "[ ] Составление фотопротокола и оформление Актов скрытых работ",
                              desc: "Произвести комплексную фотосъемку уложенных каркасов и подписать Акт освидетельствования скрытых работ до начала приёма бетонной смеси.",
                            },
                          ].map((item, idx) => (
                            <div
                              key={idx}
                              className="pb-2 border-b border-slate-100 last:border-b-0"
                            >
                              <span className="font-extrabold text-slate-800 text-[10px] block mb-0.5">
                                {item.title}
                              </span>
              ...
              <div className="space-y-4 animate-fade-in">
                <div className="p-3.5 bg-slate-50 border border-slate-250/50 rounded-xl">
                  <div className="flex items-center gap-1.5 font-bold text-slate-700 text-xs mb-1">
                    ...
                    <span>
                      Карта Рисков Строительной Площадки (ISO 31000 ERP Ledger)
                    </span>
                  </div>
                  <p className="text-[10px] text-slate-500 leading-snug">
                    Все геоморфологические, сейсмические и гидрологические риски
                    рассчитаны в режиме реального времени на основании
                    геологического профиля застройки и геометрии фундаментного
                    контура.
                  </p>
                </div>
                  ...
                  <table className="w-full text-left text-xs border-collapse min-w-[700px] font-sans">
                    <thead>
                      <tr className="bg-slate-50 border-b border-slate-200/60 text-slate-500 font-bold uppercase tracking-wider text-[9px]">
                        ...
                        <th className="py-2 px-3">
                          Проектные Защитные Меры (Mitigation)
                        </th>
                      ...
                      {results.risksList.map((risk) => (
                        <tr key={risk.Risk_ID} className="hover:bg-slate-50/50">
                          <td className="py-2.5 px-3 font-mono font-bold text-slate-800">
                            ...
                            <div className="text-[8.5px] text-slate-450 font-normal">
                              {risk.WBS_Code}
                            </div>
                          </td>
                          ...
                          <td className="py-2.5 px-3 text-center font-mono text-slate-650">
                            {risk.probability}/5
                          </td>
                          <td className="py-2.5 px-3 text-center font-mono text-slate-650">
                            {risk.impact}/5
                            ...
                            <span
                              className={`px-2 py-0.5 rounded-full text-[9px] font-black font-mono ${
                                risk.severity === "CRITICAL"
                                  ...
                                  : risk.severity === "HIGH"
                                    ? "bg-orange-100 text-orange-850 border border-orange-250"
                          ...
                          <td className="py-2.5 px-3 text-right font-mono font-bold text-slate-700">
                            {risk.mitigation_cost_mdl > 0
                              ? `+${risk.mitigation_cost_mdl.toLocaleString()}`
                              : "—"}
                          </td>
                        </tr>
                      ))}
              ...
              <div className="space-y-4 animate-fade-in">
                <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-0.5">
                      Механика Грунтов и Слои основания
                    </span>
                    <strong className="text-slate-800 text-xs block font-display leading-tight uppercase">
                      Геологическая развертка под профилем фундамента (γ_s ={" "}
                      {results.geology.soil_heterogeneity_factor})
                    </strong>
                    <div className="grid grid-cols-2 gap-3 mt-3 text-[10.5px] font-medium text-slate-550 leading-loose">
                      <div>
                        • Несущий грунт:{" "}
                        <strong className="text-slate-800">
                          {results.geology.soil_type}
                        </strong>
                      </div>
                      <div>
                        • Расч. сопротивление:{" "}
                        <strong className="text-slate-850">
                          {results.geology.design_soil_resistance} кПа
                        </strong>
                      </div>
                      <div>
                        • Деформация E_avg:{" "}
                        <strong className="text-slate-850">
                          {results.geology.deformation_modulus} МПа
                        </strong>
                      </div>
                      <div>
                        • Степень пучения:{" "}
                      ...
                      <div>
                        • Скважин (ИГИ РМ):{" "}
                        <strong className="text-slate-850">
                          {results.geology.number_of_boreholes} шт (Н=
                          {results.geology.borehole_depth}м)
                        </strong>
                      </div>
                      <div>
                        • Коэф. запаса γ_n:{" "}
                        <strong className="text-slate-850">
                          {results.geology.safety_factor}
                        </strong>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-white border border-slate-150 rounded-xl p-3 space-y-2">
                    <span className="text-[9.5px] font-bold text-emerald-600 block uppercase">
                      Поперечный разрез выработки ИГИ:
                    </span>
                    <div className="space-y-1.5 font-mono text-[10px]">
                      {results.geology.soil_layers.map((layer, idx) => (
                        <div
                          key={idx}
                          className="bg-slate-50 border-l-4 border-emerald-500 p-1.5 rounded-r"
                        >
                          <div className="flex justify-between items-center font-bold">
                            ...
                            <span className="text-slate-500 text-[9px] ml-auto">
                              Th={layer.thickness}м
                            </span>
                          </div>
                          <p className="text-[8.5px] text-slate-450 mt-0.5 truncate leading-tight">
                            {layer.description}
                          </p>
                        </div>
                      ))}
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  {results.geology.suitability_matrix.map((matrix) => {
                    const rank = results.geology.recommendation_ranking.find(
                      (r) => r.id === matrix.id,
                    );
                    return (
                      <div
                        key={matrix.id}
                        className="p-3 bg-white border border-slate-205 rounded-xl flex flex-col justify-between shadow-xs"
                      >
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            <span className="text-slate-400 text-[8px] block font-semibold uppercase">
                              {matrix.id} Вариант
                            </span>
                          ...
                          <span
                            className={`px-2 py-0.5 rounded-full text-[9px] font-mono font-black ${
                              matrix.suitability === "Высокое соответствие"
                                ? "bg-emerald-50 text-emerald-700"
                                : matrix.suitability === "Удовлетворительно"
                        ...
                        <ul className="text-[10px] text-slate-550 space-y-1 my-2 list-none font-medium">
                          <li>
                            • К-во заложений:{" "}
                            <strong>
                              h = {results.input.hasBasement ? "1.9м" : "1.2м"}
                            </strong>
                          </li>
                          <li>
                            • Риск усадки:{" "}
                            <strong>{matrix.settlementRiskCoeff}x</strong>
                          </li>
                          <li>
                            • Прок. сейсмика:{" "}
                            <strong>
                              {results.input.region === "SOUTH"
                                ? "Класс С (Особый)"
                                : "Класс В (Стандарт)"}
                            </strong>
                          </li>
                        ...
                        <div className="border-t border-slate-100 pt-2 flex items-center justify-between mt-1">
                          <span className="text-[9px] text-slate-400 font-mono">
                            Rank: #{rank?.rank} ({rank?.relativeEfficiencyScore}
                            %)
                          </span>
                          <span className="text-blue-600 font-extrabold text-[10px]">
                            {rank?.suitabilityNote}
                          </span>
                        </div>
                      </div>
                    );
              ...
              <div className="space-y-4 animate-fade-in">
                <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl">
                  <div className="flex justify-between items-center text-xs font-bold text-slate-700">
                    <span className="flex items-center gap-1.5">
                      ...
                      Инженерные коммуникации здания (Спецификация раздела
                      НВК/ТСО)
                    </span>
                    <span className="font-mono text-blue-600 font-extrabold bg-blue-50 border border-blue-100 px-2.5 py-0.5 rounded-full text-[10.5px]">
                      Общая стоимость сетей:{" "}
                      {results.utilities.totalCostMDL.toLocaleString()} MDL
                    </span>
                  </div>
                  <p className="text-[10px] text-slate-500 leading-snug mt-1">
                    Проектирование вводов питьевой воды, выпусков канализации,
                    систем электроснабжения и слаботочных сетей через толщу
                    подошвы фундамента по нормам СНиП РМ.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-3.5">
                  {[
                    {
                      key: "sewer",
                      title: "Канализация лотковая d110",
                      label: "SEWER",
                      icon: Waves,
                    },
                    {
                      key: "water",
                      title: "Ввод воды ПНД d32 PN16",
                      label: "WATER",
                      icon: Droplet,
                    },
                    {
                      key: "power",
                      title: "Сила вводная ПНД d50",
                      label: "POWER",
                      icon: Activity,
                    },
                    {
                      key: "lowCurrent",
                      title: "Слаботочные сети ПНД d25",
                      label: "LOW_CURRENT",
                      icon: Compass,
                    },
                    {
                      key: "spareSleeves",
                      title: "Резервные гильзы d160",
                      label: "SPARE_SLEEVES",
                      icon: Layers,
                    },
                  ].map((sub) => {
                    ...
                    return (
                      <div
                        key={sub.key}
                        className="bg-white border border-slate-200/90 rounded-2xl p-3 flex flex-col justify-between shadow-xs"
                      >
                        <div>
                          <div className="flex justify-between items-start mb-2 border-b border-slate-100 pb-1.5">
                            <div>
                              <strong className="text-[11px] text-slate-800 block truncate leading-tight">
                                {sub.title}
                              </strong>
                              <span className="text-[8.5px] text-slate-450 block font-bold font-mono tracking-wider">
                                {sub.label}
                              </span>
                            </div>
                          ...
                          <div className="space-y-1.5 my-2">
                            <span className="text-[8.5px] font-extrabold text-slate-400 block uppercase tracking-wider">
                              📦 Закладные материалы:
                            </span>
                            <div className="space-y-1 text-[9.5px] leading-tight">
                              {model.materials.map((mat: any, idx: number) => (
                                <div
                                  key={idx}
                                  className="bg-slate-50 p-1.5 rounded text-slate-650"
                                >
                                  <div className="font-semibold text-slate-850 truncate">
                                    {mat.name}
                                  </div>
                                  <div className="flex justify-between mt-0.5 text-slate-500 text-[8.5px]">
                                    <span>
                                      Кол-во: {mat.qty} {mat.unit}
                                    </span>
                            ...
                            <span className="text-[8.5px] font-extrabold text-slate-400 block uppercase tracking-wider mt-2.5">
                              📊 Рассчитанные объемы выпуска:
                            </span>
                            <div className="text-[9px] font-mono text-slate-550 space-y-0.5 bg-slate-50/50 p-1.5 rounded">
                              {Object.entries(model.volumes).map(
                                ([vk, vv]: any) => (
                        ...
                        <div className="border-t border-slate-100 pt-2 flex flex-col gap-1 mt-2.5">
                          <div className="flex justify-between text-xs items-baseline font-bold">
                            ...
                            <span className="text-blue-600 font-mono">
                              {model.costMDL.toLocaleString()} MDL
                            </span>
                          </div>
                          ...
                          {model.qualityChecks.map((qc: any, qidx: number) => (
                            <div
                              key={qidx}
                              className="bg-slate-50 border-l border-blue-500 p-1 rounded mt-1.5 text-[8.5px] text-slate-500 leading-snug"
                            >
                              <div className="font-extrabold text-[#111] truncate">
                                {qc.criterion}
                              </div>
                              <div className="font-mono mt-0.5 shrink-0 text-indigo-700 bg-indigo-50 px-1 py-0.5 rounded truncate">
                                {qc.value}
                              </div>
                              <div className="text-slate-400 text-[7 px] mt-0.5 font-sans leading-tight italic truncate">
                                {qc.norm}
                              </div>
                            </div>
                          ))}
                ...
                <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl">
                  <div className="flex justify-between items-center text-xs font-bold text-slate-700">
                    <span className="flex items-center gap-1.5">
                      ...
                      Специализированные Узлы и Климатические технологии
                      производства
                    </span>
                  </div>
                  <p className="text-[10px] text-slate-500 leading-snug mt-1">
                    Строгое соблюдение физико-технологических стадий гидратации
                    бетона при экстремальных температурах, расчет объемов
                    обратной засыпки грунта пазух и планирование изолированной
                    отмостки на XPS барьерах от пучения.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="bg-white border border-slate-205 rounded-xl p-3.5 flex flex-col justify-between shadow-xs">
                    <div>
                      <div className="flex justify-between border-b border-slate-100 pb-2 mb-2">
                        <div>
                          <strong className="text-xs text-slate-800 block uppercase">
                            💧 Прогрев и Уход Б2.5
                          </strong>
                          <span className="text-[8.5px] text-slate-450 block font-bold font-mono tracking-widest">
                            WBS SECTION CONCRETE_CURING
                          </span>
                        </div>
                      ...
                      <div className="text-[9.5px] leading-relaxed text-slate-600 space-y-2 mt-2 font-sans font-medium">
                        <div className="bg-slate-50 p-2 rounded leading-snug">
                          <div className="flex justify-between items-center text-[10px] text-slate-700 font-bold border-b border-slate-200/50 pb-1 mb-1">
                            ...
                            <span className="font-mono text-blue-600">
                              {results.concreteCuring.airTemperatureC}°C
                            </span>
                          </div>
                          Метод созревания:{" "}
                          <strong className="text-slate-800">
                        ...
                        <div className="space-y-1">
                          <span className="text-[8.5px] font-extrabold text-slate-400 block uppercase">
                            Закупаемый инвентарь:
                          </span>
                          {results.concreteCuring.materialsNeeded.map(
                            (mat: any, idx: number) => (
                              <div
                                key={idx}
                                className="flex justify-between text-slate-650 bg-slate-50/50 p-1 rounded font-mono text-[9px]"
                              >
                        ...
                        <div className="space-y-1 mt-2 bg-slate-50/50 p-2 rounded">
                          <span className="text-[8.5px] font-extrabold text-slate-400 block uppercase">
                            Требования инспекции (QC):
                          </span>
                          {results.concreteCuring.qcRequirements.map(
                            (qc: any, idx: number) => (
                              <div
                                key={idx}
                                className="flex items-start gap-1 text-[8.5px] leading-snug text-slate-500"
                              >
                                <span className="text-green-600 shrink-0 mt-0.5">
                                  ✔
                                </span>
                                <span>
                                  <strong>{qc.criterion}:</strong> {qc.norm}
                                </span>
                              </div>
                            ),
                          )}
                    ...
                    <div className="border-t border-slate-100 pt-2.5 flex justify-between items-baseline font-bold text-xs mt-3">
                      ...
                      <span className="text-blue-600 font-mono">
                        +{results.concreteCuring.totalCostMDL.toLocaleString()}{" "}
                        MDL
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="bg-white border border-slate-205 rounded-xl p-3.5 flex flex-col justify-between shadow-xs">
                    <div>
                      <div className="flex justify-between border-b border-slate-100 pb-2 mb-2">
                        <div>
                          ...
                          <span className="text-[8.5px] text-slate-450 block font-bold font-mono tracking-widest">
                            WBS SECTION BACKFILL_COMPACTION
                          </span>
                        </div>
                      ...
                      <div className="text-[9.5px] leading-relaxed text-slate-600 space-y-2 mt-2 font-sans font-medium">
                        <div className="bg-slate-50 p-2 rounded leading-snug space-y-1 font-mono text-[9px]">
                          <div>
                            Извлечено грунта:{" "}
                            <strong>{results.backfill.excavationM3} м³</strong>
                          </div>
                          <div>
                            Вытеснение бетоном:{" "}
                            <strong>
                              {results.backfill.concreteDisplacementM3} м³
                            </strong>
                          </div>
                          <div>
                            Чистый объем пазух:{" "}
                            <strong>
                              {results.backfill.netBackfillVolumeM3} м³
                            </strong>
                          </div>
                          <div>
                            Фактор разрыхления K_p:{" "}
                            <strong>{results.backfill.soilSwellFactor}x</strong>
                          </div>
                          <div>
                            Плотность K_com:{" "}
                            <strong>
                              {results.backfill.densityRequiredT_M3} т/м³
                            </strong>
                          </div>
                        </div>
                        ...
                        <div className="space-y-1">
                          <span className="text-[8.5px] font-extrabold text-slate-400 block uppercase">
                            Материалы и Трамбовка:
                          </span>
                          {results.backfill.materialsNeeded.map(
                            (mat: any, idx: number) => (
                              <div
                                key={idx}
                                className="flex justify-between text-slate-650 bg-slate-50/50 p-1 rounded font-mono text-[9px]"
                        ...
                        <div className="mt-2 text-[8.5px] text-slate-500 leading-snug border border-slate-100 bg-slate-50/50 p-2 rounded">
                          📌 <strong>Методика уплотнения:</strong> Послойная
                          отсыпка ПГС мелкими картами по 200 мм. Проливка водой
                          до достижения оптимума влажности{" "}
                          <strong>
                            {results.backfill.optimalMoisturePercent}%
                          </strong>
                          . Минимум{" "}
                          <strong>{results.backfill.compactionPasses}</strong>{" "}
                          холостых проходов тяжелой виброплитой весом не менее
                          120 кг.
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="border-t border-slate-100 pt-2.5 flex justify-between items-baseline font-bold text-xs mt-3">
                      ...
                      <span className="text-blue-600 font-mono">
                        +{results.backfill.totalCostMDL.toLocaleString()} MDL
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="bg-white border border-slate-205 rounded-xl p-3.5 flex flex-col justify-between shadow-xs">
                    <div>
                      <div className="flex justify-between border-b border-slate-100 pb-2 mb-2">
                        <div>
                          <strong className="text-xs text-slate-800 block uppercase">
                            ☔ Защитная Отмостка с XPS
                          </strong>
                          <span className="text-[8.5px] text-slate-450 block font-bold font-mono tracking-widest">
                            WBS SECTION BLIND_AREA
                          </span>
                        </div>
                      ...
                      <div className="text-[9.5px] leading-relaxed text-slate-600 space-y-2 mt-2 font-sans font-medium">
                        <div className="bg-slate-50 p-2 rounded leading-snug space-y-1 font-mono text-[9px]">
                          <div>
                            Периметр здания:{" "}
                            <strong>{results.blindArea.perimeterM} м.п.</strong>
                          </div>
                          <div>
                            Принятая ширина отмостки:{" "}
                            <strong>
                              {results.blindArea.blindAreaWidthM} м
                            </strong>
                          </div>
                          <div>
                            Выемка основания:{" "}
                            <strong>
                              {results.blindArea.excavationVolumeM3} м³
                            </strong>
                          </div>
                          <div>
                            Контурное утепление XPS:{" "}
                            <strong>
                              {results.blindArea.insulationXpsM3} м³
                            </strong>
                          </div>
                          <div>
                            Подсыпка гравием:{" "}
                            <strong>{results.blindArea.gravelBaseM3} м³</strong>
                          </div>
                          <div>
                            Бетонирование отмостки:{" "}
                            <strong>
                              {results.blindArea.concreteC20_25M3} м³
                            </strong>
                          </div>
                          <div>
                            Сетка армирования:{" "}
                            <strong>
                              {results.blindArea.reinforcingMeshKg} кг
                            </strong>
                          </div>
                        </div>
                        ...
                        <div className="space-y-1">
                          ...
                          {results.blindArea.materialsNeeded.map(
                            (mat: any, idx: number) => (
                              <div
                                key={idx}
                                className="flex justify-between text-slate-650 bg-slate-50/50 p-1 rounded font-mono text-[9px]"
                        ...
                        <div className="mt-2 text-[8.5px] text-slate-500 leading-snug border border-slate-100 bg-emerald-50/20 p-2 rounded">
                          📌 <strong>Технологический узел:</strong> Покрытие
                          отмостки армируется сеткой d8 100х100 в верхнем слое и
                          утепляется контурной XPS плитой толщиной 50-80 мм для
                          увода фронта теплообмена и недопущения вспучивания
                          примыкающего суглинка у цоколя.
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="border-t border-slate-100 pt-2.5 flex justify-between items-baseline font-bold text-xs mt-3">
                      ...
                      <span className="text-blue-600 font-mono">
                        +{results.blindArea.totalCostMDL.toLocaleString()} MDL
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-4 animate-fade-in">
                <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-xl">
                  <div className="flex items-center gap-1.5 font-bold text-slate-700 text-xs mb-1">
                    ...
                    <span>
                      Прозрачное проектное обоснование AI-DSS (Explainable
                      Engineering)
                    </span>
                  </div>
                  <p className="text-[10px] text-slate-500 leading-snug">
                    Система наглядно объясняет научные мотивы каждого принятого
                    решения, устраняя "черный ящик" ИИ. Все расчеты ссылаются на
                    климатические и геотехнические нормативы Республики Молдова.
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {results.explanations.map((exp, idx) => (
                    <div
                      key={idx}
                      className="p-4 bg-white border border-slate-205 rounded-xl flex flex-col justify-between shadow-xs"
                    >
                      <div>
                        <div className="flex justify-between items-center mb-1.5 border-b border-slate-100 pb-1.5">
                          ...
                          <span
                            className={`px-2 py-0.5 rounded text-[9px] font-mono font-extrabold ${
                              exp.isRecommended
                        ...
                        <ul className="text-[10.5px] text-slate-650 space-y-2.5 list-none font-medium leading-relaxed my-3 pl-0.5">
                          {exp.reasons.map((reason, ri) => (
                            <li key={ri} className="flex items-start gap-1.5">
                              <span className="text-blue-500 text-xs mt-0.5 shrink-0">
                                ✔
                              </span>
                              <span>{reason}</span>
                            </li>
          ...
          <div
            id="eurocode-load-combinations"
            className="bg-white/95 backdrop-blur-md border border-slate-200/60 rounded-[24px] p-6 shadow-sm shadow-slate-200/40 relative overflow-hidden animate-fade-in"
          >
            ...
            <div className="pb-4 border-b border-slate-100 flex items-center justify-between">
              <div>
                <span className="text-[10px] font-bold text-indigo-600 uppercase tracking-widest block mb-0.5">
                  NCM EN 1990 / NCM EN 1991 / NCM EN 1998
                </span>
            ...
            <div className="mt-4 p-3 bg-slate-50 border border-slate-150 rounded-xl">
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block mb-1.5">
                📝 Верификация исходных параметров здания:
              </span>
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 text-[11px] text-slate-600 leading-relaxed font-sans font-medium">
                ...
                <div>
                  • Площадь застройки:{" "}
                  <strong>{Math.round(width * length)} м²</strong>
                </div>
                <div>
                  • Этажность СНиП: <strong>{floors} эт.</strong> (
                  {futureFlooringExtension
                    ? "с учетом надстройки +1"
                    : "тип ИЖС"}
                  )
                </div>
                <div>
                  • Высота этажа: <strong>{floorHeight} м</strong>
                </div>
                <div>
                  • Стены:{" "}
                  <strong>{WALL_MATERIAL_DATA[wallMaterial].name}</strong>
                </div>
                <div>
                  • Плиты перекрытий:{" "}
                  <strong>{SLAB_DATA[slabMaterial].name}</strong>
                </div>
                <div>
                  • Снег (кПа):{" "}
                  <strong>{REGION_DATA[region].snowLoad} кПа</strong> ({region})
                </div>
                <div>
                  • Сейсмика:{" "}
                  <strong>{REGION_DATA[region].seismicPoints} баллов</strong> (A
                  = {REGION_DATA[region].seismicCoeff})
                </div>
              </div>
            </div>
              ...
              <table className="w-full text-left text-xs border-collapse min-w-[550px] font-sans">
                <thead>
                  <tr className="bg-slate-50 border-b border-slate-200/60 text-slate-500 font-bold uppercase tracking-wider text-[9px]">
                    ...
                    <th className="py-2.5 px-3 max-w-[200px]">
                      Проектный комментарий (NCM / СН)
                    </th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 text-[11px] text-slate-750">
                  <tr className="hover:bg-slate-50/50">
                    ...
                    <td className="py-2 px-3 text-right font-mono font-bold text-slate-900">
                      {Math.round(results.wallWeightTons * 1.2 * 10) / 10} т
                    </td>
                    <td className="py-2 px-3 text-slate-500 text-[10px] leading-tight">
                      γf = 1.2 применён для деревянных и каменных несущих
                      конструкций дома у застройщика
                    </td>
                  </tr>
                  <tr className="hover:bg-slate-50/50">
                    <td className="py-2 px-3 font-semibold">
                      Междуэтажные перекрытия и пол
                    </td>
                    <td className="py-2 px-3 text-slate-500 text-[10px]">
                      Постоянная
                    </td>
                    ...
                    <td className="py-2 px-3 text-right font-mono font-bold text-slate-900">
                      {Math.round(results.slabWeightTons * 1.24 * 10) / 10} т
                    </td>
                    <td className="py-2 px-3 text-slate-500 text-[10px] leading-tight">
                      γf = 1.24 (средневзвешенный: несущая плита 1.2, тяжелый
                      пирог пола/стяжка 1.3)
                    </td>
                  </tr>
                  <tr className="hover:bg-slate-50/50">
                    <td className="py-2 px-3 font-semibold">
                      Стропила и кровельный пирог
                    </td>
                    <td className="py-2 px-3 text-slate-500 text-[10px]">
                      Постоянная
                    </td>
                    <td className="py-2 px-3 text-right font-mono">
                      {results.roofWeightTons} т
                    </td>
                    <td className="py-2 px-3 text-center font-mono">1.26</td>
                    <td className="py-2 px-3 text-right font-mono font-bold text-slate-900">
                      {Math.round(results.roofWeightTons * 1.26 * 10) / 10} т
                    </td>
                    <td className="py-2 px-3 text-slate-500 text-[10px] leading-tight">
                      γf = 1.26 (средневзвешенный: деревянные фермы/обрешётка
                      1.2, черепица/покрытие 1.3)
                    </td>
                  </tr>
                  <tr className="hover:bg-slate-50/50">
                    <td className="py-2 px-3 font-semibold">
                      Полезная жилая нагрузка
                    </td>
                    <td className="py-2 px-3 text-slate-500 text-[10px]">
                      Временная
                    </td>
                    <td className="py-2 px-3 text-right font-mono">
                      {results.liveLoadTons} т
                    </td>
                    <td className="py-2 px-3 text-center font-mono">1.4</td>
                    <td className="py-2 px-3 text-right font-mono font-bold text-slate-900">
                      {Math.round(results.liveLoadTons * 1.4 * 10) / 10} т
                    </td>
                    <td className="py-2 px-3 text-slate-500 text-[10px] leading-tight">
                      γf = 1.4 применён для полезной нагрузки категории A
                      (бытовые жилые помещения СН)
                    </td>
                  </tr>
                  <tr className="hover:bg-slate-50/50">
                    <td className="py-2 px-3 font-semibold">
                    ...
                    <td className="py-2 px-3 text-right font-mono">
                      {results.snowLoadTons} т
                    </td>
                    <td className="py-2 px-3 text-center font-mono">1.4</td>
                    <td className="py-2 px-3 text-right font-mono font-bold text-slate-900">
                      {Math.round(results.snowLoadTons * 1.4 * 10) / 10} т
                    </td>
                    <td className="py-2 px-3 text-slate-500 text-[10px] leading-tight">
                      γf = 1.4 применён для климатического снежного покрова в
                      Молдове в соответствии с NCM EN
                    </td>
                  </tr>
                  <tr className="hover:bg-slate-50/50">
                    <td className="py-2 px-3 font-semibold">
                      Ветровая боковая нагрузка
                    </td>
                    <td className="py-2 px-3 text-slate-500 text-[10px]">
                      Временная
                    </td>
                    <td className="py-2 px-3 text-right font-mono">
                      {results.windLoadTons} т
                    </td>
                    <td className="py-2 px-3 text-center font-mono">1.4</td>
                    <td className="py-2 px-3 text-right font-mono font-bold text-slate-900">
                      {Math.round(results.windLoadTons * 1.4 * 10) / 10} т
                    </td>
                    <td className="py-2 px-3 text-slate-500 text-[10px] leading-tight">
                      γf = 1.4 применён для ветровой аэродинамической силы на
                      высоту стен по NCM EN 1991-1-4
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-slate-50 border border-slate-150 p-3.5 rounded-xl text-xs space-y-2">
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">
                  🌨️ Вариант А: Снегодоминирующее сочетание
                </span>
                <p className="text-[10px] text-slate-450 leading-relaxed font-sans">
                  Приоритетное накопление мокрого снега на кровле при пониженных
                  коэффициентах нагружения от ветра и пребывания людей (NCM EN
                  1990 Formula 6.10):
                </p>
                <div className="bg-white px-3 py-2 border border-slate-200 rounded-lg text-slate-800 leading-relaxed font-mono text-[10.5px]">
                  <div>
                    E_d,A = G_d + Q_d,snow + 0.6*Q_d,wind + 0.7*Q_d,live
                  </div>
                  <div className="text-blue-600 font-extrabold mt-1 text-[11px]">
                    ={" "}
                    {Math.round(
                      (results.wallWeightTons * 1.2 +
                        results.slabWeightTons * 1.24 +
                        results.roofWeightTons * 1.26) *
                        10,
                    ) / 10}{" "}
                    + {Math.round(results.snowLoadTons * 1.4 * 10) / 10} +{" "}
                    {Math.round(0.6 * results.windLoadTons * 1.4 * 10) / 10} +{" "}
                    {Math.round(0.7 * results.liveLoadTons * 1.4 * 10) / 10}
                    <span className="text-slate-900 font-sans font-normal ml-1">
                      🟰
                    </span>{" "}
                    {results.scenarioASnowDominantTons} тонн
                  </div>
                </div>
              </div>
              ...
              <div className="bg-slate-50 border border-slate-150 p-3.5 rounded-xl text-xs space-y-2">
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">
                  🛋️ Вариант B: Полезнодоминирующее сочетание
                </span>
                <p className="text-[10px] text-slate-450 leading-relaxed font-sans">
                  Максимальное динамическое заселение людьми на этажах при
                  сниженной толщине снежного покрова и боковом ветровом
                  давлении:
                </p>
                <div className="bg-white px-3 py-2 border border-slate-200 rounded-lg text-slate-800 leading-relaxed font-mono text-[10.5px]">
                  <div>
                    E_d,B = G_d + Q_d,live + 0.5*Q_d,snow + 0.6*Q_d,wind
                  </div>
                  <div className="text-emerald-600 font-extrabold mt-1 text-[11px]">
                    ...
                    + {Math.round(results.liveLoadTons * 1.4 * 10) / 10} +{" "}
                    {Math.round(0.5 * results.snowLoadTons * 1.4 * 10) / 10} +{" "}
                    {Math.round(0.6 * results.windLoadTons * 1.4 * 10) / 10}
                    <span className="text-slate-900 font-sans font-normal ml-1">
                      🟰
                    </span>{" "}
                    {results.scenarioBLiveDominantTons} тонн
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 text-xs">
                <div>
                  ...
                  <p className="text-[11px] text-indigo-700 font-medium mt-1 leading-normal max-w-xl font-sans">
                    Для гарантированного расчета сечений выбран максимум
                    ультимативных сил:{" "}
                    <strong className="text-slate-900 font-bold">
                      {results.totalFactoredWeightTons} т
                    </strong>
                    . С высокой точностью мы пересчитали несущую способность
                    опоры.
                  </p>
                </div>
                <div className="bg-white px-4 py-2 border border-indigo-100 rounded-xl leading-tight text-right shrink-0">
                  <span className="text-[9.5px] font-bold text-slate-400 block uppercase font-mono">
                    ULS (ПС1) Нагрузка:
                  </span>
                  <span className="font-mono text-indigo-700 font-black text-lg">
                    {results.totalFactoredWeightTons} т
                  </span>
                </div>
              </div>
              ...
              <div className="mt-3 pt-3 border-t border-indigo-100 flex flex-col md:flex-row md:items-center justify-between gap-3 text-xs w-full">
                <div className="leading-snug">
                  <span className="text-emerald-800 font-black flex items-center gap-1">
                    ⚡ Квазипостоянная сейсмическая масса по NCM EN 1998 /
                    Eurocode 8:
                  </span>
                  <span className="text-[10.5px] text-emerald-700 block mt-0.5 font-sans">
                    Жилая полезная нагрузка при землетрясении берется с
                    коэффициентом ψ₂ = 0.3, климатический снег и ветер отброшены
                    (ψ₂ = 0) с учетом молдавских норм.
                  </span>
                  <span className="text-[10px] text-emerald-600 block font-mono mt-1 leading-normal">
                    Масса при расчете: Σ G_k + 0.3 * Q_k,live ={" "}
                    {results.wallWeightTons} + {results.slabWeightTons} +{" "}
                    {results.roofWeightTons} + 0.3 * {results.liveLoadTons} ={" "}
                    {results.seismicMassCombinationTons} тонн.
                  </span>
                </div>
                <div className="bg-white px-4 py-2 border border-emerald-100 rounded-xl leading-tight text-right shrink-0">
                  ...
                  <span className="font-mono text-emerald-700 font-bold text-base">
                    {results.seismicMassCombinationTons} т
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white/95 backdrop-blur-md border border-slate-200/60 rounded-[24px] flex flex-col h-[480px] shadow-sm shadow-slate-200/40 overflow-hidden">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-xs font-bold text-slate-850 flex items-center gap-1.5">
                    Инженерный ИИ-Советник по СНиП РМ{" "}
                    <Sparkles className="w-3.5 h-3.5 text-blue-500" />
                  </h3>
                  <p className="text-[9.5px] text-slate-450 font-medium font-sans">
                    Практические поверочные консультации по нормативам
                  </p>
                </div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4 bg-neutral-50/50">
              {apiMissingKey && (
                <div className="p-3.5 bg-indigo-50 border border-indigo-100 rounded-2xl text-[11px] text-indigo-850 shadow-2xs">
                  <span className="font-extrabold flex items-center gap-1.5 text-indigo-700 mb-1">
                    <Info className="w-4.5 h-4.5" /> ИИ Ожидает Вашей
                    Инициализации
                  </span>
                  Графический САПР-конструктор слева полностью рассчитал объемы
                  бетона, арматуры и смету. Для подключения ИИ-советника,
                  добавьте свой ключ{" "}
                  <code className="bg-slate-100 px-1 py-0.5 rounded text-blue-600 font-mono text-[10px]">
                    GEMINI_API_KEY
                  </code>{" "}
                  в панель <strong>Settings &gt; Secrets</strong> в AI Studio.
                </div>
              )}
              ...
              {messages.map((m) => (
                ...
                >
                  ...
                  <div
                    className={`max-w-[80%] rounded-[20px] px-4 py-2.5 text-xs leading-relaxed ${
                      m.role === "user"
              ...
              {isChatLoading && (
                <div className="flex gap-3 justify-start">
                  ...
                  <div className="bg-white border border-slate-200 text-slate-500 rounded-[20px] rounded-tl-none px-4 py-2.5 text-xs flex items-center gap-2.5 shadow-2xs">
                    ...
                    <span className="font-medium">
                      Изучаю геологические слои в СНиП РМ...
                    </span>
                  </div>
                </div>
              )}
            ...
            <div className="px-5 py-2.5 bg-slate-50 border-t border-slate-100 flex flex-wrap gap-1.5 shrink-0">
              <span className="text-[10px] text-slate-400 font-bold flex items-center mr-1">
                ВОПРОСЫ:
              </span>
              <button
                type="button"
                onClick={() =>
                  queryAI(
                    "Какой заводить сейсмопояс в Кишиневе по нормам Молдовы (СНиП II-7-81*)?",
                  )
                }
              ...
              <button
                ...
                onClick={() =>
                  queryAI(
                    "Что происходит с влажным лессовым суглинком (лёсс) на юге Молдовы при замачивании?",
                  )
                }
                className="text-[10px] bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 rounded-lg px-2.5 py-1.5 transition font-semibold active:scale-95 shadow-2xs cursor-pointer"
              >
              ...
              <button
                ...
                onClick={() =>
                  queryAI(
                    "Какая оптимальная глубина обратной засыпки и высота песчано-гравийной подушки под плиту?",
                  )
                }
                className="text-[10px] bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 rounded-lg px-2.5 py-1.5 transition font-semibold active:scale-95 shadow-2xs cursor-pointer"
              >
          ...
          <div className="bg-white/95 backdrop-blur-md border border-slate-200/60 rounded-[24px] p-6 shadow-sm shadow-slate-200/40">
            <h3 className="text-xs font-display font-bold uppercase tracking-wider text-slate-800 mb-3.5 flex items-center gap-2">
              <ShieldCheck className="w-4.5 h-4.5 text-blue-500" /> Свод правил
              и чек-лист инженера в Молдове
            </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-[11px] text-slate-600 leading-relaxed font-sans font-medium">
              <div className="space-y-3">
                <div className="flex items-start gap-2.5">
                  <div className="w-5 h-5 rounded-lg bg-emerald-50 text-emerald-700 flex items-center justify-center text-[10px] shrink-0 font-extrabold border border-emerald-200">
                    1
                  </div>
                  <p>
                    <strong>Инженерная геология (ИГИ)</strong>: Перед началом
                    работ требуется пробурить геоскважины 6-8м на участке, чтобы
                    ИИ/проектировщик мог определить тип залегания лёссовых глин.
                  </p>
                </div>
                ...
                <div className="flex items-start gap-2.5">
                  ...
                  <p>
                    <strong>Утепление откосов и отмостки</strong>: Сезонное
                    промерзание суглинков Молдовы выпирает ненагруженные легкие
                    ленты. Обязательна укладка гидрофобного XPS на глубину
                    промерзания ({REGION_DATA[region].frostDepth}м).
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-start gap-2.5">
                  ...
                  <p>
                    <strong>Сейсмоустойчивый контур</strong>: Молдова подвержена
                    толчкам очага Враца (Карпаты) силой 6-8 баллов. Фундамент и
                    цокольные стены должны обладать связанным армопоясом
                    (Centură antiseismică).
                  </p>
                </div>
                ...
                <div className="flex items-start gap-2.5">
                  ...
                  <p>
                    <strong>Государственная марка бетона</strong>: По нормам CP
                    F.01.02 во влажных низинах заказывайте монолит класса не
                    ниже <strong>С16/20 (М250)</strong> или{" "}
                    <strong>С20/25 (М300)</strong> с узлов Macon или Dekora.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white/95 backdrop-blur-md border border-slate-200/60 rounded-[24px] p-6 shadow-sm shadow-slate-200/40 relative overflow-hidden">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 mt-4">
              ...
              <div className="bg-slate-50 p-3 rounded-2xl border border-slate-200/50 text-center">
                ...
                <strong className="text-slate-800 text-xl font-mono block mt-1">
                  56
                </strong>
                <span className="text-[9.5px] text-slate-450 block mt-0.5">
                  Сквозные связи во всех листах Excel
                </span>
              </div>
              ...
              <div className="bg-slate-50 p-3 rounded-2xl border border-slate-200/50 text-center">
                ...
                <strong
                  className={`text-xl font-mono block mt-1 ${validationErrors.length > 0 ? "text-red-650" : "text-emerald-600"}`}
                >
                  {validationErrors.length}
                </strong>
                <span className="text-[9.5px] text-slate-450 block mt-0.5">
                  Обнаружен запуск вне допусков СНиП
                </span>
              </div>
              ...
              <div className="bg-slate-50 p-3 rounded-2xl border border-slate-200/50 text-center">
                ...
                <strong className="text-emerald-600 text-xl font-mono block mt-1">
                  0
                </strong>
                <span className="text-[9.5px] text-emerald-450 block mt-0.5 font-medium">
                  100% исправлено на "="
                </span>
              </div>
              ...
              <div className="bg-slate-50 p-3 rounded-2xl border border-slate-200/50 text-center">
                <span className="text-[9px] text-slate-400 block font-bold uppercase tracking-wider">
                  Жесткие цены заменены:
                </span>
                <strong className="text-blue-600 text-xl font-mono block mt-1">
                  100%
                </strong>
                <span className="text-[9.5px] text-slate-450 block mt-0.5">
                  Все вынесено на лист 'Настройки'
                </span>
              </div>
            </div>
            ...
            <div className="mt-5 space-y-4">
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-[11px] text-slate-600 space-y-2 leading-relaxed font-medium">
                  {groundwaterDepth < 1.5 && (
                    <p className="text-amber-800">
                      •{" "}
                      <strong>
                        Критический уровень грунтовых вод (УГВ ={" "}
                        {groundwaterDepth}м)
                      </strong>
                      : Высочайший напор воды. Ленточным фундаментам угрожает
                      намокание, обязателен непрерывный пристенный дренаж
                      (комплекты колодцев d110) и гидроизоляция. Плита УШП
                      нивелирует риски.
                    </p>
                  )}
                  {soilType === SoilType.LOESS && (
                    <p className="text-amber-800">
                      • <strong>Слабый просадочный грунт (Лёсс)</strong>:
                      Склонен к резкой структурной просадке при замачивании.
                      Плотность основания должна быть искусственно увеличена
                      методом отсыпки щебня с последующим послойным проливом и
                      виброутромбовкой.
                    </p>
                  )}
                  {soilType === SoilType.FILLED && (
                    <p className="text-amber-850">
                      • <strong>Насыпной неоплотненный грунт (Filled)</strong>:
                      Крайне аморфный грунт с непредсказуемой несущей
                      способностью. Строго предписывается использование
                      свайно-ростверкового заложенного фундамента с прорезкой
                      насыпной грунтовой подушки до надежного материкового
                      пласта.
                    </p>
                  )}
                  {landSlope > 5 && (
                    <p className="text-amber-800">
                      •{" "}
                      <strong>
                        Уклон строительной площадки ({landSlope}%)
                      </strong>
                      : Повышенные поперечные сдвиговые векторы и риск
                      сползания. Требуется террасирование (ступенчатая опалубка
                      в лентах) или фиксация рельефа фундаментными подпорными
                      стенами.
                    </p>
                  )}
                  {region !== "NORTH" && (
                    <p className="text-amber-800">
                      •{" "}
                      <strong>
                        Сейсмичность ({REGION_DATA[region].seismicClass} баллов
                        MSK-64)
                      </strong>
                      : Жесткие сейсмические силы на восток и юг Молдовы (очаг
                      Враца). Фундамент и стыки угловой арматуры обязаны
                      обладать непрерывной вязкой без зазоров.
                    </p>
                  )}
                  {landSlope <= 5 &&
                    groundwaterDepth >= 1.5 &&
                    ...
                    soilType !== SoilType.FILLED && (
                      <p className="text-emerald-700 font-semibold text-center py-1">
                        • Инженерно-геологические риски сведены к минимуму.
                        Строительное пятно расположено на стабильных грунтах со
                        стабильной сжимаемой толщей.
                      </p>
                    )}
                </div>
              </div>
              ...
              <div>
                <span className="text-[10px] text-blue-600 font-extrabold uppercase tracking-widest block mb-1.5 flex items-center gap-1.5">
                  💡 Рекомендации по повышению достоверности расчётов на
                  объекте:
                </span>
                <ul className="list-disc pl-5 text-[11px] text-slate-500 space-y-1 bg-blue-50/20 border border-blue-105/50 rounded-xl p-3.5 leading-normal font-medium">
                  <li>
                    Заблаговременно заказывайте полное Инженерно-Геологическое
                    Изыскание (бурение минимум 2 скважин по диагоналям дома) для
                    калибровки модуля деформации грунта (Е, МПа) и сцепления (С,
                    кПа).
                  </li>
                  <li>
                    Параметры инфляции цен (1.12) рекомендуется перепроверять по
                    государственным ежеквартальным строительным ведомостям цен
                    Co劈ef РМ.
                  </li>
                  <li>
                    При заливке монолитов всегда отбирайте контрольные
                    образцы-кубики бетона объёмом 100х100х100мм для лабораторных
                    испытаний на прочность на сжатие через 28 суток по ГОСТ
                    10180.
                  </li>
                </ul>
              </div>
                  ...
                  <table className="w-full text-left border-collapse">
                    <thead>
                      <tr className="bg-slate-50 text-slate-500 font-bold border-b border-slate-200">
                        ...
                        <th className="py-2 px-3">
                          Какая формула заведена / заменена
                        </th>
                    ...
                    <tbody className="divide-y divide-slate-150 text-slate-600 font-medium">
                      <tr>
                        ...
                        <td className="py-2 px-3 text-red-700">
                          Ячейки формул G7...G130
                        </td>
                        <td className="py-2 px-3">
                          <code className="bg-rose-50 text-rose-700 px-1 rounded font-mono">
                            ==D[R]*F[R]
                          </code>{" "}
                          заменено на{" "}
                          <code className="bg-slate-100 text-slate-800 px-1 rounded font-mono">
                            =D[R]*F[R]
                          </code>
                        </td>
                        <td className="py-2 px-3 italic">
                          Устранен синтаксический баг избыточного двойного
                          равенства, вызывавший сбои Excel.
                        </td>
                      </tr>
                      <tr>
                        <td className="py-2 px-3 font-semibold text-slate-800">
                        ...
                        <td className="py-2 px-3">
                          <code className="bg-slate-100 text-slate-800 px-1 rounded font-mono font-mono text-[9px]">
                            ='Настройки'!$C$X * 'Настройки'!$C$7
                          </code>
                        </td>
                        <td className="py-2 px-3 italic">
                          Устранены хаотично зашитые цифры. Теперь все цены
                          берутся из Настроек с учетом коэффициента инфляции.
                        </td>
                      </tr>
                      <tr>
                        <td className="py-2 px-3 font-semibold text-slate-800">
                        ...
                        <td className="py-2 px-3 text-red-700">
                          Ячейка Итого Конвертер
                        </td>
                        <td className="py-2 px-3 font-mono text-[9.5px]">
                          =ROUND(G[Row] / 'Настройки'!$C$5, 0)
                        </td>
                        <td className="py-2 px-3 italic">
                          Связана напрямую с ячейкой EUR_RATE в Настройках,
                          убирает статические цифры.
                        </td>
                      </tr>
                      <tr>
                        <td className="py-2 px-3 font-semibold text-slate-800">
                        ...
                        <td className="py-2 px-3 italic">
                          Обеспечивает сквозную динамическую перекачку данных
                          между листами без циклических сносок.
                        </td>
                      </tr>
                      <tr>
                        <td className="py-2 px-3 font-semibold text-slate-800">
                        ...
                        <td className="py-2 px-3">
                          Коэффициент взвешенной суммы критериев
                        </td>
                        <td className="py-2 px-3 italic">
                          Заменено жестко забитое статическое число (95% и 60%).
                          Теперь оценка считается динамически.
                        </td>
                      </tr>
                    </tbody>
                  </table>
      ...
      <footer className="bg-slate-100 border-t border-slate-200 px-6 py-5 shrink-0 text-center text-xs text-slate-500 font-medium">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-2.5">
          <p>
            © 2026 AI Foundation Expert Moldova. Программный поверочный комплекс
            САПР СНиП & NCM.
          </p>
