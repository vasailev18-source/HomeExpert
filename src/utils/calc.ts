...
export const NORMATIVES = {
  FROST_CENTER: {
    code: "FROST_CENTER",
    desc: "Расчетная глубина промерзания грунта (Центр)",
    val: 0.8,
    unit: "м",
    ref: "NCM F.02.02-2008",
    rev: "2026",
  },
  FROST_NORTH: {
    code: "FROST_NORTH",
    desc: "Расчетная глубина промерзания грунта (Север)",
    val: 1.0,
    unit: "м",
    ref: "NCM F.02.02-2008",
    rev: "2026",
  ...
  FROST_SOUTH: {
    code: "FROST_SOUTH",
    desc: "Расчетная глубина промерзания грунта (Юг)",
    val: 0.7,
    unit: "м",
    ref: "NCM F.02.02-2008",
    rev: "2026",
  ...
  SNOW_CENTER: {
    code: "SNOW_CENTER",
    desc: "Нормативная снеговая нагрузка в Центре РМ",
    val: 0.9,
    unit: "кПа",
    ref: "NCM EN 1991-1-3:2010",
    rev: "2021",
  },
  SNOW_NORTH: {
    code: "SNOW_NORTH",
    desc: "Нормативная снеговая нагрузка на Севере РМ",
    val: 1.1,
    unit: "кПа",
    ref: "NCM EN 1991-1-3:2010",
    rev: "2021",
  ...
  SNOW_SOUTH: {
    code: "SNOW_SOUTH",
    desc: "Нормативная снеговая нагрузка на Юге РМ",
    val: 0.75,
    unit: "кПа",
    ref: "NCM EN 1991-1-3:2010",
    rev: "2021",
  ...
  WIND_CENTER: {
    code: "WIND_CENTER",
    desc: "Нормативное ветровое давление в Центре РМ",
    val: 0.36,
    unit: "кПа",
    ref: "NCM EN 1991-1-4:2010",
    rev: "2021",
  },
  WIND_NORTH: {
    code: "WIND_NORTH",
    desc: "Нормативное ветровое давление на Севере РМ",
    val: 0.42,
    unit: "кПа",
    ref: "NCM EN 1991-1-4:2010",
    rev: "2021",
  ...
  WIND_SOUTH: {
    code: "WIND_SOUTH",
    desc: "Нормативное ветровое давление на Юге РМ",
    val: 0.32,
    unit: "кПа",
    ref: "NCM EN 1991-1-4:2010",
    rev: "2021",
  ...
  E_BEARING_MIN: {
    code: "E_BEARING_MIN",
    desc: "Рекомендуемый минимум прочности опорного пласта",
    val: 150,
    unit: "кПа",
    ref: "NCM EN 1997-1:2012",
    rev: "2022",
  },
  REBAR_MIN_RATIO: {
    code: "REBAR_MIN_RATIO",
    desc: "Минимум кг арматуры на кубометр бетона ребра",
    val: 58.0,
    unit: "кг/м³",
    ref: "NCM F.02.02-2008",
    rev: "2026",
  ...
  CONCRETE_COVER: {
    code: "CONCRETE_COVER",
    desc: "Защитный монолитный слой арматуры в грунте",
    val: 35,
    unit: "мм",
    ref: "NCM EN 1992-1-1:2011",
    rev: "2020",
  },
  XPS_MIN_THICK: {
    code: "XPS_MIN_THICK",
    desc: "Минимальный срез утеплителя цокольной зоны",
    val: 50,
    unit: "мм",
    ref: "NCM L.02.01:2012",
    rev: "2020",
  },
  XPS_SLAB_THICK: {
    code: "XPS_SLAB_THICK",
    desc: "Толщина XPS утепления под подошвой УШП плиты",
    val: 100,
    unit: "мм",
    ref: "NCM L.02.01:2012",
    rev: "2020",
  ...
  COMPACTION_COEFF: {
    ...
    desc: "Требуемый коэфф. послойного уплотнения засыпки",
    val: 0.98,
    unit: "безр.",
    ref: "СНиП 3.02.01-87",
    rev: "Действует",
...
export function resolveReinforcementParams(
  ...
  const isSlab =
    ...
    fndId === "ush";
  const isStrip =
    fndId === "strip" || fndId === "mzlf" || fndId === "strip_with_slab";
  const isPiles =
    fndId === "piles" ||
    fndId === "drilled_piles" ||
    fndId === "tise" ||
    fndId === "column_footing";
  ...
  const seismicFactorCoeff =
    input.seismicFactor !== undefined ? input.seismicFactor : 1.0;
  ...
  if (seismicClassLabel === "CLASS_I")
    seismic_scale_factor *= 1.25; // Critical buildings
  ...
  const isHeavy =
    buildingWeight > 150 ||
    input.wallMaterial === BuildingWallMaterial.KOTELET ||
    input.wallMaterial === BuildingWallMaterial.BRICK;
  ...
  const rebar_class_secondary =
    input.rebarClassSecondary || defRebarClassSecondary;
  if (input.rebarClassSecondary) sources.rebar_class_secondary = "USER";

  const main_bar_diameter =
    input.mainBarDiameter !== undefined
      ? input.mainBarDiameter
      : defMainBarDiameter;
  if (input.mainBarDiameter !== undefined) sources.main_bar_diameter = "USER";

  const secondary_bar_diameter =
    input.secondaryBarDiameter !== undefined
      ? input.secondaryBarDiameter
      : defSecondaryBarDiameter;
  if (input.secondaryBarDiameter !== undefined)
    sources.secondary_bar_diameter = "USER";

  const longitudinal_bars_count =
    input.longitudinalBarsCount !== undefined
      ? input.longitudinalBarsCount
      : defLongitudinalBarsCount;
  if (input.longitudinalBarsCount !== undefined)
    sources.longitudinal_bars_count = "USER";

  const top_belt_count =
    input.topBeltCount !== undefined ? input.topBeltCount : defTopBeltCount;
  if (input.topBeltCount !== undefined) sources.top_belt_count = "USER";

  const bottom_belt_count =
    input.bottomBeltCount !== undefined
      ? input.bottomBeltCount
      : defBottomBeltCount;
  if (input.bottomBeltCount !== undefined) sources.bottom_belt_count = "USER";

  const stirrup_spacing =
    input.stirrupSpacing !== undefined
      ? input.stirrupSpacing
      : defStirrupSpacing;
  if (input.stirrupSpacing !== undefined) sources.stirrup_spacing = "USER";

  const protective_layer_bottom =
    input.protectiveLayerBottom !== undefined
      ? input.protectiveLayerBottom
      : defProtectiveLayerBottom;
  if (input.protectiveLayerBottom !== undefined)
    sources.protective_layer_bottom = "USER";

  const protective_layer_side =
    input.protectiveLayerSide !== undefined
      ? input.protectiveLayerSide
      : defProtectiveLayerSide;
  if (input.protectiveLayerSide !== undefined)
    sources.protective_layer_side = "USER";

  const protective_layer_top =
    input.protectiveLayerTop !== undefined
      ? input.protectiveLayerTop
      : defProtectiveLayerTop;
  if (input.protectiveLayerTop !== undefined)
    sources.protective_layer_top = "USER";
  ...
  const lap_length =
    input.lapLength !== undefined ? input.lapLength : defLapLength;
  if (input.lapLength !== undefined) sources.lap_length = "USER";

  const corner_reinforcement =
    input.cornerReinforcement !== undefined
      ? input.cornerReinforcement
      : defCornerReinforcement;
  if (input.cornerReinforcement !== undefined)
    sources.corner_reinforcement = "USER";
  ...
  const starter_bars =
    input.starterBars !== undefined ? input.starterBars : defStarterBars;
  ...
  const secondaryWeightPerMeter = getRebarWeightPerMeter(
  ...
  if (isStrip) {
    ...
    longitudinal_total_weight_kg =
      longitudinal_total_length_m * mainWeightPerMeter;
    ...
    const coreH =
      avgTotalHeight - (protective_layer_bottom + protective_layer_top) / 1000;
    const clampPerimeter =
      2 * (Math.max(0.1, coreW) + Math.max(0.1, coreH)) + 0.25; // perimeter + structural hooks
    ...
    const cornersTotal =
      corner_reinforcements_count + intersection_reinforcements_count;
    ...
    const u_bars_total_weight_kg =
      u_bars_total_length * secondaryWeightPerMeter;

    reinforcements_total_weight_kg =
      anchorages_total_weight_kg + u_bars_total_weight_kg;
    ...
    additional_elements_weight_kg =
      reinforcements_total_weight_kg + starters_total_weight_kg;
    ...
    const column_zones_weight_kg = u_bars
      ? 4 * 4 * 1.8 * mainWeightPerMeter
      : 0; // Column stiffener reinforcement zones
    const wall_zones_weight_kg = u_bars
      ? Math.ceil(info.perimeter * 0.25) * mainWeightPerMeter
      : 0; // Wall boundary rib zones

    reinforcements_total_weight_kg =
      u_bars_total_weight_kg + column_zones_weight_kg + wall_zones_weight_kg;
    ...
    const actualPileCount =
      input.soilType === SoilType.FILLED
        ? Math.ceil(basePileCount * 1.35)
        : basePileCount;
    ...
    const cageLength = pileDepthM + 0.5;
    ...
    const pileDiameter = 0.3; // 300mm standard pile diameter
    const ringCircumference = Math.PI * (pileDiameter - 0.08) + 0.1; // core diameter + tie length
    const pileSecWeight =
      actualPileCount *
      ringsPerPile *
      ringCircumference *
      getRebarWeightPerMeter(6); // spirals are d6
    ...
    const grillageMainLinearM = grillageLength * grillageLongBarsCount * 1.1; // adding 10% overlaps
    const grillageMainWeight = grillageMainLinearM * mainWeightPerMeter;

    const grillageStirrupSpacingM = stirrup_spacing / 1000;
    const grillageStirrupsCount = Math.ceil(
      grillageLength / grillageStirrupSpacingM,
    );
    const grillageHeightM = 0.45; // 450mm grillage height
    const grillageWidthM = 0.4; // 400mm grillage width
    const grillageClampPerimeter =
      2 * (grillageWidthM - 0.08) + 2 * (grillageHeightM - 0.08) + 0.2;
    const grillageSecWeight =
      grillageStirrupsCount * grillageClampPerimeter * secondaryWeightPerMeter;
    ...
    laps_total_length_m = grillageMainLinearM * 0.1;
    laps_total_weight_kg = grillageMainLinearM * 0.1 * mainWeightPerMeter;

    clamps_total_length_m =
      actualPileCount * ringsPerPile * ringCircumference +
      grillageStirrupsCount * grillageClampPerimeter;
    transverse_total_length_m = clamps_total_length_m;
    ...
    l_bars_count = l_bars
      ? totalCorners * 4 + actualPileCount * pileLongCount
      : 0;
    anchorages_total_length_m = l_bars_count * 1.2;
    anchorages_total_weight_kg = anchorages_total_length_m * mainWeightPerMeter;
  ...
  const base_steel_mass =
    main_bars_weight_kg +
    secondary_bars_weight_kg +
    additional_elements_weight_kg;
  ...
  const b_strip = isSlab
    ? 1000
    : (info.footingArea / info.perimeter || 0.45) * 1000; // in mm
  const h_concrete = isSlab
    ? info.depthM * 1000
    : (info.depthM + (info.heightAboveGround || 0.4)) * 1000; // in mm
  ...
  const barsUsed = isSlab
    ? Math.ceil(1000 / stirrup_spacing) * 2
    : longitudinal_bars_count || 6;
  const actualAs = barsUsed * singleBarAs;
  const minAsStatus =
    actualAs >= AsMin ? "PASS" : actualAs >= AsMin * 0.8 ? "WARNING" : "FAIL";
  ...
  const clearSpacing = isSlab
    ? stirrup_spacing - main_bar_diameter
    : (b_strip -
        2 * protective_layer_side -
        (barsUsed / 2) * main_bar_diameter) /
      Math.max(1, barsUsed / 2 - 1);
  ...
  const sMaxStatus =
    (isSlab ? stirrup_spacing : b_strip / Math.max(1, barsUsed / 2 - 1)) <=
    sMaxAllowed
      ? "PASS"
      : "FAIL";
  audit_checks.push({
  ...
  const coverStatus =
    protective_layer_bottom >= requiredCover ? "PASS" : "FAIL";
  audit_checks.push({
  ...
  const cornerCheckStatus = hasL_bars
    ? "PASS"
  ...
  audit_checks.push({
    ...
    value: hasU_bars
      ? `П-образные хомуты-зажимы установлены (${u_bars_count} шт)`
      : "Т-образные пересечения без П-образных соединителей",
  ...
  audit_checks.push({
    ...
    value:
      corner_reinforcement && l_bars
        ? "Связная длина перекрытия выполнена"
        : "Конструктивная анкеровка узлов не подтверждена",
  ...
  audit_checks.push({
    ...
    value: hasStarterBars
      ? `Выпуски из фундамента d12 активны в количестве ${starter_bars_count} шт.`
      : "Выпускные анкеры не предусмотрены",
  ...
  if (isPiles) {
    ...
    audit_checks.push({
      criterion:
        "Сваи: Проверка защитного слоя буронабивного ствола в контакте с грунтом",
  ...
  return {
    ...
    u_bars_count,
    starter_bars_count,
    longitudinal_total_length_m:
      Math.round(longitudinal_total_length_m * 10) / 10,
...
export const WALL_MATERIAL_DATA: Record<
  BuildingWallMaterial,
  WallMaterialDetails
> = {
  [BuildingWallMaterial.GASOBETON]: {
    ...
    description:
      "Лёгкий теплый материал, умеренная нагрузка, требует отличной жесткости фундамента во избежание трещин.",
  },
  [BuildingWallMaterial.KOTELET]: {
    ...
    typicalThickness: 0.4, // 40 cm masonry
    description:
      "Национальный природный камень Молдовы. Обладает высочайшей несущей способностью, но крайне тяжёл. Требует массивного фундамента.",
  },
  [BuildingWallMaterial.BRICK]: {
    ...
    description:
      "Тяжелая капитальная стена. Высокая устойчивость к влаге и нагрузкам, высокая теплоёмкость.",
  },
  [BuildingWallMaterial.KERAMZIT]: {
    ...
    description:
      "Хороший компромисс между газобетоном и кирпичом. Прочный, суровый, средняя масса.",
  },
  [BuildingWallMaterial.FRAME]: {
    ...
    description:
      "Сверхлегкое сооружение. Подходит практически любой упрощенный фундамент (МЗЛФ, сваи с ростверком).",
  },
};
...
export const SLAB_DATA: Record<SlabMaterial, SlabDetails> = {
  ...
  [SlabMaterial.PB_SLAB]: {
    id: SlabMaterial.PB_SLAB,
    name: "Плиты безопалубочного формования ПБ",
    density: 1600,
    weightPerM2: 400,
  },
  [SlabMaterial.TIMBER]: {
    id: SlabMaterial.TIMBER,
    name: "Деревянные двутавровые балки",
    density: 250,
    weightPerM2: 120,
  },
  [SlabMaterial.COMBINED]: {
    id: SlabMaterial.COMBINED,
    name: "Сборно-монолитные (Терива/Марко)",
    density: 1300,
    weightPerM2: 320,
  },
};
...
export const ROOF_DATA: Record<RoofType, RoofDetails> = {
  ...
  [RoofType.GABLE_SHINGLE]: {
    id: RoofType.GABLE_SHINGLE,
    name: "Двускатная с мягкой битумной черепицей (Tegola/Shinglas)",
    deadLoad: 45, // OSB + Shingle
    angleDegrees: 35,
  },
  [RoofType.HIP_METAL]: {
    id: RoofType.HIP_METAL,
    name: "Вальмовая кровля с металлочерепицей",
    deadLoad: 60,
    angleDegrees: 30,
  },
  ...
  [RoofType.COMPOSITE_TILE]: {
    id: RoofType.COMPOSITE_TILE,
    name: "Вальмовая композитная черепица",
    deadLoad: 60,
    angleDegrees: 30,
  },
};
...
export const SOIL_DATA: Record<SoilType, SoilDetails> = {
  [SoilType.SAND]: {
    ...
    description:
      "Прекрасный непучинистый дренирующий грунт. Высокая несущая способность, минимальные осадки. Встречается в долинах рек Днестр и Прут.",
    heavingRisk: 0.1,
  ...
  [SoilType.SILTY_SAND]: {
    ...
    description:
      "Пылеватый песчаный грунт с примесью ила. Подвержен водонасыщению со значительной потерей несущей способности. Требует эффективного перехватывающего дренажа.",
    heavingRisk: 0.35,
  ...
  [SoilType.SANDY_LOAM]: {
    ...
    groundwaterSensitivity: "Умеренная",
    description:
      "Смесь песка, пыли и глины. Имеет умеренную несущую способность. Склонен к пучению при насыщении влагой.",
    heavingRisk: 0.4,
    collapsibilityRisk: 0.1,
  },
  [SoilType.LOAM]: {
    ...
    description:
      "Наиболее распространенный грунт в Молдове. Имеет умеренную несущую способность. Склонен к морозному пучению при высоком УГВ.",
    heavingRisk: 0.5,
  ...
  [SoilType.CLAY]: {
    ...
    groundwaterSensitivity: "Высокая",
    description:
      "Тяжелый пучинистый грунт. Хорошо держит воду. Склонен к значительным деформациям при замачивании. Требует обязательного утепления отмостки плитами XPS.",
    heavingRisk: 0.9,
  ...
  [SoilType.LOESS]: {
    ...
    groundwaterSensitivity: "Высокая",
    description:
      "Специфический лессовый суглинок юга и центра Молдовы. Несущая способность резко падает при замачивании! Происходит резкая просадка основания на глубину деформации.",
    heavingRisk: 0.4,
  ...
  [SoilType.FILLED]: {
    ...
    groundwaterSensitivity: "Высокая",
    description:
      "Насыпной или бытовой грунт. Крайне малая прочность, высокий риск неравномерных осадок. Требуется ТИСЭ с уширением, сваи ниже насыпи или полная структурная замена грунта.",
    heavingRisk: 0.6,
  ...
  [SoilType.ROCK]: {
    ...
    groundwaterSensitivity: "Низкая",
    description:
      "Сверхпрочный скальный известняк, аргиллиты или известковый ракушечник Молдовы. Идеальное надежное основание для любых зданий. Морозное пучение отсутствует полностью. Осадки практически нулевые.",
    heavingRisk: 0.0,
    collapsibilityRisk: 0.0,
  },
};
...
export function calculateGeologyDetails(
  ...
  const Porosity =
    input.soilType === SoilType.SAND
      ? 0.35
      : input.soilType === SoilType.ROCK
        ? 0.12
        : input.soilType === SoilType.FILLED
          ? 0.55
          : 0.42;
  const Frost_Heave_Index =
    input.soilType === SoilType.CLAY
      ? 0.08
      : input.soilType === SoilType.LOESS
        ...
        : input.soilType === SoilType.FILLED
          ? 0.07
          : 0.01;
  const Weak_Layer_Depth =
    input.soilType === SoilType.FILLED
      ? 1.5
      : input.soilType === SoilType.LOESS
        ? 2.5
        : 4.5;
  const Soil_Heterogeneity =
    input.soilType === SoilType.FILLED
      ? 1.45
      : input.soilType === SoilType.LOESS
        ? 1.25
  ...
  let reasonSlab =
    "Плита обеспечивает равномерное перераспределение вертикальных сил.";
  let scoreSlab = 85;

  if (Soil_Type === SoilType.FILLED) {
    suitabilitySlab = "LOW";
    reasonSlab =
      "Высокий риск неравномерной осадки на насыпных разнородных грунтах.";
    scoreSlab = 40;
  } else if (Soil_Type === SoilType.LOESS && Groundwater_Level < 1.5) {
    suitabilitySlab = "CRITICAL";
    reasonSlab =
      "Малозаглубленный остов на просадочном подтопленном лессе крайне недопустим.";
    scoreSlab = 15;
  } else if (Groundwater_Level < 1.0) {
    suitabilitySlab = "MEDIUM";
    reasonSlab =
      "Требуется тяжелая мембранная гидроизоляция под всем зеркалом плиты.";
    scoreSlab = 65;
  }

  let suitabilityStrip: "HIGH" | "MEDIUM" | "LOW" | "CRITICAL" = "HIGH";
  let reasonStrip =
    "Классическая заглубленная лента на прочных устойчивых пластах.";
  let scoreStrip = 90;

  if (Soil_Type === SoilType.FILLED) {
    suitabilityStrip = "CRITICAL";
    reasonStrip =
      "Ленточный остов противопоказан на неконсолидированных насыпных грунтах.";
    scoreStrip = 10;
  } else if (Soil_Type === SoilType.ROCK) {
    suitabilityStrip = "HIGH";
    reasonStrip =
      "Опора на скалу (известняк) дает прочность с нулевыми деформациями.";
    scoreStrip = 98;
  } else if (Groundwater_Level < depthM) {
    suitabilityStrip = "LOW";
    reasonStrip =
      "Урез грунтовых вод проходит выше подошвы, угрожая подтоплением котлована.";
    scoreStrip = 35;
  } else if (Soil_Type === SoilType.LOESS) {
    suitabilityStrip = "MEDIUM";
    reasonStrip =
      "Просадочность лессов требует широкой отмостки и герметичных пазух.";
    scoreStrip = 70;
  }

  let suitabilityPiles: "HIGH" | "MEDIUM" | "LOW" | "CRITICAL" = "HIGH";
  let reasonPiles =
    "Буронабивные стволы переносят нагрузки ниже деформируемой толщи.";
  let scorePiles = 95;

  if (Soil_Type === SoilType.ROCK) {
    suitabilityPiles = "LOW";
    reasonPiles =
      "Бурение скважин в прочнейшем скальном известняке экономически нецелесообразно.";
    scorePiles = 30;
  } else if (Soil_Type === SoilType.FILLED) {
    suitabilityPiles = "HIGH";
    reasonPiles =
      "Единственный способ пройти насыпь с опорой на устойчивые коренные пласты.";
    scorePiles = 92;
  } else if (Soil_Type === SoilType.LOESS) {
    suitabilityPiles = "HIGH";
    reasonPiles =
      "Сваи исключают повреждения здания при замачивании просадочного лесса.";
    scorePiles = 94;
  }

  let suitabilityUSH: "HIGH" | "MEDIUM" | "LOW" | "CRITICAL" = "HIGH";
  let reasonUSH =
    "Энергоэффективная шведская плита с интегрированным теплым полов.";
  let scoreUSH = 88;
  ...
  if (Soil_Type === SoilType.FILLED) {
    ...
    reasonUSH =
      "Размещение УШП на лессах при подтоплении запрещено нормами РМ.";
    scoreUSH = 20;
  } else if (input.landSlope > 5.0) {
    suitabilityUSH = "MEDIUM";
    reasonUSH =
      "Уклон рельефа более 5% требует подсыпки и массивного террасирования.";
    scoreUSH = 60;
  }
  ...
  let reasonClassicSlab =
    "Универсальный плитный фундамент с хорошей несущей способностью.";

  let suitabilityMzlf: "HIGH" | "MEDIUM" | "LOW" | "CRITICAL" = "HIGH";
  let scoreMzlf = 75;
  let reasonMzlf =
    "Простое мелкозаглубленное заложение для легких одноэтажных коробок.";

  let suitabilityStripWithSlab: "HIGH" | "MEDIUM" | "LOW" | "CRITICAL" = "HIGH";
  let scoreStripWithSlab = 88;
  let reasonStripWithSlab =
    "Максимально надежный комбинированный конструктив для любых жилых домов.";

  let suitabilityDrilledPiles: "HIGH" | "MEDIUM" | "LOW" | "CRITICAL" = "HIGH";
  let scoreDrilledPiles = 80;
  let reasonDrilledPiles =
    "Глубокие буронабивные сваи с монолитным железобетонным ростверком.";

  let suitabilityRibbedSlab: "HIGH" | "MEDIUM" | "LOW" | "CRITICAL" = "HIGH";
  let scoreRibbedSlab = 85;
  let reasonRibbedSlab =
    "Прочная ребристая плита с ребрами жесткости вниз для сложных нагрузок.";
  ...
  let scoreTise = 88;
  let reasonTise =
    "Сваи по технологии ТИСЭ с уширенной опорной пяткой против пучения.";

  if (Soil_Type === SoilType.FILLED) {
    suitabilityClassicSlab = "LOW";
    reasonClassicSlab =
      "Высокий риск перекосов классической плиты на рыхлой насыпи.";
    scoreClassicSlab = 35;
    suitabilityMzlf = "CRITICAL";
    reasonMzlf =
      "Мелкозаглубленная лента категорически запрещена на насыпных пластах.";
    scoreMzlf = 10;
    suitabilityStripWithSlab = "LOW";
    reasonStripWithSlab =
      "Лента опускается глубоко, но насыпь угрожает внутреннему полу.";
    scoreStripWithSlab = 45;
    suitabilityDrilledPiles = "HIGH";
    reasonDrilledPiles =
      "Позволяет пройти зыбкую насыпь до прочных коренных слоев.";
    scoreDrilledPiles = 95;
    suitabilityRibbedSlab = "LOW";
    reasonRibbedSlab =
      "Ребристая плита подвержена неравномерным грунтовым усадкам.";
    scoreRibbedSlab = 40;
    suitabilityColumn = "CRITICAL";
    reasonColumn =
      "Столбы абсолютно недопустимы на рыхлом неконсолидированном грунте.";
    scoreColumn = 5;
    suitabilityTise = "HIGH";
    reasonTise =
      "Расширенная пята ТИСЭ надежно встает в плотный коренной слой.";
    scoreTise = 90;
  } else if (Soil_Type === SoilType.LOESS) {
    suitabilityClassicSlab = "MEDIUM";
    reasonClassicSlab =
      "Требуется тщательная песчано-щебеночная подготовка против просадок.";
    scoreClassicSlab = 75;
    suitabilityMzlf = "LOW";
    reasonMzlf =
      "МЗЛФ подвержен большим перекосам при замачивании лессовых грунтов.";
    scoreMzlf = 35;
    suitabilityStripWithSlab = "MEDIUM";
    reasonStripWithSlab =
      "Плита по грунту защитит полы, но пазухи ленты требуют трамбовки.";
    scoreStripWithSlab = 80;
    suitabilityDrilledPiles = "HIGH";
    reasonDrilledPiles =
      "Исключает риски замачивания и неравномерной усадки суглинка.";
    scoreDrilledPiles = 90;
    suitabilityRibbedSlab = "MEDIUM";
    reasonRibbedSlab =
      "Повышенная жесткость ребер минимизирует риски локальной просадки.";
    scoreRibbedSlab = 82;
    suitabilityColumn = "LOW";
    reasonColumn =
      "Крайне высокий риск продавливания единичных столбов при замокании лесса.";
    scoreColumn = 20;
    suitabilityTise = "HIGH";
    reasonTise = "Сваи ТИСЭ полностью обходят просадочные горизонты суглинка.";
    scoreTise = 95;
  } else if (Soil_Type === SoilType.ROCK) {
    suitabilityClassicSlab = "HIGH";
    reasonClassicSlab =
      "Оптимальная опора плиты на жесткий прочный скалистый известняк.";
    scoreClassicSlab = 92;
    suitabilityMzlf = "HIGH";
    reasonMzlf =
      "Простое и надежное заложение облегченной ленты прямо на скалу.";
    scoreMzlf = 95;
    suitabilityStripWithSlab = "HIGH";
    reasonStripWithSlab =
      "Абсолютная капитальность и нулевая усадка на известняковой подушке.";
    scoreStripWithSlab = 96;
    suitabilityDrilledPiles = "LOW";
    reasonDrilledPiles =
      "Бурение прочных скал крайне дорого и технически затруднено.";
    scoreDrilledPiles = 25;
    suitabilityRibbedSlab = "HIGH";
    reasonRibbedSlab =
      "Отличный синергетический эффект жесткой плиты и сверхпрочной скалы.";
    scoreRibbedSlab = 90;
    suitabilityColumn = "HIGH";
    reasonColumn =
      "Простые опоры на твердый известняк для летних домиков без затрат.";
    scoreColumn = 80;
    suitabilityTise = "LOW";
    reasonTise =
      "Расширение ТИСЭ уширителем невозможно выполнить в скальном грунте.";
    scoreTise = 30;
  }
  ...
  if (Groundwater_Level < 1.5) {
    suitabilityClassicSlab = "MEDIUM";
    reasonClassicSlab =
      "Требуется качественная рулонная двухслойная гидроизоляция плиты.";
    scoreClassicSlab = Math.max(30, scoreClassicSlab - 20);
    suitabilityMzlf = "LOW";
    reasonMzlf =
      "Высокие риски пучения мелкозаглубленного фундамента при зимнем замерзании.";
    scoreMzlf = Math.max(20, scoreMzlf - 35);
    suitabilityStripWithSlab = "MEDIUM";
    reasonStripWithSlab =
      "Требуются уширенные гидроизоляционные работы на сопряжениях.";
    scoreStripWithSlab = Math.max(30, scoreStripWithSlab - 15);
    suitabilityDrilledPiles = "MEDIUM";
    reasonDrilledPiles =
      "Вымывание бетона при заливке скважин водой требует обсадных труб.";
    scoreDrilledPiles = Math.max(35, scoreDrilledPiles - 15);
    suitabilityRibbedSlab = "MEDIUM";
    reasonRibbedSlab =
      "Необходима отсыпка песчаного ядра и дренажный контур вокруг ребер.";
    scoreRibbedSlab = Math.max(35, scoreRibbedSlab - 15);
    suitabilityColumn = "LOW";
    reasonColumn =
      "Намокание почвы под опорами может вызвать вспучивание и выталкивание столбов.";
    scoreColumn = Math.max(10, scoreColumn - 30);
    suitabilityTise = "HIGH";
    reasonTise =
      "Уширенная пята препятствует выдергивающим силам морозного пучения.";
    scoreTise = Math.max(40, scoreTise - 5);
  }
  ...
  const ratingDetails = [
    ...
    {
      id: "piles",
      name: "Свайно-ростверковый буронабивной",
      score: scorePiles,
    },
    { id: "ush", name: "Утепленная шведская плита (УШП)", score: scoreUSH },
    {
      id: "classic_slab",
      name: "Классическая монолитная плита",
      score: scoreClassicSlab,
    },
    { id: "mzlf", name: "Мелкозаглубленный ленточный СНиП", score: scoreMzlf },
    {
      id: "strip_with_slab",
      name: "Ленточный с монолитной плитой по грунту",
      score: scoreStripWithSlab,
    },
    {
      id: "drilled_piles",
      name: "Буронабивные сваи с ростверком",
      score: scoreDrilledPiles,
    },
    {
      id: "ribbed_slab",
      name: "Монолитная ребристая плита",
      score: scoreRibbedSlab,
    },
    {
      id: "column_footing",
      name: "Столбчатый фундамент с ростверком",
      score: scoreColumn,
    },
  ...
  const soil_layers: GeologyLayer[] = [
    {
      name: "Почвенно-растительный слой (Слой 1)",
      thickness: 0.4,
      description: "Чернозем влажный с мелкими органическими включениями",
    },
    {
      name: `Несущий слой основания: ${soil.name} (Слой 2)`,
      thickness: 3.2,
      description: soil.description,
    },
    {
      name: "Суглинок тугопластичный серовато-бурый (Слой 3)",
      thickness: 2.4,
      description: "Суглинистая порода с осколками ракушечника",
    },
  ];
  ...
  const suitability_matrix = [
    {
      ...
      suitability:
        (suitabilityStrip as string) === "HIGH"
          ? "Высокое соответствие"
          : (suitabilityStrip as string) === "MEDIUM"
            ? "Удовлетворительно"
            : "Ограниченное соответствие",
    ...
    },
    {
      ...
      suitability:
        (suitabilitySlab as string) === "HIGH"
          ? "Высокое соответствие"
          : (suitabilitySlab as string) === "MEDIUM"
            ? "Удовлетворительно"
            : "Ограниченное соответствие",
    ...
    },
    {
      ...
      suitability:
        (suitabilityPiles as string) === "HIGH"
          ? "Высокое соответствие"
          : (suitabilityPiles as string) === "MEDIUM"
            ? "Удовлетворительно"
            : "Ограниченное соответствие",
    ...
    },
    {
      ...
      suitability:
        (suitabilityUSH as string) === "HIGH"
          ? "Высокое соответствие"
          : (suitabilityUSH as string) === "MEDIUM"
            ? "Удовлетворительно"
            : "Ограниченное соответствие",
      settlementRiskCoeff: 1.0,
    },
    {
      ...
      suitability:
        (suitabilityClassicSlab as string) === "HIGH"
          ? "Высокое соответствие"
          : (suitabilityClassicSlab as string) === "MEDIUM"
            ? "Удовлетворительно"
            : "Ограниченное соответствие",
      settlementRiskCoeff: 1.0,
    },
    {
      ...
      suitability:
        (suitabilityMzlf as string) === "HIGH"
          ? "Высокое соответствие"
          : (suitabilityMzlf as string) === "MEDIUM"
            ? "Удовлетворительно"
            : "Ограниченное соответствие",
    ...
    },
    {
      ...
      suitability:
        (suitabilityStripWithSlab as string) === "HIGH"
          ? "Высокое соответствие"
          : (suitabilityStripWithSlab as string) === "MEDIUM"
            ? "Удовлетворительно"
            : "Ограниченное соответствие",
      settlementRiskCoeff: Soil_Heterogeneity,
    },
    {
      ...
      suitability:
        (suitabilityDrilledPiles as string) === "HIGH"
          ? "Высокое соответствие"
          : (suitabilityDrilledPiles as string) === "MEDIUM"
            ? "Удовлетворительно"
            : "Ограниченное соответствие",
      settlementRiskCoeff: 0.8,
    },
    {
      ...
      suitability:
        (suitabilityRibbedSlab as string) === "HIGH"
          ? "Высокое соответствие"
          : (suitabilityRibbedSlab as string) === "MEDIUM"
            ? "Удовлетворительно"
            : "Ограниченное соответствие",
      settlementRiskCoeff: 1.0,
    },
    {
      ...
      suitability:
        (suitabilityColumn as string) === "HIGH"
          ? "Высокое соответствие"
          : (suitabilityColumn as string) === "MEDIUM"
            ? "Удовлетворительно"
            : "Ограниченное соответствие",
    ...
    },
    {
      ...
      suitability:
        (suitabilityTise as string) === "HIGH"
          ? "Высокое соответствие"
          : (suitabilityTise as string) === "MEDIUM"
            ? "Удовлетворительно"
            : "Ограниченное соответствие",
...
export function calculateUtilities(
  ...
  const sewerSub: UtilitySubsystem = {
    Project_ID,
    Element_ID: "UTL-SEW-001",
    WBS_Code: "WBS-1.2.1-SEWER",
    Risk_ID: "RSK-UTL-SEW",
    Normative_ID: "СНиП 2.04.03-85",
    Calculation_ID,
    Inspection_ID,
    id: "SEWER",
    ...
    materials: [
      {
        name: "Труба ПВХ d110 SN4 наружная уличная в отрезках 3м",
        qty: Math.ceil(sewerLength / 3),
        unit: "шт",
        cost: sewerLength * COST_RATES.NET_SEWER_PVC_MDL_M,
      },
      {
        name: "Фитинги, компенсационные муфты, ревизионные люки",
        qty: 1,
        unit: "компл",
        cost: 1200,
      },
    ...
    works: [
      {
        name: "Разработка траншей под постоянный уклон 2% вручную с песчаным ложем",
        qty: sewerLength,
        unit: "м.п.",
        cost: sewerLength * 150,
      },
      {
        name: "Укладка безнапорного рукава d110 в гильзу, засыпка",
        qty: 1,
        unit: "компл",
        cost: 2000,
      },
    ...
    qualityChecks: [
      {
        ...
        status: "PASS",
        value: "2.10% уклона",
        norm: "СНиП 2.04.03-85: Постоянный уличный уклон транзита d110 равен 2%",
      },
    ],
    risks: [
      "Засорение трассы при отклонениях уклона",
      "Деформация выпуска весом монолита",
    ],
  };

  const waterLength = 15;
  const waterMatCost =
    waterLength * COST_RATES.NET_WATER_HDPE_MDL_M + waterLength * 60 + 450;
  const waterWorkCost = waterLength * 120 + 1500;
  const waterTotalCost = waterMatCost + waterWorkCost;
  const waterSub: UtilitySubsystem = {
    Project_ID,
    Element_ID: "UTL-WAT-001",
    WBS_Code: "WBS-1.2.2-WATER",
    Risk_ID: "RSK-UTL-WAT",
    Normative_ID: "СНиП 2.04.02-84",
    Calculation_ID,
    Inspection_ID,
    id: "WATER",
    ...
    materials: [
      {
        name: "Труба ПНД d32 PN16 напорная питьевая",
        qty: waterLength,
        unit: "м.п.",
        cost: waterLength * COST_RATES.NET_WATER_HDPE_MDL_M,
      },
      {
        name: "Защитный рукав-футляр HDPE d50 мм",
        qty: waterLength,
        unit: "м.п.",
        cost: waterLength * 60,
      },
      {
        name: "Саморегулирующийся греющий кабель мощностью 16 Вт/м",
        qty: 3,
        unit: "м.п.",
        cost: 450,
      },
    ...
    works: [
      {
        name: "Прокладка футляра HDPE d50 ниже промерзания на глубине 1.2м",
        qty: waterLength,
        unit: "м.п.",
        cost: waterLength * 120,
      },
      {
        name: "Монтаж греющего контура, обжим переходников, опрессовка",
        qty: 1,
        unit: "компл",
        cost: 1500,
      },
    ...
    qualityChecks: [
      {
        ...
        norm: "СНиП 2.04.02-84: Трубы закладывать глубже расчетного промерзания грунта",
      },
    ],
    risks: [
      "Временное замерзание при критических холодах без прогрева",
  ...
  const powerMatCost =
    powerLength * COST_RATES.NET_POWER_CONDUIT_MDL_M + powerLength * 95;
  const powerWorkCost = powerLength * 90 + 1000;
  const powerTotalCost = powerMatCost + powerWorkCost;
  const powerSub: UtilitySubsystem = {
    Project_ID,
    Element_ID: "UTL-POW-001",
    WBS_Code: "WBS-1.2.3-POWER",
    Risk_ID: "RSK-UTL-POW",
    Normative_ID: "ПУЭ Республики Молдова",
    Calculation_ID,
    Inspection_ID,
    id: "POWER",
    ...
    materials: [
      {
        name: "Двустенная полиэтиленовая ПНД гофротруба d50 красная",
        qty: powerLength,
        unit: "м.п.",
        cost: powerLength * COST_RATES.NET_POWER_CONDUIT_MDL_M,
      },
      {
        name: "Кабель ВВГнг-LS 4х16 медный бронированный",
        qty: powerLength,
        unit: "м.п.",
        cost: powerLength * 95,
      },
    ...
    works: [
      {
        name: "Протяжка бронированного кабеля ВВГнг-LS в канале d50",
        qty: powerLength,
        unit: "м.п.",
        cost: powerLength * 90,
      },
      {
        name: "Заземление вводного бронепояса, опрессовка наконечников",
        qty: 1,
        unit: "компл",
        cost: 1000,
      },
    ...
    qualityChecks: [
      {
        ...
        norm: "ПУЭ: Под фундаментами прокладка бронированных кабелей осуществляется в жестких гильзах",
      },
    ],
    risks: [
      "Механический срез кабеля при просадках отмостки",
      "Заплывание вводов илом",
    ],
  };

  const lowLength = 15;
  const lowMatCost =
    lowLength * COST_RATES.NET_WEAK_CONDUIT_MDL_M + lowLength * 15;
  const lowWorkCost = lowLength * 70 + 800;
  const lowTotalCost = lowMatCost + lowWorkCost;
  const lowSub: UtilitySubsystem = {
    Project_ID,
    Element_ID: "UTL-LC-001",
    WBS_Code: "WBS-1.2.4-LOW_CURRENT",
    Risk_ID: "RSK-UTL-LC",
    Normative_ID: "СНиП 3.05.06-85",
    Calculation_ID,
    Inspection_ID,
    id: "LOW_CURRENT",
    ...
    materials: [
      {
        name: "Труба ПНД гладкая техническая d25 черная",
        qty: lowLength,
        unit: "м.п.",
        cost: lowLength * COST_RATES.NET_WEAK_CONDUIT_MDL_M,
      },
      {
        name: "Кабель сигнальный Shielded UTP под домофон/интернет",
        qty: lowLength,
        unit: "м.п.",
        cost: lowLength * 15,
      },
    ...
    works: [
      {
        name: "Прокладка ПНД труб d25 в песчаном лотке раздельно от силы",
        qty: lowLength,
        unit: "м.п.",
        cost: lowLength * 70,
      },
      {
        name: "Затяжка протяжного шнура и сигнальной витой пары",
        qty: 1,
        unit: "компл",
        cost: 800,
      },
    ...
    qualityChecks: [
      {
        ...
        status: "PASS",
        value:
          "Песчаная траншея, защитная ПНД полиэтиленовая гладкая труба d25",
        norm: "СНиП 3.05.06-85: Слаботочные кабельные магистрали укладываются раздельно от силовых цепей",
      },
    ],
    risks: [
      "Электромагнитные наводки близлежащих цепей",
      "Передавливание рукава камнями",
    ],
  };
  ...
  const spareSleevesSub: UtilitySubsystem = {
    Project_ID,
    Element_ID: "UTL-RSV-001",
    WBS_Code: "WBS-1.2.5-RESERVE_SLEEVES",
    Risk_ID: "RSK-UTL-RSV",
    Normative_ID: "NCM F.02.02",
    Calculation_ID,
    Inspection_ID,
    id: "SPARE_SLEEVES",
    ...
    materials: [
      {
        name: "Пластиковая жесткая толстостенная гильза HDPE d160",
        qty: sleeveCount,
        unit: "шт",
        cost: sleeveCount * 180,
      },
      {
        name: "Сальники и расширяющиеся пробки d160 с водоотталкивающей мастикой",
        qty: sleeveCount,
        unit: "шт",
        cost: sleeveCount * 240,
      },
    ],
    works: [
      {
        name: "Инсталляция гильз d160 перед бетонированием в теле фундамента",
        qty: sleeveCount,
        unit: "шт",
        cost: sleeveCount * 150,
      },
      {
        name: "Герметизация пустых зазоров сальниками и термоусадкой",
        qty: 1,
        unit: "компл",
        cost: 1000,
      },
    ...
    qualityChecks: [
      {
        ...
        status: "PASS",
        value: `${sleeveCount} загерметизированных гильз d160`,
        norm: "NCM F.02.02: Обеспечение подвижного зазора прохода коммуникаций через капитальные бетонные стены",
      },
    ],
    risks: [
      "Смещение заложения гильз при приеме раствора",
      "Усыхание мастики через 5-8 лет",
    ],
  };
...
export function calculateConcreteCuring(
  ...
  const winterProtectionRequired =
    input.region === MoldovaRegion.NORTH || input.safetyFactor > 1.35;
  const winterCostMDL = winterProtectionRequired
    ? Math.round(footingArea * 75)
    : 0;

  const antifreezeAdditiveQtyKg = winterProtectionRequired
    ? Math.ceil(concreteVol * 5.25)
    : 0;
  ...
  const materials: BIMEntityMaterial[] = [
    {
      name: "Рулонная защитная ПЭ пленка высокой плотности 150 мкм",
      qty: peFilmAreaM2,
      unit: "м²",
      cost: peFilmCostMDL,
    },
    {
      name: "Вода технологическая очищенная для систематического орошения",
      qty: moisturizingVolumeM3,
      unit: "м³",
      cost: moisturizingCostMDL - 1200,
    },
  ];
  ...
  const works: UtilitySubsystemWork[] = [
    {
      name: "Укрытие горизонтального зеркала залитого свежего бетона полиэтиленом",
      qty: peFilmAreaM2,
      unit: "м²",
      cost: Math.round(peFilmAreaM2 * 10),
    },
    {
      name: "Регулярный пролив водой поверхности 3 раза в сутки в течение норматива",
      qty: moisturizingDays,
      unit: "суток",
      cost: 1200,
    },
  ];
  ...
  const qualityChecks: BIMQualityCheck[] = [
    {
      criterion:
        "Предотвращение критического усадочного пересыхания бетона на ранней стадии созревания",
      status: "PASS",
  ...
  return {
    ...
    qualityChecks,
    risks: [
      "Риск усадочных волосных трещин от солнечной радиации",
      "Замедление набора прочности при ночных падениях температуры",
    ],
    airTemperatureC:
      input.region === MoldovaRegion.NORTH
        ? 8
        : input.region === MoldovaRegion.SOUTH
          ? 18
          : 14,
    methodDescription:
      "Мокрое выдерживание под герметичной влагозащитной полиэтиленовой пленкой по СНиП 3.03.01-87",
...
export function calculateBackfill(
  ...
  const backfillVolumeM3 = Math.round(
    Math.max(12, excavationVolumeM3 - constructionVolumeM3),
  );

  const materialName =
    "Супесь/песчано-гравийная смесь (ПГС) Ватич мелкой и средней фракции";
  ...
  const materials: BIMEntityMaterial[] = [
    {
      ...
      qty: materialQtyM3,
      unit: "м³",
      cost: materialCostMDL,
    },
  ];

  const works: UtilitySubsystemWork[] = [
    {
      name: "Послойная засыпка траншей/пазух песком с обильным увлажнением",
      qty: backfillVolumeM3,
      unit: "м³",
      cost: Math.round(workCostMDL * 0.4),
    },
    {
      name: "Послойное вибротрамбование площадки плитой весом 120кг",
      qty: compactionRuns,
      unit: "слоев",
      cost: Math.round(workCostMDL * 0.6),
    },
  ];
...
export function calculateBlindArea(
  ...
  const widthM = input.buildQuality === "ECONOMY" ? 0.6 : 0.8; // Экономим ширину
  const thicknessMM = input.buildQuality === "ECONOMY" ? 60 : 80;
  ...
  const rebarWeightKg = Math.round(areaM2 * (input.buildQuality === "ECONOMY" ? 2.5 : 3.8)); // Тонкая сетка
  const rebarCostMDL = Math.round(rebarWeightKg * 21);

  const xpsVolumeM3 = input.buildQuality === "ECONOMY"
    ? 0
    : Math.round(areaM2 * 0.05 * 10) / 10; // Без утеплителя в экономе
  const xpsCostMDL = input.buildQuality === "ECONOMY" ? 0 : Math.round(xpsVolumeM3 * 1400);

  const preparationSandM3 =
    Math.round(areaM2 * (input.buildQuality === "ECONOMY" ? 0.05 : 0.1) * 10) / 10;
  const preparationSandCostMDL = Math.round(preparationSandM3 * 450);

  const workCostMDL = Math.round(areaM2 * (input.buildQuality === "ECONOMY" ? 100 : 140));
  const totalCostMDL =
    concreteCostMDL +
    rebarCostMDL +
    xpsCostMDL +
  ...
  const materials: BIMEntityMaterial[] = [
    {
      name: "Товарный бетон С12/15 (М200) для защитной жесткой стяжки отмостки",
      qty: concreteVolumeM3,
      unit: "м³",
      cost: concreteCostMDL,
    },
    {
      name: "Решетка сварная арматурная дорожная d4 с ячейкой 100х100мм",
      qty: areaM2,
      unit: "м²",
      cost: rebarCostMDL,
    },
    {
      name: "Утеплитель экструдированный пенополистирол XPS 50мм (для цоколя)",
      qty: xpsVolumeM3,
      unit: "м³",
      cost: xpsCostMDL,
    },
    {
      name: "Подсыпка из ПГС коренная уплотненная с планировкой склона (карьер)",
      qty: preparationSandM3,
      unit: "м³",
      cost: preparationSandCostMDL,
    },
  ];

  const works: UtilitySubsystemWork[] = [
    {
      name: "Монтаж песчано-гравийной подушки 100мм с уплотнением виброрейкой",
      qty: areaM2,
      unit: "м²",
      cost: Math.round(workCostMDL * 0.3),
    },
    {
      name: "Укладка теплоизоляционных плит XPS 50мм, демпферная кромочная лента",
      qty: areaM2,
      unit: "м²",
      cost: Math.round(workCostMDL * 0.2),
    },
    {
      name: "Формовка бетонной разуклонной доски 80мм по периметру с затиркой",
      qty: areaM2,
      unit: "м²",
      cost: Math.round(workCostMDL * 0.5),
    },
  ];
  ...
  const qualityChecks: BIMQualityCheck[] = [
    ...
    {
      criterion:
        "Наличие жесткого гидрофобно-термического амортизационного XPS шва",
      status: "PASS",
...
export function calculateRisksList(
  ...
  if (input.soilType === SoilType.FILLED) {
    list.push({
      Risk_ID: "RSK-GEO-SL-01",
      Project_ID,
      Element_ID: "GEO-001",
      WBS_Code: "WBS-1.1",
      Normative_ID: "NCM EN 1997-1",
      Calculation_ID: "CALC-GEO-01",
      Inspection_ID: "INS-GEO-01",
      description:
        "Насыпной разнородный грунт основания с низкой несущей прочностью",
      severity: "CRITICAL",
      probability: 85,
      impact: 90,
      status: "ACTIVE",
      mitigation:
        "Выполнить замену насыпного грунта послойной песчаной подушкой с вибротрамбованием, либо применить свайно-ростверковый остов.",
    });
  } else if (input.soilType === SoilType.LOESS) {
    list.push({
      Risk_ID: "RSK-GEO-SL-02",
      Project_ID,
      Element_ID: "GEO-001",
      WBS_Code: "WBS-1.1",
      Normative_ID: "NCM EN 1997-1",
      ...
      Inspection_ID: "INS-GEO-01",
      description:
        "Просадочный чувствительный к влаге лессовый грунт основания",
      severity: "HIGH",
      probability: 65,
      impact: 75,
      status: "ACTIVE",
      mitigation:
        "Устроить широкую водонепроницаемую отмостку 2.5%, обеспечить герметичные пазухи и замки.",
    });
  }

  if (input.groundwaterDepth < 1.5) {
    list.push({
      Risk_ID: "RSK-GEO-WT-01",
      Project_ID,
      Element_ID: "GEO-002",
      WBS_Code: "WBS-1.1",
      Normative_ID: "СНиП 2.02.01",
      Calculation_ID: "CALC-GEO-01",
      Inspection_ID: "INS-GEO-01",
      ...
      status: "ACTIVE",
      mitigation:
        "Устройство кольцевого глубинного дренажа, монтаж ревизионных колодцев ПВХ d315, использование гидрофобного бетона W8.",
    });
  }

  if (rebarDensity < 65) {
    list.push({
      Risk_ID: "RSK-STR-RB-01",
      Project_ID,
      Element_ID: "STR-001",
      WBS_Code: "WBS-1.5",
      Normative_ID: "NCM F.02.02",
      Calculation_ID: "CALC-STR-01",
      Inspection_ID: "INS-STR-01",
      description:
        "Спад жесткости балок из-за низкого удельного веса арматурного каркаса",
      severity: "HIGH",
      probability: 50,
      impact: 85,
      status: "ACTIVE",
      mitigation:
        "Добавить дополнительные рабочие продольные стержни d12 А500С и сузить шаг хомутов до 150-200мм.",
    });
  }

  if (!hasCuring) {
    list.push({
      Risk_ID: "RSK-CON-CR-01",
      Project_ID,
      Element_ID: "CUR-001",
      WBS_Code: "WBS-1.3",
      Normative_ID: "СНиП 3.03.01-87",
      Calculation_ID: "CALC-CUR-01",
      Inspection_ID: "INS-CUR-01",
      description:
        "Угроза растрескивания монолита от пересыхания при отсутствии ухода",
      severity: "HIGH",
      probability: 80,
      impact: 70,
      status: "ACTIVE",
      mitigation:
        "Закрыть чашу бетона полиэтиленовой укрывной пленкой 150мкм и поливать водой в течение первых 10-14 дней.",
    });
  }

  if (!hasInsulation) {
    list.push({
      Risk_ID: "RSK-THM-INS-01",
      Project_ID,
      Element_ID: "XPS-001",
      WBS_Code: "WBS-1.6",
      Normative_ID: "NCM L.02.01:2012",
      Calculation_ID: "CALC-THM-01",
      Inspection_ID: "INS-THM-01",
      description:
        "Промерзание пучинистого грунта цоколя при отсутствии теплозащиты",
      severity: "HIGH",
      probability: 70,
      impact: 65,
      status: "ACTIVE",
      mitigation:
        "Заложить утепляющие плиты XPS плотностью не менее 35 кг/м³ толщиной 50-100мм по цоколю здания.",
    });
  }

  if (selectedOption.id === "strip" && depthM < frostDepth) {
    list.push({
      Risk_ID: "RSK-GEO-DP-01",
      Project_ID,
      Element_ID: "STR-002",
      WBS_Code: "WBS-1.5",
      Normative_ID: "NCM F.02.02",
      ...
      Inspection_ID: "INS-GEO-01",
      description:
        "Закладка подошвы выше нормативной глубины зимнего промерзания суглинка",
      severity: "CRITICAL",
      probability: 85,
      impact: 90,
      status: "ACTIVE",
      mitigation:
        "Углубить заложение монолитного ребра до отметки ниже расчетного нуля промерзания почвы (до 1.1-1.2м).",
    });
  }

  if (list.length === 0) {
    list.push({
      Risk_ID: "RSK-PLN-SAFE-01",
      Project_ID,
      Element_ID: "ALL-001",
      WBS_Code: "WBS-1",
      Normative_ID: "СНиП 2.02.01",
      Calculation_ID: "CALC-ALL",
      Inspection_ID: "INS-ALL",
...
export function calculateOptionExplanations(
  ...
  // 1. Slab / УШП
  const isSlabRecommended =
    soilType !== SoilType.FILLED &&
    !(soilType === SoilType.LOESS && gwt < 1.5) &&
    input.landSlope <= 5.0;
  let slabVer =
    "Рекомендовано в качестве оптимального плитно-энергетического ядра.";
  const slabReasons: string[] = [];
  if (soilType !== SoilType.FILLED) {
    slabReasons.push(
      `Грунт основания (${soilType}) стабилен, позволяет выполнить заливку плит с равномерным осадочным давлением.`,
    );
  }
  if (gwt >= 1.8) {
    slabReasons.push(
      "Глубокий горизонт грунтовых вод (УГВ) гарантирует сухость распределительного цокольного пирога.",
    );
  } else {
    slabReasons.push(
      "Высокий УГВ надежно отводится кольцевым дренажем с выводом в коллектор.",
    );
  }
  if (floors <= 2) {
    slabReasons.push(
      `Умеренная этажность коттеджа (${floors} эт.) обеспечивает нагрузку в пределах прочности подошвы плиты.`,
    );
  }
  if (input.landSlope <= 3.0) {
    slabReasons.push(
      "Минимальный естественный уклон рельефа исключает боковые сдвиги земляного пятна плиты.",
    );
  }
  if (!isSlabRecommended) {
    slabVer = "Не рекомендуется из-за геотехнических рисков.";
    slabReasons.push(
      "Наличие высокой просадочности грунта, насыпных отсыпок или высокого уклона делают УШП плиту опасной.",
    );
  }
  list.push({
    optionId: "slab",
  ...
  if (soilType !== SoilType.FILLED && soilType !== SoilType.LOESS) {
    stripReasons.push(
      "Плотные коренные пласты суглинков имеют высокое расчетное сопротивление R0.",
    );
  }
  if (gwt >= 1.5) {
    stripReasons.push(
      "Сухой горизонт заложения позволяет разработать траншеи без оползания и заиливания корыта.",
    );
  }
  if (
    input.wallMaterial === BuildingWallMaterial.KOTELET ||
    ...
    stripReasons.push(
      "Тяжелые каменные стены (молдавский котелец/кирпич) требуют жесткого массивного монолита ленты.",
    );
  }
  if (!isStripRecommended) {
    stripVer = "Не рекомендуется.";
    stripReasons.push(
      "Насыпной грунт или высокий уровень воды (УГВ) требуют дорогостоящей замены грунта и забивки свай.",
    );
  }
  list.push({
    optionId: "strip",
  ...
  if (soilType === SoilType.FILLED || soilType === SoilType.LOESS) {
    pilesReasons.push(
      "Единственный экономичный способ пройти слабый насыпной слой с опиранием на коренной известняк.",
    );
  }
  if (input.landSlope > 5.0) {
    pilesReasons.push(
      "Существенный перепад высот ландшафта нивелируется разной длиной буронабивных свайных стволов.",
    );
  }
  if (soilType === SoilType.ROCK) {
    pilesVer = "Нецелесообразно.";
    pilesReasons.push(
      "Прочнейшая скальная порода известняка ракушечника (ROCK) делает бурение скважин буроямом разорительным.",
    );
  }
  list.push({
    optionId: "piles",
...
export function calculateWhiteBox(
  input: CalculatorInput,
  walls: import("../types").WallCalculationResult,
): import("../types").WhiteBoxCalculationResult {
  // 1. Roof Calculation
  let roofAreaMultiplier = 1;
  let roofPriceM2 = 250; // MDL purely for material, base

  switch (input.roofType) {
    case "GABLE_METAL":
      roofAreaMultiplier = 1.3;
      roofPriceM2 = 180; // Metal Tile
      break;
    case "GABLE_SHINGLE":
      roofAreaMultiplier = 1.3;
      roofPriceM2 = 230; // OSB + Soft Roof (Мягкая кровля) + Подкладка
      break;
    case "HIP_METAL":
      roofAreaMultiplier = 1.45;
      roofPriceM2 = 210; // Вальмовая сложнее по обрезкам
      break;
    case "HIP_CERAMIC":
      roofAreaMultiplier = 1.45;
      roofPriceM2 = 450; // Ceramic Tile
      break;
    case "FLAT_PVC":
      roofAreaMultiplier = 1.05;
      roofPriceM2 = 300; // PVC Membrane
      break;
    case "SHED_BOARD":
      roofAreaMultiplier = 1.15;
      roofPriceM2 = 140; // Profnastil
      break;
  }

  // Apply Economy Mode discounts on roof
  if (input.buildQuality === "ECONOMY") {
    roofPriceM2 *= 0.8; // Cheapest metal, no premium membranes
  } else if (input.buildQuality === "PREMIUM") {
    roofPriceM2 *= 1.35; // Premium membranes, click-falz, premium coating
  }

  // Base footprint area + overhangs
  const baseArea = input.width * input.length;
  const roofAreaM2 = Math.ceil(baseArea * roofAreaMultiplier);

  // Timber frame (Mauerlat, Rafters, etc.)
  const timberVolM3 = roofAreaM2 * 0.05; // rough estimate
  const timberCostMDL = timberVolM3 * 6500; // 6500 MDL / m3 timber
  const insulationCostMDL = roofAreaM2 * 120; // 120 MDL/m2
  const coveringCostMDL = roofAreaM2 * roofPriceM2;
  const roofLaborMDL = roofAreaM2 * 400; // 400 MDL / m2 work
  const roofTotalCostMDL =
    timberCostMDL + insulationCostMDL + coveringCostMDL + roofLaborMDL;

  // 2. Glazing Calculation
  // Total wall area = perimeter * height
  const glazingPercent = input.glazingType === "PANORAMIC" ? 0.35 : 0.15;
  const glazingAreaM2 = Math.ceil(walls.wallAreaGrossM2 * glazingPercent);
  // Standard profile vs warm Alu/Premium PVC
  let windowPriceM2 = input.glazingType === "PANORAMIC" ? 4500 : 2500;
  if (input.buildQuality === "ECONOMY") windowPriceM2 *= 0.75; // Basic 4 or 5 chamber profiles
  if (input.buildQuality === "PREMIUM") windowPriceM2 *= 1.4; // Schuco/Alutech warm aluminum or premium 7-chamber PVC
  const windowsCostMDL = glazingAreaM2 * windowPriceM2;

  // 3. Facade Calculation
  const facadeAreaM2 = Math.ceil(walls.wallAreaGrossM2 - glazingAreaM2);
  let facadePriceM2 = 0;
  if (input.buildQuality === "ECONOMY") {
    facadePriceM2 = 450; // basic EPS 50mm + bare base coat
  } else if (input.buildQuality === "PREMIUM") {
    switch (input.facadeTech) {
      case "WET":
        facadePriceM2 = 900; // Premium silicone plaster + 150mm insulation
        break;
      case "VENTILATED":
        facadePriceM2 = 2200; // Premium HPL / Kerlite / Compact laminate
        break;
      case "FACE_BRICK":
        facadePriceM2 = 1800; // Clinker premium imported brick
        break;
    }
  } else {
    switch (input.facadeTech) {
      case "WET":
        facadePriceM2 = 600; // 600 MDL / m2 (EPS + Plaster + Labor)
        break;
      case "VENTILATED":
        facadePriceM2 = 1500; // 1500 MDL / m2 (Substructure + Insulation + Panels + Labor)
        break;
      case "FACE_BRICK":
        facadePriceM2 = 1200; // 1200 MDL / m2 (Facing brick + mortar + Labor)
        break;
    }
  }
  const facadeCostMDL = facadeAreaM2 * facadePriceM2;

  // 4. Floor Screed
  const floorsScreedAreaM2 = baseArea * input.floors;
  let screedPriceM2 = 250;
  if (input.buildQuality === "ECONOMY") screedPriceM2 = 160;
  if (input.buildQuality === "PREMIUM") screedPriceM2 = 350; // Extra thick foamglass or thick XPS
  const floorsScreedCostMDL = floorsScreedAreaM2 * screedPriceM2;

  // 5. Inter-Floor Slabs (Перекрытия)
  let slabCostMDL = 0;
  if (input.floors > 1 || input.hasBasement) {
    // Calculate the area of slabs needed
    const slabsCount = input.floors - 1 + (input.hasBasement ? 1 : 0);
    const totalSlabAreaM2 = baseArea * slabsCount;
    let slabPriceM2 = 0;

    switch (input.slabMaterial) {
      case "MONOLITH":
        slabPriceM2 = 1400; // Concrete + Rebar + Formwork + Labor
        break;
      case "HOLLOW_CORE":
        slabPriceM2 = 900; // Precast plates (ПБК) + Crane + Installation
        break;
      case "TIMBER":
        slabPriceM2 = 450; // Timber joists + OSB
        break;
    }
    if (input.buildQuality === "ECONOMY") slabPriceM2 *= 0.85;
    if (input.buildQuality === "PREMIUM") slabPriceM2 *= 1.25; // Premium noise isolation + flat ceiling
    slabCostMDL = totalSlabAreaM2 * slabPriceM2;
  }

  // 6. Internal Plastering (Внутренняя штукатурка стен)
  // Internal wall area normally roughly equals Gross Wall Area * 1.5-2x (counting inside outer walls and partitioning)
  const internalPlasterAreaM2 = Math.ceil(walls.wallAreaGrossM2 * 1.6);
  let plasterPriceM2 = 180;
  if (input.buildQuality === "ECONOMY") plasterPriceM2 = 120;
  if (input.buildQuality === "PREMIUM") plasterPriceM2 = 250; // Includes initial q2-q3 finish
  const internalPlasterCostMDL = internalPlasterAreaM2 * plasterPriceM2;

  // 7. Stairs (ЖБ лестницы)
  const hasStairs = input.floors > 1 || input.hasBasement;
  const flightsCount = input.floors - 1 + (input.hasBasement ? 1 : 0);
  let stairFlightPrice = 25000;
  if (input.buildQuality === "ECONOMY" && input.slabMaterial === "TIMBER") {
    stairFlightPrice = 8000; // Wooden stair frame instead of concrete
  }
  const stairsCostMDL = hasStairs ? flightsCount * stairFlightPrice : 0;

  const totalWhiteBoxCostMDL =
    roofTotalCostMDL +
    windowsCostMDL +
    facadeCostMDL +
    floorsScreedCostMDL +
    slabCostMDL +
    internalPlasterCostMDL +
    stairsCostMDL;

  return {
    roofAreaM2,
    roofCostMDL: Math.round(roofTotalCostMDL),
    glazingAreaM2,
    windowsCostMDL: Math.round(windowsCostMDL),
    facadeAreaM2,
    facadeCostMDL: Math.round(facadeCostMDL),

    hasStairs,
    stairsCostMDL: Math.round(stairsCostMDL),
    floorsScreedAreaM2: Math.round(floorsScreedAreaM2),
    floorsScreedCostMDL: Math.round(floorsScreedCostMDL),
    internalPlasterAreaM2: Math.round(internalPlasterAreaM2),
    internalPlasterCostMDL: Math.round(internalPlasterCostMDL),
    slabCostMDL: Math.round(slabCostMDL),

    totalWhiteBoxCostMDL: Math.round(totalWhiteBoxCostMDL),
  };
}

export function calculateWalls(
  input: CalculatorInput,
): import("../types").WallCalculationResult {
  let wallThickness = 0.3;
  let blocksPerM3 = 28;
  let pricePerBlock = 40; // MDL
  let materialDensity = 600;

  let isFrame = false;
  switch (input.wallMaterial) {
    case "KOTELET":
      wallThickness = 0.4;
      blocksPerM3 = 71;
      pricePerBlock = 16;
  ...
  const seismicity =
    input.region === "NORTH" ? 7 : input.region === "CENTER" ? 8 : 9;

  let needsColumns = true;
  let needsBelts = true;
  ...
  if (isFrame) {
    ...
    if (seismicity === 7) {
      // Zone 7 (North)
      if (input.floors <= 2 || input.buildingType === "GARAGE") {
        needsColumns = false;
      }
    } else if (seismicity === 8) {
      // Zone 8 (Center)
      if (input.floors === 1 && input.buildingType === "GARAGE") {
        needsColumns = false;
  ...
  // Assuming 1 internal bearing wall for spans > 6m
  const internalWallsLength =
    Math.max(input.width, input.length) > 6
      ? Math.min(input.width, input.length)
      : 0;
  const totalWallLength = perimeter + internalWallsLength;

  // Average floor height
  const floorHeight = 3.0; // meters
  let totalFloorsHeight = input.floors * floorHeight;
  let wallAreaGrossM2 = totalWallLength * totalFloorsHeight;

  // Add roof gables (фронтоны) if Gable Roof (Двускатная)
  if (input.roofType === "GABLE_METAL") {
    const gableHeightM = (input.width / 2) * 0.577; // tan(30 degrees)
    const gableAreaM2 = ((input.width * gableHeightM) / 2) * 2; // Two gables
    wallAreaGrossM2 += gableAreaM2;
  }

  // 20% opening for windows and doors
  let wallAreaNetM2 = wallAreaGrossM2 * 0.8;

  // Calculate seismic frame
  let columnsCount = 0;
  let concreteColumnsM3 = 0;
  let seismicBeltLengthM = 0;
  let concreteBeltM3 = 0;
  let totalConcreteM3 = 0;
  let rebarKg = 0;

  if (needsColumns && input.buildingType !== "GARAGE") {
    // Columns every ~4 meters + 4 corners + internal
    columnsCount = Math.ceil(totalWallLength / 4) + 2;
    const colSize = wallThickness > 0.3 ? 0.4 : 0.3;
    const volumePerColumn = colSize * colSize * totalFloorsHeight;
    concreteColumnsM3 = columnsCount * volumePerColumn;

    // Belts per floor
    const beltCount = Math.ceil(input.floors);
    seismicBeltLengthM = totalWallLength * beltCount;
    concreteBeltM3 = seismicBeltLengthM * wallThickness * 0.2; // 20cm thick belt

    totalConcreteM3 = concreteColumnsM3 + concreteBeltM3;
    rebarKg = totalConcreteM3 * 80;
  } else if (needsColumns && input.buildingType === "GARAGE") {
    // Garages usually have simpler frames
    columnsCount = 4;
    const colSize = 0.3;
    const volumePerColumn = colSize * colSize * (input.floors * 3);
    concreteColumnsM3 = columnsCount * volumePerColumn;
    seismicBeltLengthM = perimeter;
    concreteBeltM3 = seismicBeltLengthM * wallThickness * 0.2;

    totalConcreteM3 = concreteColumnsM3 + concreteBeltM3;
    rebarKg = totalConcreteM3 * 70;
  } else if (needsBelts && !needsColumns) {
    // Only belts required
    const beltCount = Math.ceil(input.floors);
    seismicBeltLengthM = totalWallLength * beltCount;
    concreteBeltM3 = seismicBeltLengthM * Math.min(wallThickness, 0.3) * 0.2; // max 30x20 belt

    totalConcreteM3 = concreteBeltM3;
    rebarKg = totalConcreteM3 * 60; // Less reinforcement for just belts
  }

  const blocksVolumeGrossM3 = wallAreaNetM2 * wallThickness;
  // Subtract frame volume
  const blocksVolumeM3 = Math.max(0, blocksVolumeGrossM3 - totalConcreteM3);
  const blocksCount = blocksVolumeM3 * blocksPerM3;
  const mortarVolumeM3 = blocksVolumeM3 * 0.05;

  // Masonry reinforcement (every 3rd row ~600mm)
  let masonryRebarKg = 0;
  if (!isFrame) {
    const rowsCount = Math.ceil(totalFloorsHeight / 0.6);
    const meshLengthM = totalWallLength * rowsCount;
    // ~0.5kg per linear meter of masonry reinforcement (mesh or 2x d8 bars)
    masonryRebarKg = meshLengthM * 0.5;
  }

  // Ventilation and exhaust channels
  let ventChannelsCount = 0;
  if (input.buildingType === "HOUSE") {
    ventChannelsCount = 4; // Kitchen, Kotelnaya, 2x Bathrooms
  } else {
    ventChannelsCount = 1; // Garage exhaust
  }
  const ventBlocksCount =
    ventChannelsCount * Math.ceil(totalFloorsHeight * 3.3); // ~3.3 blocks per meter height per channel
  const ventBlocksCostMDL = ventBlocksCount * 45; // ~45 MDL per vent block

  const masonryRebarCostMDL = masonryRebarKg * 18;
  const blocksCostMDL = blocksCount * pricePerBlock;
  const concreteCostMDL = totalConcreteM3 * COST_RATES.CONCRETE_MDL_M3;
  const rebarCostMDL = rebarKg * 18;

  // Installation: 600 MDL / m3 for blocks, 1000 MDL/m3 for concrete pouring
  const laborCostMDL =
    blocksVolumeM3 * 600 + totalConcreteM3 * 1000 + ventBlocksCount * 25;

  const materialsCostMDL =
    blocksCostMDL +
    concreteCostMDL +
    rebarCostMDL +
    masonryRebarCostMDL +
    ventBlocksCostMDL;
  const totalCostMDL = materialsCostMDL + laborCostMDL;

  return {
    columnsCount: Math.ceil(columnsCount),
    concreteColumnsM3: Number(concreteColumnsM3.toFixed(1)),
    seismicBeltLengthM: Number(seismicBeltLengthM.toFixed(1)),
    concreteBeltM3: Number(concreteBeltM3.toFixed(1)),
    totalConcreteM3: Number(totalConcreteM3.toFixed(1)),
    rebarKg: Number(rebarKg.toFixed(0)),
    wallAreaGrossM2: Number(wallAreaGrossM2.toFixed(1)),
    wallAreaNetM2: Number(wallAreaNetM2.toFixed(1)),
    blocksVolumeM3: Number(blocksVolumeM3.toFixed(1)),
    blocksCount: Math.ceil(blocksCount),
    mortarVolumeM3: Number(mortarVolumeM3.toFixed(1)),
    blocksCostMDL: Number(blocksCostMDL.toFixed(0)),
    concreteCostMDL: Number(concreteCostMDL.toFixed(0)),
    rebarCostMDL: Number(rebarCostMDL.toFixed(0)),
    masonryRebarKg: Number(masonryRebarKg.toFixed(0)),
    masonryRebarCostMDL: Number(masonryRebarCostMDL.toFixed(0)),
    ventChannelsCount,
    ventBlocksCount: Math.ceil(ventBlocksCount),
    ventBlocksCostMDL: Number(ventBlocksCostMDL.toFixed(0)),
    materialsCostMDL: Number(materialsCostMDL.toFixed(0)),
    laborCostMDL: Number(laborCostMDL.toFixed(0)),
    totalCostMDL: Number(totalCostMDL.toFixed(0)),
    needsColumns,
    needsBelts,
    seismicity,
    isFrame,
 ...
 */
import { enrichFoundationWithProofs } from "./engineeringProofs";

export function calculateFoundation(
  input: CalculatorInput,
): CalculationResults {
  const wallsData = calculateWalls(input);
  ...
  const structuralFloors = input.futureFlooringExtension
    ? input.floors + 1
    : input.floors;
  ...
  const scenarioASnowDominantTons =
    Gd_dead_total + Qd_snow + 0.6 * Qd_wind + 0.7 * Qd_live;

  // Scenario B (Live Load Dominant): Full Design Imposed/Live Load + Reduced Snow (psi0 = 0.5) + Reduced Wind (psi0 = 0.6)
  const scenarioBLiveDominantTons =
    Gd_dead_total + Qd_live + 0.5 * Qd_snow + 0.6 * Qd_wind;
  ...
  const seismicAmplification =
    2.5 * (seismicGroundTypeFactor / seismicBehaviorFactor);

  // Dynamic horizontal seismic shear force (in tons)
  const seismicForceTons =
    seismicMassCombinationTons *
    seismicPGA *
  ...
  const totalFactoredWeightTons =
    Math.round(
      Math.max(scenarioASnowDominantTons, scenarioBLiveDominantTons) * 10,
    ) / 10;
  ...
  const seismicBearingAreaMultiplier =
    1.0 +
    (seismicForceTons / totalFactoredWeightTons) *
      (seismicPGA >= 0.16 ? 0.38 : 0.22);
  const bearingAreaRequiredM2 =
    Math.round(
      ((totalFactoredForceKN *
        input.safetyFactor *
        seismicBearingAreaMultiplier) /
  ...
  switch (input.soilType) {
    ...
    case SoilType.LOAM:
      ...
      break;
    case SoilType.CLAY:
      soilConcreteMultiplier = 1.1;
      soilRebarMultiplier = 1.15;
      break;
    case SoilType.LOESS:
      soilConcreteMultiplier = 1.2;
      soilRebarMultiplier = 1.25;
      break;
    case SoilType.FILLED:
      soilConcreteMultiplier = 1.35;
      soilRebarMultiplier = 1.4;
      break;
    case SoilType.ROCK:
      soilConcreteMultiplier = 0.8; // highly stable solid bedrock
  ...
  const seismicRebarMultiplier =
    seismicPGA === 0.24 ? 1.3 : seismicPGA === 0.16 ? 1.15 : 1.0;
  const seismicSlabThicknessIncrease =
    seismicPGA === 0.24 ? 0.05 : seismicPGA === 0.16 ? 0.02 : 0.0;
  ...
  let hasDrainage = input.groundwaterDepth < 1.5;
  if (input.buildQuality === "ECONOMY" && input.groundwaterDepth >= 1.0) {
    hasDrainage = false; // "Самый дешевый" без дренажа если грунтовки нет прямо на поверхности
  }
  ...
  const drainageStoneM3 = hasDrainage
    ? Math.ceil(drainagePipeM * 0.4 * 0.35)
    : 0;
  ...
  const requiredStripWidthM = Math.max(
    0.4,
    Math.ceil((bearingAreaRequiredM2 / perimeter) * 10) / 10,
  );
  // SNiP requires laying below frost depth. Economy: +0.05, Luxury: +0.2
  const stripDepthM = reg.frostDepth + (input.buildQuality === "ECONOMY" ? 0.05 : 0.2); 

  // Min socle above ground by SNiP is 0.2m.
  const stripHeightAboveGroundM = input.buildQuality === "ECONOMY" ? 0.2 : 0.4; 
  ...
  const stripConcreteVolumeM3 =
    L_total * requiredStripWidthM * avgTotalHeight * soilConcreteMultiplier;

  // 1.2 Опалубка деревянная (внутренняя и внешняя грани цоколя плюс верхняя часть траншеи)
  const formworkHeight = stripHeightAboveGroundM + heightDiff / 2 + 0.2; // Высота щитов
  ...
  const stripSandVolumeM3 =
    L_total * requiredStripWidthM * 0.2 * (1 + slopeFrac * 0.5);
  ...
  const excavationVolumeM3Strip =
    L_total * (requiredStripWidthM + 0.4) * (stripDepthM + heightDiff / 2);
  const excavationCostStripMDL = Math.round(
    excavationVolumeM3Strip * COST_RATES.EXCAVATION_MDL_M3,
  );

  // 1.5 Рабочая арматура по СНиПу (минимум 0.1% от сечения ж/б элемента)
  const rStrip = resolveReinforcementParams(input, "strip", {
    perimeter,
    footingArea,
    depthM: stripDepthM,
    concreteVolumeM3: stripConcreteVolumeM3,
  });
  ...
  const rebarTransverseKgStrip = Math.round(
    (rStrip.secondary_bars_weight_kg || 0) +
      (rStrip.additional_elements_weight_kg || 0),
  );
  const rebarBindingCostStripMDL = Math.round(
    reinforcementBarKgStrip * COST_RATES.REBAR_BINDING_LABOR_MDL_KG,
  );

  // 1.6 Гидроизоляция и утепление (без утепления цоколя в эконом-варианте)
  const stripWaterproofM2 =
    L_total * (stripDepthM + stripHeightAboveGroundM + heightDiff / 2) * 2;
  const stripInsulationM3 = input.buildQuality === "ECONOMY" ? 0 : perimeter * 0.8 * 0.05 * (1 + slopeFrac); // утепление цоколя плитами XPS 50мм
  ...
  const rFloorConcreteM3Strip = rFloorAreaStrip * 0.1; // Стяжка 10 см
  ...
  const slopeComplicationCostStripMDL =
    input.landSlope > 0
      ? Math.round(perimeter * (input.landSlope / 100) * 800)
      : 0;

  // Локальный расчет бюджета
  const stripCost = compileDetailedBudget(
    stripMaterials,
    excavationCostStripMDL,
    rebarBindingCostStripMDL,
    drainageCostMDL,
    slopeComplicationCostStripMDL,
    input,
  );

  const seismicPoints =
    reg.seismicCoeff === 0.08 ? 6.5 : reg.seismicCoeff === 0.16 ? 7 : 8;
  ...
  let stripFrost =
    input.soilType === SoilType.CLAY || input.soilType === SoilType.LOAM
      ? 80
      : 92;
  if (input.groundwaterDepth < 1.5) stripFrost -= 10;
  let stripGroundwater = input.groundwaterDepth < 1.5 ? 55 : 85;
  let stripCollapsible =
    input.soilType === SoilType.LOESS || input.soilType === SoilType.FILLED
      ? input.soilType === SoilType.FILLED
        ? 40
        : 55
      : 90;
  let stripRepair = 85;
  let stripReserve =
    soil.resistanceKPa >= 200 ? 95 : soil.resistanceKPa >= 150 ? 88 : 75;

  let stripReliability = Math.round(
    stripSeismic * 0.2 +
      stripFrost * 0.15 +
      stripGroundwater * 0.15 +
      stripCollapsible * 0.2 +
      stripRepair * 0.15 +
      stripReserve * 0.15,
  );
  ...
  const stripPros = [
    "Капитальный классический вариант",
    "Надежность подтверждена десятилетиями",
    "Возможность легкого обустройства подпола или цоколя",
  ];
  const stripCons = [
  ...
  if (input.groundwaterDepth < stripDepthM + 0.3) {
    stripRisks.push(
      "Опасность затопления траншеи при строительстве, ослабление грунта основания",
    );
  }

  // --- OPTION 2: МОНОЛИТНАЯ ПЛИТА (Slab on Grade) ---
  const slabDepthM = 0.3; // Монолитная несущая плита 30 см (стандарт NCM)
  ...
  const slabPlinthConcreteVolumeM3 = perimeter * 0.3 * plinthHeightDiff;
  const slabConcreteVolume =
    (footingArea * slabDepthM + slabPlinthConcreteVolumeM3) *
    soilConcreteMultiplier;
  ...
  const excavationVolumeM3Slab = slabExcArea * (0.4 + plinthHeightDiff);
  const excavationCostSlabMDL = Math.round(
    excavationVolumeM3Slab * COST_RATES.EXCAVATION_MDL_M3,
  );
  ...
  const rSlab = resolveReinforcementParams(input, "slab", {
    perimeter,
    footingArea,
    depthM: slabDepthM,
    concreteVolumeM3: footingArea * slabDepthM + slabPlinthConcreteVolumeM3,
  });
  ...
  const rebarTransverseKgSlab = Math.round(
    (rSlab.secondary_bars_weight_kg || 0) +
      (rSlab.additional_elements_weight_kg || 0),
  );
  const rebarBindingCostSlabMDL = Math.round(
    reinforcementBarKgSlab * COST_RATES.REBAR_BINDING_LABOR_MDL_KG,
  );
  ...
  if (computedXpsThicknessM < 0.1) {
    computedXpsThicknessM = 0.1; // Минимальный проектный лимит для плит типа УШП по СНиП РМ
  }

  // Боковое утепление торцов плиты (выполняет роль демпфера промерзания слепых зон на цоколе)
  const computedEdgeXpsThicknessM = Math.max(
    0.05,
    Math.ceil(computedXpsThicknessM / 2 / 0.05) * 0.05,
  );

  const slabInsulationM3 =
    footingArea * computedXpsThicknessM +
    perimeter * 0.4 * computedEdgeXpsThicknessM;
  ...
  const slopeComplicationCostSlabMDL =
    input.landSlope > 0
      ? Math.round(footingArea * (input.landSlope / 100) * 1500)
      : 0;

  const slabCost = compileDetailedBudget(
    slabMaterials,
    excavationCostSlabMDL,
    rebarBindingCostSlabMDL,
    drainageCostMDL,
    slopeComplicationCostSlabMDL,
    input,
  );
  ...
  let slabReserve =
    soil.resistanceKPa >= 200 ? 99 : soil.resistanceKPa >= 150 ? 98 : 95;
  ...
  const basePileCount =
    Math.ceil(perimeter / 1.5) + (structuralFloors > 1 ? 4 : 2);
  const pileCount =
    input.soilType === SoilType.FILLED
      ? Math.ceil(basePileCount * 1.35)
      : basePileCount;

  const pileDepthM = input.buildQuality === "ECONOMY" ? Math.max(1.5, reg.frostDepth + 0.2) : 2.2;

  // На уклоне сваи бурятся на стандартную глубину, но часть ростверка приподнимается над землей (низкое удорожание!)
  const singlePileVolume = Math.PI * 0.15 * 0.15 * pileDepthM;
  const pilesConcreteVolume =
    pileCount * singlePileVolume * (1 + slopeFrac * 0.2);

  const beamHeightM = 0.45;
  const beamWidthM = 0.4;
  const beamVolume =
    perimeter * beamWidthM * beamHeightM * (1 + slopeFrac * 0.4);
  const pileStripConcreteVolume =
    (pilesConcreteVolume + beamVolume) * soilConcreteMultiplier;

  // Объем разработки земли (бурение свайных шахт d300мм + неглубокая траншея ростверка 20см)
  const excavationPileShaftsM3 = pileCount * Math.PI * 0.15 * 0.15 * pileDepthM;
  const excavationBeamTrenchM3 = perimeter * beamWidthM * 0.2;
  const excavationVolumeM3Pile =
    excavationPileShaftsM3 + excavationBeamTrenchM3 * (1 + slopeFrac);
  const excavationCostPileMDL =
    Math.round(excavationVolumeM3Pile * COST_RATES.EXCAVATION_MDL_M3) + 4500;
  ...
  const pileSandVolume = perimeter * beamWidthM * 0.1;
  const sandGravelWeightPileTons = pileSandVolume * 1.6;

  // Армирование свай и ростверка по физической схеме NCM / EC2
  const rPiles = resolveReinforcementParams(input, "piles", {
    perimeter,
    footingArea,
    depthM: 2.2,
    concreteVolumeM3: pileStripConcreteVolume,
  });
  ...
  const rebarTransverseKgPile = Math.round(
    (rPiles.secondary_bars_weight_kg || 0) +
      (rPiles.additional_elements_weight_kg || 0),
  );
  const rebarBindingCostPileMDL = Math.round(
    reinforcementBarKgPile * COST_RATES.REBAR_BINDING_LABOR_MDL_KG,
  );
  ...
  const rFloorConcreteM3Pile = rFloorAreaPile * 0.1; // Стяжка 10 см
  ...
  const pileMaterials: MaterialRequirement = {
    ...
    pileCount,
    pileDrillingM: Math.round(pileCount * pileDepthM * 10) / 10,
  };

  // Миноритари за уклон на сваях
  const slopeComplicationCostPileMDL =
    input.landSlope > 0
      ? Math.round(perimeter * (input.landSlope / 100) * 400)
      : 0;

  const pileCost = compileDetailedBudget(
    pileMaterials,
    excavationCostPileMDL,
    rebarBindingCostPileMDL,
    drainageCostMDL,
    slopeComplicationCostPileMDL,
    input,
  );
  ...
  let pileRepair = 80;
  let pileReserve =
    soil.resistanceKPa >= 200 ? 85 : soil.resistanceKPa >= 150 ? 78 : 65;
  ...
  const pilePros = [
    ...
    `Заглубление свайные пят на ${pileDepthM}м - гарантированно ниже промерзания во всех районах РМ`,
  ];
  const pileCons = [
  ...
  if (
    input.wallMaterial === BuildingWallMaterial.FRAME ||
    (input.wallMaterial === BuildingWallMaterial.GASOBETON &&
      input.floors === 1)
  ) {
  ...
  if (input.soilType === SoilType.LOESS) {
    pileRisks.push(
      "Просадка грунта может оголить сваи, вызвав потерю сцепления по бокам",
    );
  }

  // --- OPTION 4: КЛАССИЧЕСКАЯ МОНОЛИТНАЯ ПЛИТА (classic_slab) ---
  const classicSlabDepthM = 0.25;
  const classicSlabConcreteM3 =
    footingArea * classicSlabDepthM * soilConcreteMultiplier;
  const rClassicSlab = resolveReinforcementParams(input, "classic_slab", {
    perimeter,
    footingArea,
    depthM: classicSlabDepthM,
    concreteVolumeM3: classicSlabConcreteM3,
  });
  const classicSlabRebarKg = Math.round(
    rClassicSlab.total_rebar_weight_kg || 0,
  );
  ...
  const classicSlabMaterials: MaterialRequirement = {
    ...
    rebarTransverseKg: Math.round(
      (rClassicSlab.secondary_bars_weight_kg || 0) +
        (rClassicSlab.additional_elements_weight_kg || 0),
    ),
  ...
  const excavationCostClassicSlabMDL = Math.round(
    classicSlabMaterials.excavationVolumeM3 * COST_RATES.EXCAVATION_MDL_M3,
  );
  const rebarBindingCostClassicSlabMDL = Math.round(
    classicSlabMaterials.reinforcementBarKg *
      COST_RATES.REBAR_BINDING_LABOR_MDL_KG,
  );
  const slopeComplicationCostClassicSlabMDL =
    input.landSlope > 0
      ? Math.round(footingArea * (input.landSlope / 100) * 850)
      : 0;
  const classicSlabCost = compileDetailedBudget(
    classicSlabMaterials,
    excavationCostClassicSlabMDL,
    rebarBindingCostClassicSlabMDL,
    drainageCostMDL,
    slopeComplicationCostClassicSlabMDL,
    input,
  );
  ...
  const mzlfWidthM = Math.max(
    0.4,
    Math.ceil(requiredStripWidthM * 0.8 * 10) / 10,
  );
  const mzlfConcreteVolumeM3 =
    L_total * mzlfWidthM * mzlfTotalHeight * soilConcreteMultiplier;
  const rMzlf = resolveReinforcementParams(input, "mzlf", {
    perimeter,
    footingArea,
    depthM: mzlfDepthM,
    concreteVolumeM3: mzlfConcreteVolumeM3,
  });
  ...
  const mzlfMaterials: MaterialRequirement = {
    ...
    rebarTransverseKg: Math.round(
      (rMzlf.secondary_bars_weight_kg || 0) +
        (rMzlf.additional_elements_weight_kg || 0),
    ),
  ...
  const excavationCostMzlfMDL = Math.round(
    mzlfMaterials.excavationVolumeM3 * COST_RATES.EXCAVATION_MDL_M3,
  );
  const rebarBindingCostMzlfMDL = Math.round(
    mzlfMaterials.reinforcementBarKg * COST_RATES.REBAR_BINDING_LABOR_MDL_KG,
  );
  const slopeComplicationCostMzlfMDL =
    input.landSlope > 0
      ? Math.round(perimeter * (input.landSlope / 100) * 450)
      : 0;
  const mzlfCost = compileDetailedBudget(
    mzlfMaterials,
    excavationCostMzlfMDL,
    rebarBindingCostMzlfMDL,
    drainageCostMDL,
    slopeComplicationCostMzlfMDL,
    input,
  );

  // --- OPTION 6: ЛЕНТОЧНЫЙ С ПЛИТОЙ ПО ГРУНТУ (strip_with_slab) ---
  const stripWithSlabConcreteVolumeM3 =
    stripConcreteVolumeM3 + footingArea * 0.1 * soilConcreteMultiplier;
  const stripWithSlabRebarKg =
    reinforcementBarKgStrip + footingArea * 4.5 * soilRebarMultiplier;
  const stripWithSlabSandM3 =
    stripSandVolumeM3 + footingArea * 0.15 * (1 + slopeFrac * 0.5);
  ...
  const rebarBindingCostStripWithSlabMDL = Math.round(
    stripWithSlabMaterials.reinforcementBarKg *
      COST_RATES.REBAR_BINDING_LABOR_MDL_KG,
  );
  const slopeComplicationCostStripWithSlabMDL = slopeComplicationCostStripMDL;
  const stripWithSlabCost = compileDetailedBudget(
    stripWithSlabMaterials,
    excavationCostStripWithSlabMDL,
    rebarBindingCostStripWithSlabMDL,
    drainageCostMDL,
    slopeComplicationCostStripWithSlabMDL,
    input,
  );
  ...
  const drilledPilesConcreteVolumeM3 =
    (drilledPilesCount * (Math.PI * 0.15 * 0.15 * drilledPilesDepthM) +
      perimeter * 0.4 * 0.45) *
    soilConcreteMultiplier;
  const rDrilledPiles = resolveReinforcementParams(input, "drilled_piles", {
    perimeter,
    footingArea,
    depthM: drilledPilesDepthM,
    concreteVolumeM3: drilledPilesConcreteVolumeM3,
  });
  const drilledPilesRebarKg = Math.round(
    rDrilledPiles.total_rebar_weight_kg || 0,
  );
  const drilledPilesSandM3 = perimeter * 0.4 * 0.15 * (1 + slopeFrac * 0.5);
  const drilledPilesExcavationM3 =
    drilledPilesCount * (Math.PI * 0.15 * 0.15 * drilledPilesDepthM) +
    perimeter * 0.4 * 0.3;
  const drilledPilesFormworkM2 = perimeter * 2 * 0.45;
  const drilledPilesWaterproofingM2 =
    drilledPilesCount * (Math.PI * 0.3 * drilledPilesDepthM) +
    perimeter * 0.45 * 2;
  ...
  const drilledPilesMaterials: MaterialRequirement = {
    ...
    rebarTransverseKg: Math.round(
      (rDrilledPiles.secondary_bars_weight_kg || 0) +
        (rDrilledPiles.additional_elements_weight_kg || 0),
    ),
    ...
    waterproofProtMembraneM2: Math.ceil(
      perimeter * drilledPilesFormworkM2 * 0.1,
    ),
    roughFloorAreaM2: footingArea,
  ...
  const excavationCostDrilledPilesMDL =
    Math.round(
      drilledPilesMaterials.excavationVolumeM3 * COST_RATES.EXCAVATION_MDL_M3,
    ) +
    drilledPilesCount *
      COST_RATES.PILE_DRILLING_MDL_M *
      drilledPilesDepthM *
      0.5;
  const rebarBindingCostDrilledPilesMDL = Math.round(
    drilledPilesMaterials.reinforcementBarKg *
      COST_RATES.REBAR_BINDING_LABOR_MDL_KG,
  );
  const slopeComplicationCostDrilledPilesMDL = Math.round(
    perimeter * (input.landSlope / 100) * 400,
  );
  const drilledPilesCost = compileDetailedBudget(
    drilledPilesMaterials,
    excavationCostDrilledPilesMDL,
    rebarBindingCostDrilledPilesMDL,
    drainageCostMDL,
    slopeComplicationCostDrilledPilesMDL,
    input,
  );

  // --- OPTION 8: МОНОЛИТНАЯ РЕБРИСТАЯ ПЛИТА (ribbed_slab) ---
  const ribbedSlabConcreteVolumeM3 =
    (footingArea * 0.16 + L_total * 0.3 * 0.3) * soilConcreteMultiplier;
  const rRibbedSlab = resolveReinforcementParams(input, "ribbed_slab", {
    perimeter,
    footingArea,
    depthM: 0.4,
    concreteVolumeM3: ribbedSlabConcreteVolumeM3,
  });
  ...
  const ribbedSlabMaterials: MaterialRequirement = {
    ...
    rebarTransverseKg: Math.round(
      (rRibbedSlab.secondary_bars_weight_kg || 0) +
        (rRibbedSlab.additional_elements_weight_kg || 0),
    ),
  ...
  const excavationCostRibbedSlabMDL = Math.round(
    ribbedSlabMaterials.excavationVolumeM3 * COST_RATES.EXCAVATION_MDL_M3,
  );
  const rebarBindingCostRibbedSlabMDL = Math.round(
    ribbedSlabMaterials.reinforcementBarKg *
      COST_RATES.REBAR_BINDING_LABOR_MDL_KG,
  );
  const slopeComplicationCostRibbedSlabMDL =
    input.landSlope > 0
      ? Math.round(footingArea * (input.landSlope / 100) * 950)
      : 0;
  const ribbedSlabCost = compileDetailedBudget(
    ribbedSlabMaterials,
    excavationCostRibbedSlabMDL,
    rebarBindingCostRibbedSlabMDL,
    drainageCostMDL,
    slopeComplicationCostRibbedSlabMDL,
    input,
  );
  ...
  const columnConcreteVolumeM3 =
    (columnCount * (0.4 * 0.4 * columnDepthM) + perimeter * 0.3 * 0.4) *
    soilConcreteMultiplier;
  const rColumn = resolveReinforcementParams(input, "column_footing", {
    perimeter,
    footingArea,
    depthM: columnDepthM,
    concreteVolumeM3: columnConcreteVolumeM3,
  });
  ...
  const columnFormworkM2 =
    columnCount * 4 * 0.4 * columnDepthM + perimeter * 2 * 0.4;
  ...
  const columnMaterials: MaterialRequirement = {
    ...
    rebarTransverseKg: Math.round(
      (rColumn.secondary_bars_weight_kg || 0) +
        (rColumn.additional_elements_weight_kg || 0),
    ),
  ...
  const excavationCostColumnMDL = Math.round(
    columnMaterials.excavationVolumeM3 * COST_RATES.EXCAVATION_MDL_M3,
  );
  const rebarBindingCostColumnMDL = Math.round(
    columnMaterials.reinforcementBarKg * COST_RATES.REBAR_BINDING_LABOR_MDL_KG,
  );
  const slopeComplicationCostColumnMDL = Math.round(
    perimeter * (input.landSlope / 100) * 350,
  );
  const columnCost = compileDetailedBudget(
    columnMaterials,
    excavationCostColumnMDL,
    rebarBindingCostColumnMDL,
    0,
    slopeComplicationCostColumnMDL,
    input,
  );
  ...
  const tisePileConcreteM3 =
    (tisePileCount * (Math.PI * 0.125 * 0.125 * tisePileDepthM + 0.045) +
      perimeter * 0.3 * 0.45) *
    soilConcreteMultiplier;
  const rTise = resolveReinforcementParams(input, "tise", {
    perimeter,
    footingArea,
    depthM: tisePileDepthM,
    concreteVolumeM3: tisePileConcreteM3,
  });
  const tiseRebarKg = Math.round(rTise.total_rebar_weight_kg || 0);
  const tiseSandM3 = tisePileCount * 0.05;
  const tiseExcavationM3 =
    tisePileCount * (Math.PI * 0.125 * 0.125 * tisePileDepthM + 0.05) +
    perimeter * 0.3 * 0.15;
  ...
  const tiseMaterials: MaterialRequirement = {
    ...
    rebarTransverseKg: Math.round(
      (rTise.secondary_bars_weight_kg || 0) +
        (rTise.additional_elements_weight_kg || 0),
    ),
  ...
  const excavationCostTiseMDL =
    Math.round(
      tiseMaterials.excavationVolumeM3 * COST_RATES.EXCAVATION_MDL_M3,
    ) +
    (tisePileCount * COST_RATES.PILE_DRILLING_MDL_M * tisePileDepthM * 0.5 +
      tisePileCount * 450); // ТИСЭ уширитель
  const rebarBindingCostTiseMDL = Math.round(
    tiseMaterials.reinforcementBarKg * COST_RATES.REBAR_BINDING_LABOR_MDL_KG,
  );
  const slopeComplicationCostTiseMDL = Math.round(
    perimeter * (input.landSlope / 100) * 350,
  );
  const tiseCost = compileDetailedBudget(
    tiseMaterials,
    excavationCostTiseMDL,
    rebarBindingCostTiseMDL,
    drainageCostMDL,
    slopeComplicationCostTiseMDL,
    input,
  );
  ...
  const allCosts = [
    stripCost.totalCostMDL,
    slabCost.totalCostMDL,
    pileCost.totalCostMDL,
    classicSlabCost.totalCostMDL,
    mzlfCost.totalCostMDL,
    stripWithSlabCost.totalCostMDL,
    drilledPilesCost.totalCostMDL,
    ribbedSlabCost.totalCostMDL,
    columnCost.totalCostMDL,
  ...
  // 1. Strip
  const stripCostScore = Math.round(
    100 - ((stripCost.totalCostMDL - minCost) / costSpread) * 75,
  );
  const stripRiskScore = seismicPoints === 8 ? 80 : 88;
  const stripEnergyScore = 30;
  const stripGeologyScore =
    input.soilType === SoilType.ROCK
      ? 98
      : input.soilType === SoilType.LOESS
        ? 70
  ...
  const finalStripReliability = Math.round(
    (seismicPoints === 8 ? 82 : 88) * 0.25 +
      stripGeologyScore * 0.25 +
      stripRiskScore * 0.2 +
      stripComplexity * 0.15 +
      stripEnergyScore * 0.15,
  );

  // 2. Slab (USH)
  const slabCostScore = Math.round(
    100 - ((slabCost.totalCostMDL - minCost) / costSpread) * 75,
  );
  const slabRiskScore = 95;
  const slabEnergyScore = 100;
  const slabGeologyScore =
    input.soilType === SoilType.ROCK
      ? 85
      : input.soilType === SoilType.LOESS
        ? input.groundwaterDepth < 1.5
  ...
  const finalSlabReliability = Math.round(
    (seismicPoints === 8 ? 90 : 95) * 0.25 +
      slabGeologyScore * 0.25 +
      slabRiskScore * 0.2 +
      slabComplexity * 0.15 +
      slabEnergyScore * 0.15,
  );

  // 3. Piles
  const pileCostScore = Math.round(
    100 - ((pileCost.totalCostMDL - minCost) / costSpread) * 75,
  );
  const pileRiskScore = 75;
  const pileEnergyScore = 20;
  const pileGeologyScore =
    input.soilType === SoilType.ROCK
      ? 30
      : input.soilType === SoilType.LOESS
        ? 92
  ...
  const finalPileReliability = Math.round(
    (seismicPoints === 8 ? 72 : 82) * 0.25 +
      pileGeologyScore * 0.25 +
      pileRiskScore * 0.2 +
      pileComplexity * 0.15 +
      pileEnergyScore * 0.15,
  );

  // 4. Classic Slab
  const classicSlabCostScore = Math.round(
    100 - ((classicSlabCost.totalCostMDL - minCost) / costSpread) * 75,
  );
  const classicSlabRiskScore = 90;
  const classicSlabComplexity = 70;
  const classicSlabEnergyScore = 55;
  const classicSlabGeologyScore =
    input.soilType === SoilType.ROCK
      ? 90
      : input.soilType === SoilType.LOESS
        ? 78
  ...
  const classicSlabReliability = Math.round(
    (seismicPoints === 8 ? 88 : 92) * 0.25 +
      classicSlabGeologyScore * 0.25 +
      classicSlabRiskScore * 0.2 +
      classicSlabComplexity * 0.15 +
      classicSlabEnergyScore * 0.15,
  );

  // 5. MZLF
  const mzlfCostScore = Math.round(
    100 - ((mzlfCost.totalCostMDL - minCost) / costSpread) * 75,
  );
  const mzlfRiskScore = 60;
  const mzlfComplexity = 80;
  const mzlfEnergyScore = 30;
  const mzlfGeologyScore =
    input.soilType === SoilType.ROCK
      ? 95
      : input.soilType === SoilType.LOESS
        ? 40
  ...
  const mzlfReliability = Math.round(
    (seismicPoints === 8 ? 55 : 70) * 0.25 +
      mzlfGeologyScore * 0.25 +
      mzlfRiskScore * 0.2 +
      mzlfComplexity * 0.15 +
      mzlfEnergyScore * 0.15,
  );

  // 6. Strip with Slab
  const stripWithSlabCostScore = Math.round(
    100 - ((stripWithSlabCost.totalCostMDL - minCost) / costSpread) * 75,
  );
  const stripWithSlabRiskScore = 92;
  const stripWithSlabComplexity = 55;
  const stripWithSlabEnergyScore = 50;
  const stripWithSlabGeologyScore =
    input.soilType === SoilType.ROCK
      ? 95
      : input.soilType === SoilType.LOESS
        ? 85
  ...
  const stripWithSlabReliability = Math.round(
    (seismicPoints === 8 ? 90 : 94) * 0.25 +
      stripWithSlabGeologyScore * 0.25 +
      stripWithSlabRiskScore * 0.2 +
      stripWithSlabComplexity * 0.15 +
      stripWithSlabEnergyScore * 0.15,
  );

  // 7. Drilled Piles
  const drilledPilesCostScore = Math.round(
    100 - ((drilledPilesCost.totalCostMDL - minCost) / costSpread) * 75,
  );
  const drilledPilesRiskScore = 75;
  const drilledPilesComplexity = 65;
  const drilledPilesEnergyScore = 20;
  const drilledPilesGeologyScore =
    input.soilType === SoilType.ROCK
      ? 25
      : input.soilType === SoilType.LOESS
        ? 90
  ...
  const drilledPilesReliability = Math.round(
    (seismicPoints === 8 ? 72 : 80) * 0.25 +
      drilledPilesGeologyScore * 0.25 +
      drilledPilesRiskScore * 0.2 +
      drilledPilesComplexity * 0.15 +
      drilledPilesEnergyScore * 0.15,
  );

  // 8. Ribbed Slab
  const ribbedSlabCostScore = Math.round(
    100 - ((ribbedSlabCost.totalCostMDL - minCost) / costSpread) * 75,
  );
  const ribbedSlabRiskScore = 88;
  const ribbedSlabComplexity = 60;
  const ribbedSlabEnergyScore = 50;
  const ribbedSlabGeologyScore =
    input.soilType === SoilType.ROCK
      ? 88
      : input.soilType === SoilType.LOESS
        ? 88
  ...
  const ribbedSlabReliability = Math.round(
    (seismicPoints === 8 ? 85 : 90) * 0.25 +
      ribbedSlabGeologyScore * 0.25 +
      ribbedSlabRiskScore * 0.2 +
      ribbedSlabComplexity * 0.15 +
      ribbedSlabEnergyScore * 0.15,
  );

  // 9. Column Footing
  const columnCostScore = Math.round(
    100 - ((columnCost.totalCostMDL - minCost) / costSpread) * 75,
  );
  const columnRiskScore = 40;
  const columnComplexity = 90;
  const columnEnergyScore = 10;
  const columnGeologyScore =
    input.soilType === SoilType.ROCK
      ? 75
      : input.soilType === SoilType.LOESS
        ? 15
  ...
  const columnReliability = Math.round(
    (seismicPoints === 8 ? 30 : 50) * 0.25 +
      columnGeologyScore * 0.25 +
      columnRiskScore * 0.2 +
      columnComplexity * 0.15 +
      columnEnergyScore * 0.15,
  );

  // 10. TISE
  const tiseCostScore = Math.round(
    100 - ((tiseCost.totalCostMDL - minCost) / costSpread) * 75,
  );
  const tiseRiskScore = 80;
  const tiseComplexity = 60;
  const tiseEnergyScore = 20;
  const tiseGeologyScore =
    input.soilType === SoilType.ROCK
      ? 35
      : input.soilType === SoilType.LOESS
        ? 95
  ...
  const tiseReliability = Math.round(
    (seismicPoints === 8 ? 78 : 84) * 0.25 +
      tiseGeologyScore * 0.25 +
      tiseRiskScore * 0.2 +
      tiseComplexity * 0.15 +
      tiseEnergyScore * 0.15,
  );
  ...
  options.push({
    ...
    bimEntities: generateBIMEntities(
      "strip",
      input,
      stripMaterials,
      stripCost,
    ...
    reinforcement: resolveReinforcementParams(input, "strip", {
      perimeter,
      footingArea,
      depthM: stripDepthM,
      concreteVolumeM3: stripMaterials.concreteVolumeM3,
    }),
  });

  // Calculate FBS differences
  const fbsBlocksCount = Math.round((stripConcreteVolumeM3 * 0.85) / 0.5); // block is ~0.5 m3
  const fbsConcreteM3 = stripConcreteVolumeM3 * 0.15; // monolithic belts
  const fbsCostMDL =
    stripCost.totalCostMDL -
    stripMaterials.concreteVolumeM3 * 1800 +
    fbsBlocksCount * 1200 +
    15000 /* crane */;

  options.push({
    id: "fbs",
    type: "Сборный из фундаментных блоков (ФБС)",
    nameRu: "Сборный ленточный фундамент из блоков ФБС с монолитными поясами",
    isRecommended: stripIsRecommended && !input.hasPit,
    costMDL: fbsCostMDL,
    reliabilityScore: finalStripReliability - 5,
    complexityScore: stripComplexity - 15,
    pros: [
      "Высокая скорость монтажа",
      "Технически прост в исполнении",
      "Подходит для глубоких подвалов",
    ],
    cons: [
      "Требуется автокран для монтажа",
      "Швы между блоками уязвимы к влаге (нужна качественная гидроизоляция)",
      "Меньшая пространственная жесткость по сравнению с монолитом",
    ],
    risks: stripRisks,
    materials: {
      ...stripMaterials,
      concreteVolumeM3: fbsConcreteM3,
      fbsBlocksCount: fbsBlocksCount,
    },
    costEstimate: { ...stripCost, totalCostMDL: fbsCostMDL },
    widthM: requiredStripWidthM,
    depthM: Math.round(stripDepthM * 100) / 100,
    bimEntities: generateBIMEntities(
      "strip",
    ...
    costScore: stripCostScore + 5,
    riskScore: stripRiskScore - 5,
    energyScore: stripEnergyScore - 5,
    geologyScore: stripGeologyScore - 10,
    totalScore: finalStripReliability - 5,
    reinforcement: resolveReinforcementParams(input, "strip", {
      perimeter,
      footingArea,
      depthM: stripDepthM,
  ...
  options.push({
    ...
    type: "Утепленная шведская плита (УШП)",
    nameRu:
      "Мелкозаглубленная утепленная шведская плита (УШП) по специальному теплотехническому расчету",
    ...
    reinforcement: resolveReinforcementParams(input, "slab", {
      perimeter,
      footingArea,
      depthM: slabDepthM,
      concreteVolumeM3: slabMaterials.concreteVolumeM3,
    }),
  });
  ...
  options.push({
    ...
    reinforcement: resolveReinforcementParams(input, "piles", {
      perimeter,
      footingArea,
      depthM: 2.2,
      concreteVolumeM3: pileMaterials.concreteVolumeM3,
    }),
  });
  ...
  options.push({
    ...
    reinforcement: resolveReinforcementParams(input, "classic_slab", {
      perimeter,
      footingArea,
      depthM: classicSlabDepthM,
      concreteVolumeM3: classicSlabMaterials.concreteVolumeM3,
    }),
  });
  ...
  options.push({
    ...
    reinforcement: resolveReinforcementParams(input, "mzlf", {
      perimeter,
      footingArea,
      depthM: mzlfDepthM,
      concreteVolumeM3: mzlfMaterials.concreteVolumeM3,
    }),
  });
  ...
  options.push({
    ...
    reinforcement: resolveReinforcementParams(input, "strip_with_slab", {
      perimeter,
      footingArea,
      depthM: stripDepthM,
      concreteVolumeM3: stripWithSlabMaterials.concreteVolumeM3,
    }),
  });
  ...
  options.push({
    ...
    reinforcement: resolveReinforcementParams(input, "drilled_piles", {
      perimeter,
      footingArea,
      depthM: drilledPilesDepthM,
      concreteVolumeM3: drilledPilesMaterials.concreteVolumeM3,
    }),
  });
  ...
  options.push({
    ...
    reinforcement: resolveReinforcementParams(input, "ribbed_slab", {
      perimeter,
      footingArea,
      depthM: 0.45,
      concreteVolumeM3: ribbedSlabMaterials.concreteVolumeM3,
    }),
  });
  ...
  options.push({
    ...
    reinforcement: resolveReinforcementParams(input, "column_footing", {
      perimeter,
      footingArea,
      depthM: columnDepthM,
      concreteVolumeM3: columnMaterials.concreteVolumeM3,
    }),
  });
  ...
  options.push({
    ...
    nameRu:
      "Технологический фундамент ТИСЭ с уширением пяты свай и висячим ростверком",
    ...
    reinforcement: resolveReinforcementParams(input, "tise", {
      perimeter,
      footingArea,
      depthM: tisePileDepthM,
      concreteVolumeM3: tiseMaterials.concreteVolumeM3,
    }),
  });

  if (input.buildingType === "GARAGE") {
    options.length = 0; // Clear house options

    // GARAGE SLAB (Without Pit)
    const garSlabDepthM = 0.2; // 20cm slab for cars
    const garSlabConcreteVolumeM3 =
      footingArea * garSlabDepthM * soilConcreteMultiplier;
    const garSlabSandVolumeM3 = footingArea * 0.2;
    const garSlabFormworkM2 = perimeter * garSlabDepthM;
    const garReinforcementBarKg = footingArea * 12; // ~12kg/m2 for 20cm slab
    const garExcavationM3 = footingArea * 0.4;

    const garSlabMaterials: MaterialRequirement = {
      concreteVolumeM3: Math.ceil(garSlabConcreteVolumeM3),
      reinforcementBarKg: Math.ceil(garReinforcementBarKg),
      sandGravelM3: Math.ceil(garSlabSandVolumeM3),
      waterproofingM2: Math.ceil(footingArea * 1.1),
      insulationM3: 0, // usually no insulation unless heated
      formworkM2: Math.ceil(garSlabFormworkM2),
      formworkBoardsCount: Math.ceil(garSlabFormworkM2 / 0.9),
      excavationVolumeM3: Math.ceil(garExcavationM3),
      sandGravelWeightTons: Math.ceil(garSlabSandVolumeM3 * 1.6),
      rebarLongitudinalKg: Math.ceil(garReinforcementBarKg * 0.8),
      rebarTransverseKg: Math.ceil(garReinforcementBarKg * 0.2),
      rebarLongitudinalDiameter: 12,
      rebarTransverseDiameter: 8,
    };
    const garSlabCost = compileDetailedBudget(
      garSlabMaterials,
      garExcavationM3 * COST_RATES.EXCAVATION_MDL_M3,
      garReinforcementBarKg * COST_RATES.REBAR_BINDING_LABOR_MDL_KG,
      0,
      0,
      input,
    );
    ...
    options.push({
      id: "slab",
      type: "Монолитная фундаментная плита (Гараж)",
      nameRu: "Плитный фундамент под гараж (20 см)",
      isRecommended: !input.hasPit,
      costMDL: garSlabCost.totalCostMDL,
      reliabilityScore: 90,
      complexityScore: 50,
      pros: [
        "Распределяет нагрузку от автомобиля",
        "Служит готовым полом гаража",
        "Устойчив к пучению",
      ],
      cons: [
        "Невозможно обустроить смотровую яму",
        "Требует ровного основания",
      ],
      risks: ["Трещины при недостаточном армировании под колесами"],
      materials: garSlabMaterials,
      costEstimate: garSlabCost,
      widthM: Math.max(input.width, input.length),
      depthM: garSlabDepthM,
      bimEntities: [],
      costScore: 85,
      riskScore: 90,
      energyScore: 60,
      ...
      reinforcement: resolveReinforcementParams(input, "slab", {
        ...
        footingArea,
        depthM: garSlabDepthM,
        concreteVolumeM3: garSlabConcreteVolumeM3,
      }),
    });

    // GARAGE STRIP
    // If hasPit, make it deep. If no pit, make it shallow strip.
    const pitDepthM = input.hasPit ? 1.8 : 0.8;
    const garStripDepthM = pitDepthM + 0.2;
    const garStripWidthM = 0.3;
    const lTotal = perimeter + (input.hasPit ? input.width * 2 : 0);
    const garStripConcreteVolumeM3 = lTotal * garStripWidthM * garStripDepthM;
    const garStripExcavationM3 = input.hasPit
      ? footingArea * pitDepthM
      : lTotal * garStripWidthM * garStripDepthM * 1.5;
    const garStripFormworkM2 = lTotal * 2 * garStripDepthM;
    const garStripRebarKg = garStripConcreteVolumeM3 * 40;

    const garStripMaterials: MaterialRequirement = {
      concreteVolumeM3: Math.ceil(garStripConcreteVolumeM3),
      reinforcementBarKg: Math.ceil(garStripRebarKg),
      sandGravelM3: Math.ceil(lTotal * garStripWidthM * 0.2),
      waterproofingM2: Math.ceil(garStripFormworkM2 / 2),
      insulationM3: 0,
      formworkM2: Math.ceil(garStripFormworkM2),
      formworkBoardsCount: Math.ceil(garStripFormworkM2 / 0.9),
      excavationVolumeM3: Math.ceil(garStripExcavationM3),
      sandGravelWeightTons: Math.ceil(lTotal * garStripWidthM * 0.2 * 1.6),
      rebarLongitudinalKg: Math.ceil(garStripRebarKg * 0.7),
      rebarTransverseKg: Math.ceil(garStripRebarKg * 0.3),
      rebarLongitudinalDiameter: 12,
      rebarTransverseDiameter: 8,
    };

    const garStripCost = compileDetailedBudget(
      garStripMaterials,
      garStripExcavationM3 * COST_RATES.EXCAVATION_MDL_M3,
      garStripRebarKg * COST_RATES.REBAR_BINDING_LABOR_MDL_KG,
      0,
      0,
      input,
    );
    ...
    options.push({
      id: "strip",
      type: input.hasPit
        ? "Ленточный фундамент + Смотровая яма"
        : "Мелкозаглубленный ленточный (МЗЛФ)",
      nameRu: input.hasPit
        ? "Ленточный фундамент глубокого заложения со смотровой ямой"
        : "Лента МЗЛФ под легкий гараж",
      isRecommended: input.hasPit,
      costMDL: garStripCost.totalCostMDL,
      reliabilityScore: 85,
      complexityScore: 85,
      pros: input.hasPit
        ? [
            "Полноценная смотровая яма для авто",
            "Надежное опирание ниже глубины промерзания",
          ]
        : ["Экономия бетона", "Достаточно для легких стен"],
      cons: [
        "Большой объем земляных работ",
        "Отдельная заливка полов",
        "Сложнее устроить заезд",
      ],
      risks: ["Риск пучения при МЗЛФ или подтопления ямы грунтовыми водами"],
      materials: garStripMaterials,
      costEstimate: garStripCost,
      widthM: garStripWidthM,
      depthM: garStripDepthM,
      bimEntities: [],
      costScore: 60,
      riskScore: 70,
      energyScore: 50,
      ...
      reinforcement: resolveReinforcementParams(input, "strip", {
        ...
        footingArea,
        depthM: garStripDepthM,
        concreteVolumeM3: garStripConcreteVolumeM3,
      }),
    });

    // Push a dummy piles option so `results.options[2]` exists if UI tries to map 3 items
    options.push({
      ...options[1],
      id: "piles",
      type: "Свайный под гараж (Альтернатива)",
      nameRu: "Буронабивные сваи с ростверком",
      isRecommended: false,
    });
  }
  ...
  const characteristicLoadKN =
    (Gk_dead_total + Qk_live + Qk_snow + Qk_wind) * 9.81;
  // Effective active area of the foundation elements in contact with the soil
  const activeAreaM2 = Math.max(
    bearingAreaRequiredM2 * 1.1,
    input.width * input.length * 0.15,
  );
  const p_contact_kPa = Math.min(
    soilBearingCapacityKPa,
    characteristicLoadKN / activeAreaM2,
  );
  ...
  if (input.groundwaterDepth < 1.0) {
    if (
      ...
      input.soilType === SoilType.SILTY_SAND ||
      input.soilType === SoilType.SANDY_LOAM
    ) {
      Kd = 1.3;
    }
  }
  ...
  if (input.soilType === SoilType.ROCK) {
    ...
    settlementTotalMM =
      Math.round(
        (1.5 + Math.min(1.5, (characteristicLoadKN / 3000) * 1.5)) * 100,
      ) / 100;
  } else {
    // S = p * b * (1 - nu^2) * omega / E (in meters) multiplied by 1000 to get mm, then by groundwater factor Kd
    const rawSettlement =
      ((p_contact_kPa * b_eq * (1 - nu * nu) * omega) / E_avg_kPa) * 1000 * Kd;
    ...
    input.soilType === SoilType.ROCK
      ...
      : input.soilType === SoilType.FILLED
        ...
        : input.soilType === SoilType.LOESS
          ? 0.4
          : 0.25;
  const settlementDiffMM =
    input.soilType === SoilType.ROCK
      ? 0.0
      : Math.round(settlementTotalMM * homogeneityCoeff * 100) / 100;
  const length_mm = input.length * 1000;
  const settlementUnequal =
    Math.round((settlementDiffMM / length_mm) * 100000) / 100000;
  ...
  const settlementRiskCoeff =
    Math.round((settlementTotalMM / settlementLimitMM) * 100) / 100;
  const requiresGeotechnicalSurvey =
    settlementRiskCoeff > 0.6 ||
    input.soilType === SoilType.FILLED ||
    input.soilType === SoilType.LOESS ||
    input.landSlope > 8.0;
  ...
  // ======================================================================
  const geologyDetailsResolved = calculateGeologyDetails(
    input,
    soilBearingCapacityKPa,
    reg,
    soil,
  ...
  const selectedOptionForRiskAndCuring =
    options.find((o) => o.isRecommended) || options[0];
  const selectedConcreteM3 =
    selectedOptionForRiskAndCuring.materials.concreteVolumeM3 || 25;
  const selectedExcavationM3 =
    selectedOptionForRiskAndCuring.materials.excavationVolumeM3 || 50;

  const concreteCuringResolved = calculateConcreteCuring(
    input,
    footingArea,
    selectedConcreteM3,
  );
  const backfillResolved = calculateBackfill(
    input,
    selectedExcavationM3,
    selectedConcreteM3,
  );
  ...
  const enrichedOptions = enrichFoundationWithProofs(options, input);
  ...
  return {
    ...
    seismicMassCombinationTons:
      Math.round(seismicMassCombinationTons * 10) / 10,
...
function compileDetailedBudget(
  ...
  m.drainageStoneM3 = Math.ceil(m.drainagePipeM * 0.4 * 0.35);
  ...
  m.backfillVolumeM3 = Math.round(
    Math.max(15, excavationVol - concreteVol * 0.6),
  );
  ...
  const concreteCostMDL = Math.round(
    m.concreteVolumeM3 * COST_RATES.CONCRETE_MDL_M3,
  );
  const steelCostMDL = Math.round(
    m.reinforcementBarKg * COST_RATES.STEEL_MDL_KG,
  );
  const sandCushionCostMDL = Math.round(
    m.sandGravelM3 * COST_RATES.SAND_GRAVEL_MDL_M3,
  );
  ...
  const formworkCostMDL = Math.round(
    (m.formworkM2 || 0) * COST_RATES.FORMWORK_MDL_M2,
  );

  const materialsSubtotalMDL =
    concreteCostMDL +
    steelCostMDL +
    sandCushionCostMDL +
    waterproofInsulationCostMDL +
    formworkCostMDL;

  const constructionLaborCostMDL = Math.round(
    m.concreteVolumeM3 * 800 +
      rebarBindingCostMDL +
      (m.hasDrainage ? m.drainagePipeM || 0 : 0) * 85,
  );

  const machineryLogisticsCostMDL = Math.round(
    excavationCostMDL + m.concreteVolumeM3 * 120 + 4000,
  );

  // Расчет стоимости устройства чернового пола по грунту внутри цоколя (для ленточного и свайно-ростверкового)
  const rFloorConcreteCost = Math.round(
    (m.roughFloorConcreteM3 || 0) * COST_RATES.CONCRETE_MDL_M3,
  );
  const rFloorSteelCost = Math.round(
    (m.roughFloorRebarKg || 0) * COST_RATES.STEEL_MDL_KG,
  );
  const rFloorSandCost = Math.round(
    (m.roughFloorSandM3 || 0) * COST_RATES.SAND_GRAVEL_MDL_M3,
  );
  const rFloorWaterproofCost = Math.round(
    (m.roughFloorWaterproofingM2 || 0) * 35,
  ); // 35 MDL/м² за два слоя пленки 150-200мкм
  ...
  const roughFloorCostMDL =
    rFloorConcreteCost +
    rFloorSteelCost +
    rFloorSandCost +
    rFloorWaterproofCost +
    rFloorLaborCost;

  // Добавление отдельного учета бурения свай пог.м
  const pileDrillingCostMDL = Math.round(
    (m.pileDrillingM || 0) * COST_RATES.PILE_DRILLING_MDL_M,
  );

  const subtotalWithSlopeDrain =
    materialsSubtotalMDL +
    constructionLaborCostMDL +
    machineryLogisticsCostMDL +
    detailedDrainageCostMDL +
    slopeComplicationCostMDL +
    roughFloorCostMDL +
    pileDrillingCostMDL +
    engineeringNetworksCostMDL +
    groundingSystemCostMDL +
    backfillCompactionCostMDL +
    waterproofProtectionCostMDL;

  // 1. VAT (20%) - Moldovan tax on materials and construction works
  const useVAT = input?.includeVAT !== false;
  const vatMDL = useVAT ? Math.round(subtotalWithSlopeDrain * 0.2) : 0;
  ...
  const reservePercent =
    input?.projectReservePercent !== undefined
  ...
  const engineeringReserveMDL = Math.round(
    (subtotalWithSlopeDrain + vatMDL) * reservePercent,
  );

  const totalCostMDL =
    subtotalWithSlopeDrain +
    vatMDL +
    designCostMDL +
    geologyCostMDL +
    supervisionCostMDL +
    expertiseCostMDL +
    engineeringReserveMDL;
...
export function generateBIMEntities(
  ...
  // 1. GEOLOGY
  const geoPass =
    input.soilType !== SoilType.FILLED && input.soilType !== SoilType.LOESS;
  entities.push({
    id: "GEOLOGY",
    ...
    materials: [
      {
        name: "Технический отчет по геологической экспертизе грунта",
        qty: 1,
        unit: "копл.",
        cost: c.geologyCostMDL || 9500,
      },
    ],
  ...
  entities.push({
    ...
    materials: [
      {
        name: "Лабораторные акты динамического пенетрационного тестирования",
        qty: 4,
        unit: "точек",
        cost: 2400,
      },
  ...
  entities.push({
    ...
    materials: [
      {
        name: "Пьезометрическая зондирующая трубка d50 из нержавеющей стали",
        qty: 1,
        unit: "шт",
        cost: 2500,
      },
    ...
    materials: [
      {
        ...
        qty: Math.ceil(sewerLength / 3),
        unit: "шт",
        cost: Math.round(sewerLength * 110),
      },
      {
        name: "Фитинги, компенсационные муфты, ревизионные люки",
        qty: 1,
  ...
  entities.push({
    ...
    nameRo: "Introducerea conductei de apă potabilă",
    parameters: {
      "Материал проводника питьевой воды":
        "ПНД PE100 SDR11 питьевая синяя черта",
    ...
    materials: [
      {
        ...
        qty: waterLength,
        unit: "м.п.",
        cost: Math.round(waterLength * 45),
      },
      {
        name: "Защитный рукав-футляр HDPE d50 мм",
        qty: waterLength,
        unit: "м.п.",
        cost: Math.round(waterLength * 60),
      },
    ],
    costMDL: Math.round(
      waterLength * 45 + waterLength * 60 + waterLength * 110,
    ),
    dependencies: ["GEOLOGY"],
    qualityChecks: [
      {
    ...
    materials: [
      {
        ...
        qty: powerLength,
        unit: "м.п.",
        cost: Math.round(powerLength * 35),
      },
      {
        name: "Стальная тяговая гибкая проволока ф2мм",
        qty: powerLength,
        unit: "м.п.",
        cost: Math.round(powerLength * 10),
      },
    ],
  ...
  entities.push({
    ...
    materials: [
      {
        name: "Труба ПНД гладкая жесткая черная d25 техническая",
        qty: lowLength,
        unit: "м.п.",
        cost: Math.round(lowLength * 28),
      },
    ],
  ...
  entities.push({
    ...
    parameters: {
      ...
      "Способ герметизации прохода":
        "Сальниковая набивка с водоотталкивающей мастикой",
    },
    volumes: {
    ...
    materials: [
      ...
      {
        name: "Комплекты сальников и расширяющихся пробок d160 для вводов",
        qty: sleeveCount,
        unit: "шт",
        cost: sleeveCount * 240,
      },
    ...
    costMDL: Math.round(
      sleeveCount * 180 + sleeveCount * 240 + sleeveCount * 120,
    ),
    dependencies: ["GEOLOGY"],
    qualityChecks: [
      {
  ...
  entities.push({
    ...
    parameters: {
      "Режим влажностного ухода":
        "Регулярный полив водой 3 раза в сутки с укрытием",
    ...
    materials: [
      ...
      {
        name: "Вода технологическая для полива и увлажнения",
        qty: 10,
        unit: "м³",
        cost: 1200,
      },
  ...
  entities.push({
    ...
    materials: [
      {
        name: "Карьерная супесчано-гравийная смесь с доставкой",
        qty: backfillVol,
        unit: "м³",
        cost: Math.round(backfillVol * 180),
      },
    ],
    costMDL: Math.round(
      backfillVol * 180 + backfillVol * 90 + backfillVol * 45,
    ),
  ...
  entities.push({
    ...
    materials: [
      {
        name: "Товарный бетон С12/15 (М200) с РБУ",
        qty: blindConcreteM3,
        unit: "м³",
        cost: Math.round(blindConcreteM3 * 1350),
      },
      {
        name: "Сварная дорожная армирующая сетка d4 яч.100х100мм",
        qty: blindAreaM2,
        unit: "м²",
        cost: Math.round(blindAreaM2 * 85),
      },
      {
        name: "Утеплитель экструдированный пенополистирол XPS 50мм",
        qty: blindXPSM3,
        unit: "м³",
        cost: Math.round(blindXPSM3 * 1400),
      },
    ],
    costMDL: Math.round(
      blindConcreteM3 * 1350 +
        blindAreaM2 * 85 +
        blindXPSM3 * 1400 +
        blindAreaM2 * 140,
    ),
  ...
  entities.push({
    ...
    parameters: {
      "Класс защитного барьера":
        "Профилированная HDPE мембрана Planter с шипами",
      "Высота фиксирующих шипов": "8 мм",
      "Материал жесткого полимера":
        "Полиэтилен высокой плотности высокой упругости",
    },
    volumes: {
    ...
    materials: [
      {
        name: "Мембрана профилированная HDPE Planter Стандарт 2х20м",
        qty: wrapM2,
        unit: "м²",
        cost: Math.round(wrapM2 * 75),
      },
      {
        name: "Крепежные полимерные грибы-дюбели, прижимные планки",
        qty: 1,
        unit: "компл",
        cost: 1800,
      },
  ...
  entities.push({
    ...
    parameters: {
      "Контроль подвижности смеси бетона":
        "Осадка конуса класса П3 (10-15см) на объекте",
      "Марка прочности ж/б конструкций":
        "В25 (C20/25) на 28-е сутки созревания",
      "Наличие обязательных актов скрытых работ":
        "Акты на армирование каркасов и приемку подошв на месте",
    },
    volumes: {
      "Лабораторный раздавливающий тест кубиков":
        "1 серия (3 формовочных кубика бетона)",
    },
    materials: [
      {
        name: "Официальный сертификат аттестованной стройлаборатории РМ",
        qty: 1,
        unit: "компл",
        cost: 3800,
      },
  ...
  entities.push({
    ...
    materials: [
      {
        name: "Служба доставки бетонной массы миксерами РБУ",
        qty: mixersCount,
        unit: "рейс",
        cost: mixersCount * 1800,
      },
      {
        name: "Аренда логистической платформы КАМАЗ спецрейсы",
        qty: 2,
        unit: "рейс",
        cost: 6000,
      },
    ...
    qualityChecks: [
      {
        criterion:
          "Ритмичность непрерывной укладки бетона для исключения слоев",
        status: "PASS",
  ...
  entities.push({
    ...
    parameters: {
      "Специалист строительного контроля":
        "Лицензированный сертифицированный технадзор РМ",
      "Ведение регламентного журнала работ":
        "Обязательное ведение общего журнала работ",
      "Регистрация ответственных конструкций":
        "Оформление актов скрытых работ по СНиП",
    },
    volumes: {
    ...
    materials: [
      {
        name: "Полное консультационное сопровождение инженера технадзора",
        qty: 1,
        unit: "сертификат",
        cost: 12000,
      },
  ...
  let mainDiameterCheckStatus: "CRITICAL" | "PASS" | "WARNING" | "FAIL" =
    "PASS";
  let mainDiameterCheckText = `Предусмотрен диаметр d${rCheck.main_bar_diameter}мм`;
  const buildingWeight = input.width * input.length * (input.floors || 1) * 1.5;
  const isHeavy =
    buildingWeight > 150 || input.wallMaterial === BuildingWallMaterial.KOTELET;
  ...
  if (
    rCheck.protective_layer_bottom < 35 ||
    rCheck.protective_layer_side < 35
  ) {
  ...
  if (
    ...
    reinforcementChecks.push({
      ...
      status:
        rCheck.corner_reinforcement && (rCheck.l_bars || rCheck.u_bars)
          ? "PASS"
          : "WARNING",
      value: rCheck.corner_reinforcement
        ? "Предусмотрено анкерование"
        : "Анкеровка отсутствует",
  ...
  entities.push({
    ...
    parameters: {
      ...
      "Узлы специального сопряжения углов": rCheck.corner_reinforcement
        ? "L- и U-образные анкеры"
        : "Обычная вязка внахлест",
    ...
    materials: [
      {
        name: `Арматурная сталь периодического профиля класс ${rCheck.rebar_class_main}`,
        qty: Math.round(m.reinforcementBarKg * 0.8),
        unit: "кг",
        cost: Math.round(m.reinforcementBarKg * 0.8 * 22),
      },
      {
        name: `Арматура гладкая хомутная класс ${rCheck.rebar_class_secondary}`,
        qty: Math.round(m.reinforcementBarKg * 0.2),
        unit: "кг",
        cost: Math.round(m.reinforcementBarKg * 0.2 * 22),
      },
      {
        name: "Пластиковые фиксаторы защитного слоя (звездочки/стульчики)",
        qty: rCheck.spacers_count,
        unit: "шт",
        cost: Math.round(rCheck.spacers_count * 2.5),
      },
    ],
